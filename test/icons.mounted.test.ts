...
import { ICON_DATA } from '../src/lib/iconData.ts';
import { requireValue } from './types/ui-test-types.ts';

const sourceRoot = join(process.cwd(), 'src');
const iconDataPath = join(sourceRoot, 'lib/iconData.ts');
...
] satisfies ReadonlyArray<readonly [keyof typeof ICON_DATA, string]>;
...
describe('offline icon coverage', () => {
  test('bundles exactly the icon string literals and dynamic known values used by source', () => {
    ...
    for (const { text } of scannedSource) {
      for (const match of text.matchAll(iconLiteralPattern)) usedNames.add(requireValue(match[2]));
    }
...
describe('offline icon rendering', () => {
  test.each(renderedIcons)('renders %s geometry and forwards SVG props', (name, label) => {
    ...
    const svg = requireValue(mounted.container.querySelector('svg'));
    const data = requireValue(ICON_DATA[name]);
    const left = 'left' in data ? data.left ?? 0 : 0;
    const top = 'top' in data ? data.top ?? 0 : 0;
  ...
  test('renders a visible diagnostic and never delegates a missing name', () => {
    ...
    const svg = requireValue(mounted.container.querySelector('[data-missing-icon="mdi:not-bundled"]'));

    expect(svg).not.toBeNull();
    expect(svg.getAttribute('width')).toBe('17');
    expect(svg.getAttribute('class')).toBe('missing');
    expect(requireValue(svg.querySelector('rect')).getAttribute('fill')).toBe('#ff00ff');
