...
import { validateClipTimelineState } from '../src/lib/clipTimeline.ts';
import {
  deterministicUuid,
...
function visualTrack(id: string, extra: Partial<VisualTimelineTrack> = {}): VisualTimelineTrack {
  return visualTimelineTrack({
    id,
    kind: 'visual',
...
function visualClip(
  ...
  return visualTimelineClip({
    ...
    frameKeys: [{
      tick: 0,
      value: { cells: { '0,0': cell(requireValue(id[0]).toUpperCase()) } },
    }],
    propertyTracks: {},
    ...extra,
  });
...
function operationTag(result: Record<string, unknown>): TimelineTag {
  const tag = record(result['tag'], 'Expected operation tag');
  assert.equal(typeof tag['id'], 'string');
  assert.equal(typeof tag['tick'], 'number');
  assert.equal(typeof tag['type'], 'string');
  return tag as unknown as TimelineTag;
}

function operationState(): ClipTimelineState {
...
test('canonical stores publish validated transactions and mutation revisions', () => {
  ...
  type ControllerView = Parameters<Parameters<typeof controller.subscribe>[0]>[0];
  const views: ControllerView[] = [];
  ...
  assert.equal('set' in controller.durationTicks, false);
  ...
  assert.deepEqual(requireValue(views[0]).tracks.map((track) => track.id), ['visual']);
  ...
  const edited = controller.editVisualFrame('visual', 1, (draft: unknown) => {
    const cells = record(record(draft)['cells']);
    record(cells['0,0'])['c'] = 'B';
  });
  ...
  const latestView = requireValue(views.at(-1));
  assert.equal(latestView.mutationRevision, 1);
  const latestValue = record(requireValue(requireValue(latestView.state.clips[0]).frameKeys[1]).value);
  assert.equal(record(record(latestValue['cells'])['0,0'])['c'], 'B');
...
test('capture, restore, and reset isolate history snapshots and context', () => {
  ...
  record(record(requireValue(requireValue(isolated.state.clips[0]).frameKeys[0]).value)['nested'])['value'] = 99;
...
test('edit, property, move, trim, razor, delete, ripple, and add-empty are atomic wrappers', () => {
  ...
  const frame = controller.editVisualFrame('visual', 1, (draft: unknown) => {
    const cells = record(record(draft)['cells']);
    record(cells['0,0'])['c'] = 'B';
  });
  ...
  assert.equal(record(split['right'])['id'], 'right');
  ...
  assert.equal(controller.getState().clips.find((clip) => clip.id === 'right'), undefined);
...
test('a deduplicated Razor path cuts distinct unlocked boundaries in one mutation', () => {
  ...
  requireValue(state.tracks.find((track) => track.id === 'c')).locked = true;
  const controller = createCanonicalClipTimelineController({
    initialState: state,
    makeId: createStableClipTimelineIdGenerator(),
    playheadTick: 4,
  ...
  assert.deepEqual(splits.map((split) => record(split)['originalId']), ['a1', 'b1']);
  assert.equal(controller.getMutationRevision(), 1);
...
test('middle frame and property deletion reconnects the canonical hold and interpolation', () => {
  ...
  const resolved = requireValue(controller.resolveLayersAtTick(2)[0]);
  assert.equal(requireValue(resolved.cells['0,0']).c, 'A');
...
test('project and mutation revision guards reject stale transactions and history', () => {
  ...
  assert.equal(requireValue(controller.getState().clips[0]).startTick, 1);
  ...
  assert.equal(requireValue(controller.getState().tracks[0]).id, 'new');
});
...
test('gesture guards preserve Undo, Redo, replacement, and no-op mutations', () => {
  ...
  assert.equal(requireValue(controller.getState().clips[0]).startTick, 0,
...
test('validation failures expose diagnostics without publishing invalid state', () => {
  assert.throws(() => createCanonicalClipTimelineController({
    ...
    assert.ok(error instanceof ClipTimelineStateValidationError);
    assert.equal(error.errors.some((message: string) => message.includes('Duplicate timeline id')), true);
    return true;
  });
  ...
  assert.throws(() => controller.transact('invalid-overlap', (state) => ({
    ...
    assert.ok(error instanceof ClipTimelineStateValidationError);
    assert.equal(error.errors.some((message: string) => message.includes('overlap')), true);
    return true;
  });
  assert.deepEqual(controller.getState(), before);
  assert.equal(controller.getMutationRevision(), 0);

  assert.throws(() => controller.setSelection({ clipIds: ['missing'] }), (error: unknown) => {
    assert.ok(error instanceof ClipTimelineStateValidationError);
...
test('canonical audio clips extend transport and remain in timeline history state', () => {
  const controller = createCanonicalClipTimelineController({
    initialState: {
      fps: 10,
      tracks: [visualTrack('visual'), timelineTrack('audio', 'audio')],
      clips: [
        visualClip('clip', 'visual', 0, 2),
        audioTimelineClip({
          id: 'audio-clip', trackId: 'audio', kind: 'audio', assetId: 'asset',
          startTick: 7, inTick: 0, outTick: 5, sourceDuration: 5,
          inPoint: 0.25, outPoint: 0.75, duration: 1,
...
test('sequence tags keep stable UUIDs, branch safely, and clamp when duration shrinks', () => {
  ...
  const start = operationTag(controller.setLoopStart(0));
  const end = operationTag(controller.setLoopEnd(5));
  const first = operationTag(controller.addCustomTag(5, 'event'));
  const second = operationTag(controller.addCustomTag(5, 'event'));
  assert.notEqual(first.id, second.id);
  assert.deepEqual(controller.getState().tags.map((tag) => [
    tag.type,
    tag.tick,
    tag.type === 'custom' ? tag.value : undefined,
  ]), [
  ...
  const movedStart = operationTag(controller.setLoopStart(3));
  ...
  const history = controller.captureState();
  assert.equal(operationTag(controller.updateCustomTag(first.id, { value: '世界' })).id, first.id);
  ...
  const branch = operationTag(controller.addCustomTag(5, 'branch'));
  ...
  assert.equal(requireValue(controller.getState().tags.find((tag) => tag.id === end.id)).tick, 1);
});
