import { planSubcellFill } from '../src/lib/fill.ts';
import { maskFromChar } from '../src/lib/subcell.ts';
import type { EditorCellMap, EditorCellValue } from '../src/lib/types/editor-domain.ts';
...
function reader(cells: EditorCellMap): (x: number, y: number) => EditorCellValue {
...
const blockGlyphs = ['', '▘', '▝', '▖', '▗', '▀', '▄', '▌', '▐', '▚', '▞', '▛', '▜', '▙', '▟', '█'];
let propertyFailure: unknown = null;
for (let run = 0; run < 32 && !propertyFailure; run++) {
  ...
  const args: Parameters<typeof planSubcellFill>[0] = {
    width, height, x, y,
  ...
  const next: EditorCellMap = { ...cells };
