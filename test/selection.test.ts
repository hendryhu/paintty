...
import { cmGet, cmKey } from '../src/lib/cellmap.ts';
...
import {
  ...
  shapeGlyphs as editorShapeGlyphs,
} from '../src/lib/shapes.ts';
import {
  PAINTTY_CLIPBOARD_MIME,
  PAINTTY_CLIPBOARD_TEXT,
  clipboardHasMatchingClipMarker,
  clipboardImageFile as editorClipboardImageFile,
  clipboardMediaPlacementSucceeded as editorClipboardMediaPlacementSucceeded,
  clipboardPasteIntent as editorClipboardPasteIntent,
  clearClipClipboard,
  copyClipsForContext,
  hasClipClipboard,
  pasteClipsFromClipboard as pasteEditorClipsFromClipboard,
...
import { importMediaFile as importEditorMediaFile } from '../src/lib/mediaCommands.ts';
...
} from '../src/lib/documentLifecycle.ts';
import type { ClipClipboardPayload } from '../src/lib/frames.ts';
import type {
  EditorBounds,
  EditorCellMap,
  EditorEffectLayer,
...
import type { MediaAssetInput, MediaKind } from '../src/lib/types/media-types.ts';

type TestDataTransfer = DataTransfer & { clearCount: number };
interface ClipboardOptions {
  files?: Array<File | { name: string; type: string }>;
  items?: Array<Partial<DataTransferItem>>;
  data?: Record<string, string>;
}
interface MutableClipboard {
  files: Array<File | { name: string; type: string }>;
  items: Array<Partial<DataTransferItem>>;
  clearCount: number;
  clearData(): void;
...
type MoveStateValue = Parameters<Parameters<typeof S.moveState.subscribe>[0]>[0];

function renderShapeToCells(shapeValue: object): EditorCellMap {
  return renderEditorShapeToCells(shapeValue as EditorShape);
}

function shapeGlyphs(shapeValue: object): ReturnType<typeof editorShapeGlyphs> {
  return editorShapeGlyphs(shapeValue as EditorShape);
}

function pasteClipsFromClipboard(clipboard: DataTransfer): ClipboardPasteResult {
  return pasteEditorClipsFromClipboard(clipboard) as ClipboardPasteResult;
}

function clipboardImageFile(clipboard: object): File | null {
  return editorClipboardImageFile(clipboard as DataTransfer);
}

function clipboardPasteIntent(
  clipboard: object,
  context: string,
): ReturnType<typeof editorClipboardPasteIntent> {
  return editorClipboardPasteIntent(clipboard as DataTransfer, context);
}

function clipboardMediaPlacementSucceeded(result: object | null): boolean {
  return editorClipboardMediaPlacementSucceeded(result);
}

function loadTimeline(state: unknown): void {
  F.loadCanonicalTimeline(state as Parameters<typeof F.loadCanonicalTimeline>[0]);
}

async function importMediaFile(
  file: Blob,
  kind: Parameters<typeof importEditorMediaFile>[1],
  options: Parameters<typeof importEditorMediaFile>[2],
): Promise<NonNullable<Awaited<ReturnType<typeof importEditorMediaFile>>> & { placement: string }> {
  return importEditorMediaFile(file as File, kind, options) as Promise<NonNullable<
    Awaited<ReturnType<typeof importEditorMediaFile>>
  > & { placement: string }>;
}

function captureLayerClipClipboard(): ClipClipboardPayload & {
  projectRevision: number;
  fps: number;
} {
  return F.captureLayerClipClipboard() as ClipClipboardPayload & {
    projectRevision: number;
    fps: number;
  };
}
...
function reset(layerDefs: unknown[]): string {
  G.setLayers(layerDefs as Parameters<typeof G.setLayers>[0]);
...
function layerById(id: string | null): EditorLayer {
  return get(G.layers).find((layer) => layer.id === id)!;
}
function layerAt(index: number): EditorLayer {
  return get(G.layers)[index]!;
}
function currentMoveState(): NonNullable<MoveStateValue> {
  return get(S.moveState)!;
}
function keys(layer: EditorLayer): string[] {
  return Object.keys(layer.cells).sort();
}
function maskState(layer: EditorLayer): Array<[string, number | undefined]> {
  const effect = layer as EditorEffectLayer;
  return Object.entries(effect.mask!.cells).sort(([a], [b]) => a.localeCompare(b))
...
function nativeClipboard({ files = [], items = [], data = {} }: ClipboardOptions = {}): TestDataTransfer {
  const values = new Map(Object.entries(data));
  const clipboard: MutableClipboard = {
    files: [...files],
    items: [...items],
    clearCount: 0,
    clearData() {
    ...
    setData(type: string, value: string) { values.set(type, String(value)); },
    getData(type: string) { return values.get(type) || ''; },
  };
  return clipboard as unknown as TestDataTransfer;
}
...
eq('shape-rejects-cell-move-and-clears-stale-selection', [
  S.beginMove(),
  activeLayer().type,
  (activeLayer() as EditorShapeLayer).shape?.kind,
  selectionKeys(),
], [false, 'shape', 'line', []]);
...
let source = layerById(id);
let created = layerById(newId as string);
...
source = layerById(id);
created = layerById(newId as string);
...
let copyGroup = layerAt(0);
let groupedSource = layerAt(1);
...
eq('whole-mask-translation-preserves-mask-metadata-and-effect-position', {
  ...
  defaultStrength: (activeLayer() as EditorEffectLayer).mask!.defaultStrength,
  opacity: (activeLayer() as EditorEffectLayer).mask!.opacity,
  offset: activeLayer().offset,
}, {
...
eq('moving-selection-keeps-mask-transform-independent', {
  cells: maskState(activeLayer()),
  maskOffset: (activeLayer() as EditorEffectLayer).mask!.offset,
...
const outwardTransformCases = new Map<string, [EditorPoint, EditorBounds]>([
...
const inwardTransformCases = new Map<string, [EditorPoint, EditorBounds]>([
...
const oneCellOutwardDeltas = new Map<string, EditorPoint>([
...
for (const handle of S.TRANSFORM_HANDLES) {
  ...
  const startBounds = currentMoveState().bounds;
  ...
  eq(`one-cell-${handle}-control-produces-a-visible-scaled-preview`, {
    bounds: currentMoveState().bounds,
    previewCount: currentMoveState().preview.length,
...
S.beginTransformSelection();
const noOpBounds = currentMoveState().bounds;
S.updateTransformBounds(S.transformBoundsFromDrag(
  noOpBounds,
  'body',
  dragStart,
...
let transformGroup = layerAt(0);
let transformChild = layerAt(1);
...
eq('grouped-transform-preserves-layer-and-parent-identity', {
  ...
  groupOffset: layerById(transformGroup.id).offset,
...
eq('effect-mask-transform-nearest-neighbor-scales-strengths', [
  ...
  ] as Array<[string, number]>).sort(([a], [b]) => a.localeCompare(b)),
  8,
]);
S.cancelMove();
...
F.seekTick(1);
const layerPayload = captureLayerClipClipboard();
...
eq('layer-copy-pastes-only-that-clip-on-one-new-named-track', {
  ...
  glyph: (pastedTimeline.clips.find((clip) => clip.id === clipboardPaste.clipIds[0])!
    .frameKeys[0]!.value as { cells: EditorCellMap }).cells[cmKey(0, 0)]!.c,
...
const timelineTrackA = clipboardState.tracks[0]!;
const timelineTrackB = clipboardState.tracks[1]!;
const timelineClipA = {
...
const sourceShapeFrame = sourceShapeClip.frameKeys[0]!.value as { shape: unknown };
const shapePropertyTracks = {
  position: [{ tick: 0, value: { x: 3, y: 4 }, interpolation: 'linear' }],
  shapePath: [{
    tick: 0,
    value: { path: sourceShapeFrame.shape },
    interpolation: 'hold',
  }],
};
loadTimeline({
...
G.resetEditorStateForProjectLoad();
const clipboardOriginalShape = layerAt(0) as EditorShapeLayer;
systemClipboard = nativeClipboard();
copyClipsForContext('layers', systemClipboard);
clipboardPaste = pasteClipsFromClipboard(systemClipboard);
const clipboardShape = layerById(clipboardPaste.layerIds[0]!) as EditorShapeLayer;
...
let groupA = layerAt(0);
let memberA = layerAt(1);
let groupB = layerAt(2);
let memberB = layerAt(3);
...
let existingGroup = layerAt(0);
let removedSource = layerAt(1);
...
loadTimeline({ fps: get(F.fps), tracks: [], clips: [], tags: [] });
loadMediaRegistry({
  generation: 1,
...
const imagePayload = captureLayerClipClipboard();
...
copyClipsForContext('layers', systemClipboard);
clipboardPaste = pasteClipsFromClipboard(systemClipboard);
const clipboardPastedImage = layerById(clipboardPaste.layerIds[0]!) as EditorImageLayer;
...
const videoPayload = captureLayerClipClipboard();
...
const importedAssetId = (get(G.layers).find(
  (layer) => layer.id === importedImage.placement,
)! as EditorImageLayer).assetId;
G.undo();
...
const stalePayload = captureLayerClipClipboard();
stalePayload.projectRevision++;
const beforeStalePaste = structuredClone(getClipTimelineState());
const stalePaste = F.pasteClipClipboard(stalePayload) as ClipboardPasteResult;
...
const staleFpsPayload = captureLayerClipClipboard();
staleFpsPayload.fps += 1;
const beforeStaleFpsPaste = structuredClone(getClipTimelineState());
const staleFpsPaste = F.pasteClipClipboard(staleFpsPayload) as ClipboardPasteResult;
