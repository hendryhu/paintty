...
import { activeTool, paintColor, recentColors } from '../src/lib/stores.ts';
...
function shapeDefinition(color = '#112233'): Parameters<typeof setLayers>[0][number] {
  return {
    name: 'Shape',
    type: 'shape',
    visible: true,
...
function textDefinition(): Parameters<typeof setLayers>[0][number] {
  return {
    name: 'Text',
    type: 'text',
    visible: true,
...
function firstShape(): EditorShapeLayer {
  return get(layers)[0] as EditorShapeLayer;
}

function firstText(): EditorTextLayer {
  return get(layers)[0] as EditorTextLayer;
}

function firstEffect(): EditorEffectLayer {
  return get(layers)[0] as EditorEffectLayer;
}
...
setLayers([{
  ...
  type: 'effect',
  visible: true,
  effect: { kind: 'solid-color', color: '#112233', intensity: 1 },
  cells: {},
}]);
const effectId = firstEffect().id;
assert.equal(colorEditSession.open({ kind: 'effect', layerId: effectId }), true);
colorEditSession.preview('#aabbcc');
colorEditSession.commit();
assert.equal(firstEffect().effect?.kind, 'solid-color');
assert.equal((firstEffect().effect as Extract<NonNullable<EditorEffectLayer['effect']>, { kind: 'solid-color' }>).color, '#aabbcc');
undo();
assert.equal((firstEffect().effect as Extract<NonNullable<EditorEffectLayer['effect']>, { kind: 'solid-color' }>).color, '#112233',
  'Solid Color picker previews commit as one undo gesture');
colorEditSession.close();
...
colorEditSession.preview('#aabbcc');
colorEditSession.commit();
assert.equal(firstShape().shape!.fg, '#aabbcc');
undo();
assert.equal(firstShape().shape!.fg, '#112233', 'all shape previews commit as one undo gesture');
assert.equal(get(canUndo), false, 'shape preview does not leak extra history entries');
colorEditSession.close();

setLayers([shapeDefinition()]);
const cancelledShapeId = firstShape().id;
colorEditSession.open({ kind: 'shape', layerId: cancelledShapeId });
colorEditSession.preview('#abcdef');
colorEditSession.cancel();
assert.equal(firstShape().shape!.fg, '#112233');
...
undo();
assert.deepEqual(firstText().runs, []);
colorEditSession.close();

setLayers([shapeDefinition()]);
const sampledShapeId = firstShape().id;
...
assert.equal(colorEditSession.sample(null as unknown as string), false);
assert.equal(get(activeTool), 'eyedropper', 'transparent samples keep the session in sampling mode');
assert.equal(firstShape().shape!.fg, '#112233', 'transparent samples do not change the target');
...
assert.equal(firstShape().shape!.fg, '#fedcba');
colorEditSession.cancel();
assert.equal(get(activeTool), 'select');
assert.equal(firstShape().shape!.fg, '#112233', 'cancelling a sample drag restores its start');
...
undo();
assert.equal(firstShape().shape!.fg, '#112233', 'all drag samples coalesce into one undo gesture');
colorEditSession.close();

setLayers([shapeDefinition()]);
const switchedToolShapeId = firstShape().id;
activeTool.set('select');
...
assert.equal(firstShape().shape!.fg, '#112233', 'changing tools cancels an unfinished sample');
assert.equal(get(canUndo), false);

setLayers([shapeDefinition()]);
const abortedId = firstShape().id;
...
assert.equal(firstShape().shape!.fg, '#112233', 'abort cancels the gesture and closes the session');
assert.equal(get(canUndo), false);

setLayers([shapeDefinition()]);
const deletedId = firstShape().id;
activeTool.set('move');
...
undo();
assert.equal(firstShape().shape!.fg, '#112233', 'deletion remains one undoable operation');

setLayers([shapeDefinition()]);
const deletedDuringPreviewId = firstShape().id;
...
assert.equal(colorEditSession.open({ kind: 'shape', layerId: -1 as unknown as string }), false);
assert.equal(state().active, false);

setLayers([shapeDefinition('#102030')]);
const replacedId = firstShape().id;
activeTool.set('select');
...
assert.equal(firstShape().shape!.fg, '#fedcba', 'old preview rollback cannot touch the new project');
assert.equal(get(activeTool), 'select');
