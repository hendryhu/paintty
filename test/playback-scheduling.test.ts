...
import { layers } from '../src/lib/grid.ts';
import * as F from '../src/lib/frames.ts';
import { loadAudioState, resetAudioState } from '../src/lib/audio.ts';
import { transactClipTimeline } from '../src/lib/clipTimelineState.ts';
import { videoFramePlan } from '../src/lib/fileio.ts';
import {
  requireValue,
  type EditorCellMap,
  type TimelineTag,
...
function cellsAt(tick: number): EditorCellMap {
  return {
    '0,0': { c: requireValue(glyphs[Math.floor(tick / glyphs.length)]), fg: '#ffffff', bg: null },
    '1,0': { c: requireValue(glyphs[tick % glyphs.length]), fg: '#ffffff', bg: null },
  };
}
...
function loadSequence(ticks = SOURCE_TICKS, tags: TimelineTag[] = []): void {
...
const fakeClearTimeout = (id: unknown): void => {
  const task = pending.find((candidate) => candidate.id === Number(id));
...
function runNextTimer(lateness = 0): void {
  let task: TimerTask | undefined;
...
try {
  ...
  assert.deepEqual(requireValue(requireValue(get(F.frames)[360]).layers[0]).offset, { x: 360, y: 0 });
  ...
  assert.deepEqual(requireValue(get(layers)[0]).offset, { x: 719, y: 0 });
  assert.equal(requireValue(get(layers)[0]).blink, true);
  ...
  resetClock();
  F.looping.set(false);
  let stoppedView: { tick: number; signature: string } | null = null;
  ...
  transactClipTimeline('shrink-during-playback', (state) => {
    ...
    clip.propertyTracks['position'] = requireValue(clip.propertyTracks['position'])
      .filter((key) => key.tick < 2);
  ...
  transactClipTimeline('extend-during-playback', (state) => {
    ...
    requireValue(clip.propertyTracks['position']).push({
      tick: 5,
...
for (let seed = 1; seed <= 64; seed++) {
  ...
  plan.forEach((sample, index) => {
    assert.deepEqual({ sourceIndex: sample.sourceIndex, tick: sample.tick }, requireValue(expected[index]));
