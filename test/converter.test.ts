...
class FakeCanvas {
  ...
  constructor(width = 0, height = 0, pixels: ArrayLike<number> | null = null) {
    this._width = width;
    this._height = height;
  ...
  set width(value: number) { this.resize(value, this._height); }
  get height() { return this._height; }
  set height(value: number) { this.resize(this._width, value); }
...
class FakeContext {
  ...
  fillText(character: string): void {
...
let passed = 0;
let failed = 0;
const asyncTests: Array<{ name: string; run: () => Promise<void> }> = [];
...
function fixture(width: number, height: number, rgba: ArrayLike<number>): CanvasImageSource {
  assert.equal(rgba.length, width * height * 4);
  return new FakeCanvas(width, height, rgba) as unknown as CanvasImageSource;
}
...
test('opaque auto conversion with sampled backgrounds never leaves transparent holes', () => {
  const opaque = fixture(2, 2, [
    255, 0, 0, 255, 0, 255, 0, 255,
    0, 0, 255, 255, 255, 255, 255, 255,
  ]);
  const result = convertImage(opaque, 'auto', {
    cols: 2,
    rows: 2,
    sampleW: 1,
    sampleH: 1,
  ...
  assert.deepEqual(Object.keys(result.background).sort(), ['0,0', '0,1', '1,0', '1,1']);
  assert.equal(result.meta.transparentRatio, 0);
});
...
testAsync('cooperative conversion is invariant to batch boundaries', async () => {
  ...
  const options: Parameters<typeof convertImageAsync>[2] = {
    cols: 2,
    rows: 1,
    sampleW: 2,
    sampleH: 2,
...
testAsync('cooperative conversion can abort before pixel analysis', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => hasErrorName(error, 'AbortError'),
  );
