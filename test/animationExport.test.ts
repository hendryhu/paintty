...
} from '../src/lib/animationExport.ts';
import { encodePcmWav } from '../src/lib/audioExport.ts';
...
async function bytesOf(value: Blob | Uint8Array): Promise<Uint8Array> {
  return value instanceof Blob ? new Uint8Array(await value.arrayBuffer()) : value;
}

function pcm(channels: number[][]): Parameters<typeof encodePcmWav>[0] {
  const values = channels.map((channel) => Float32Array.from(channel));
  return {
    sampleRate: 48_000,
    numberOfChannels: values.length,
    numberOfFrames: requireValue(values[0]).length,
    getChannelData(channel: number) { return requireValue(values[channel]); },
  };
}

function centralDirectoryMethods(bytes: Uint8Array): number[] {
...
sparseResources = accountAnimationVisualFrame(sparseResources, {
  layers: [{ layerId: 0, cells: [{
    x: 0, y: 0, glyph: 'A', foreground: '#fff', background: null, width: 1,
  }] }],
  composite: [{ x: 0, y: 0, glyph: 'A', foreground: '#fff', background: null, width: 1 }],
});
...
const denseCells = new Array(256 * 256).fill({
  x: 0, y: 0, glyph: 'X', foreground: null, background: null, width: 1,
});
...
const byteBoundary = accountAnimationVisualFrame(byteBoundarySeed, {
  layers: [],
  composite: [{ x: 0, y: 0, glyph: null, foreground: null, background: null, width: 1 }],
});
...
assert.deepEqual(requireValue(visualDocument.frames[0]).layers, [{
  layerId: 0,
  cells: [{
    x: 0,
    y: 0,
...
const visualFrame = requireValue(visualDocument.frames[0]);
assert.deepEqual(visualFrame.composite, requireValue(visualFrame.layers[0]).cells);
...
const adjustmentDocument = buildAnimationDocument({
  dimensions: { w: 1, h: 1 },
  fps: 10,
  layerMetadata: [
    { id: 'clip', name: 'Colour Clip', type: 'effect', effect: { kind: 'color-clip', intensity: 1 } },
    { id: 'clip-shape', name: 'Colour Clip Shape', type: 'shape', shape: { channel: 'color-clip' } },
    { id: 'ink', name: 'Ink', type: 'cell' },
  ],
  frames: [{
    hold: 1,
    layers: [
      { id: 'clip', type: 'effect', cells: [[{ bg: '#ff0000' }]] },
      { id: 'clip-shape', type: 'shape', shape: { channel: 'color-clip' }, cells: [[{ bg: '#00ff00' }]] },
      { id: 'ink', cells: [[{ c: 'A', fg: '#ffffff' }]] },
    ],
  }],
  compositeCells: [[[{ c: 'A', fg: '#ff0000' }]]],
});
assert.deepEqual(adjustmentDocument.layers, [{ id: 0, name: 'Ink', order: 0 }],
  'non-emitting adjustment layers stay out of runtime layer metadata');
assert.deepEqual(requireValue(adjustmentDocument.frames[0]).layers, [{
  layerId: 0,
  cells: [{ x: 0, y: 0, glyph: 'A', foreground: '#ffffff', background: null, width: 1 }],
}], 'non-emitting adjustment source cells never become runtime backgrounds');
assert.deepEqual(requireValue(adjustmentDocument.frames[0]).composite, [{
  x: 0, y: 0, glyph: 'A', foreground: '#ff0000', background: null, width: 1,
}], 'the resolved composite retains the applied adjustment color');
...
assert.ok(archiveBlob instanceof Blob);
...
assert.deepEqual(requireValue(archiveEntries['audio.wav']), mixedWav);
assert.deepEqual(JSON.parse(strFromU8(requireValue(archiveEntries['hero.json']))), mixedDocument);
...
function runtimeGolden(ids: typeof authorIdsA) {
...
assert.ok(firstZip instanceof Uint8Array);
assert.ok(repeatedZip instanceof Uint8Array);
assert.ok(reidentifiedZip instanceof Uint8Array);
...
assert.equal(strFromU8(requireValue(goldenEntries['runtime-golden.json'])), firstGolden.json);
assert.deepEqual(requireValue(goldenEntries['audio.wav']), mixedWav);
