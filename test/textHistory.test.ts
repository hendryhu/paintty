...
import { renderTextToCells } from '../src/lib/textLayer.ts';
import type {
  EditorBounds,
  EditorTextLayer,
  EditorTextRun,
...
function textDefinition(
  text: string,
  wrap = true,
): Parameters<typeof setLayers>[0][number] {
...
function activeText(): EditorTextLayer {
  return get(layers).find((layer): layer is EditorTextLayer => layer.type === 'text')!;
}
...
updateTextLayer(createdId as string, { text: 'A' }, renderNarrowText);
updateTextLayer(createdId as string, { text: 'AB' }, renderNarrowText);
endStroke();
undo();
