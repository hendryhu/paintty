...
} from '../src/lib/timelineUiState.ts';
import type { StorageLike } from '../src/lib/types/project-types.ts';
import {
  requireValue,
  type TimelineTag,
...
assert.equal(planTimelineTagMove({ id: 'start', tick: 2, type: 'loop-start' }, -999, 14, 8).tick, 0);
assert.equal(planTimelineTagMove({ id: 'end', tick: 2, type: 'loop-end' }, 999, 14, 8).tick, 7);
assert.equal(planTimelineTagMove({
  id: 'click', tick: 2, type: 'custom', value: 'click',
}, 2.99, 14, 8).moved, false,
...
assert.equal(requireValue(stackedTagLayouts[0]).top, 8,
  'the first tag body begins below the playhead head hit zone');
for (let index = 1; index < stackedTagLayouts.length; index++) {
  const layout = requireValue(stackedTagLayouts[index]);
  const previous = requireValue(stackedTagLayouts[index - 1]);
  assert.ok(layout.top >= previous.top + previous.height,
...
assert.ok(requireValue(adjacentMinimumZoom[1]).left >=
  requireValue(adjacentMinimumZoom[0]).left + requireValue(adjacentMinimumZoom[0]).width,
  'neighboring tick markers must not overlap at minimum zoom');

const clusteredTags: TimelineTag[] = [
  { id: 'loop-start', tick: 2, type: 'loop-start' },
  ...Array.from({ length: 100 }, (_, index): TimelineTag => ({
    id: `custom-${index}`,
    tick: 2,
    type: 'custom',
    value: `event-${index}`,
...
const clusteredMarkers = buildTimelineTagMarkers(clusteredTags, { startTick: 0, endTick: 8 });
...
assert.equal(requireValue(clusteredMarkers[1]).cluster, true);
assert.equal(requireValue(clusteredMarkers[1]).customCount, 100);
assert.equal(requireValue(requireValue(clusteredMarkers[1]).customValues).length, 100);
const onlyMarkers = buildTimelineTagMarkers([
  { id: 'only', tick: 1, type: 'custom', value: 'only' },
], { startTick: 0, endTick: 2 });
assert.equal(requireValue(onlyMarkers[0]).id, 'only',
  'one custom tag keeps its exact marker');
...
assert.match(clipTimelineSource,
  /moveClips\(plan\.operations, \{ guard: commitGuard \}\)/);
assert.match(clipTimelineSource,
  /trimClips\(plan\.operations, \{ guard: commitGuard \}\)/);
assert.match(clipTimelineSource,
...
assert.match(clipTimelineSource, /setZoom\(Number\(planned\.zoom\), event\.clientX, \{ source: 'wheel' \}\)/,
...
assert.match(clipTimelineSource,
  /layoutTimelineKeyMarkers\(markers, propertyMarkers, \{[\s\S]*pixelsPerTick: zoom,[\s\S]*rowHeight/,
...
assert.match(timelineHeaderSource, /class="zoom-control"[^>]*title="Zoom"/);
assert.doesNotMatch(timelineHeaderSource, /pixels per tick|Zoom out \(-\)|Zoom in \(\+ or =\)/,
  'Timeline zoom hover copy stays terse');
