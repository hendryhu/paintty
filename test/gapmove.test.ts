import {
  canonicalLayerDropGap as canonicalEditorLayerDropGap,
  computeGapMove as computeEditorGapMove,
  computeSelectedGapMove as computeEditorSelectedGapMove,
  normalizeGroups as normalizeEditorGroups,
} from '../src/lib/grid.ts';
import type { EditorLayer, EditorPoint } from '../src/lib/types/editor-domain.ts';
...
function computeGapMove(
  items: GapLayer[],
  movingId: number,
  beforeId: number | null,
  intoGroup: boolean,
): GapLayer[] {
  return computeEditorGapMove(
    items as unknown as EditorLayer[],
    movingId as unknown as string,
    beforeId as unknown as string | null,
    intoGroup,
  ) as unknown as GapLayer[];
}

function computeSelectedGapMove(
  items: GapLayer[],
  selectedIds: Set<number>,
  beforeId: number | null,
  intoGroup: boolean,
): GapLayer[] {
  return computeEditorSelectedGapMove(
    items as unknown as EditorLayer[],
    selectedIds as unknown as Set<string>,
    beforeId as unknown as string | null,
    intoGroup,
  ) as unknown as GapLayer[];
}

function normalizeGroups(items: GapLayer[]): GapLayer[] {
  return normalizeEditorGroups(items as unknown as EditorLayer[]) as unknown as GapLayer[];
}

function canonicalLayerDropGap(
  items: GapLayer[],
  selectedIds: Set<number>,
  movingId: number,
  beforeId: number | null,
  intoGroup: boolean,
): { beforeId: number | null; intoGroup: boolean } {
  return canonicalEditorLayerDropGap(
    items as unknown as EditorLayer[],
    selectedIds as unknown as Set<string>,
    movingId as unknown as string,
    beforeId as unknown as string | null,
    intoGroup,
  ) as unknown as { beforeId: number | null; intoGroup: boolean };
}
...
const ids = (arr: GapLayer[]) => arr.map((layer) =>
  layer.groupId ? `${layer.name}<${layer.groupId}` : layer.name);

function scene(): GapLayer[] {
  return [
