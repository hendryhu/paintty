...
import {
  cloneShapePathKey as cloneEditorShapePathKey,
  editShapePathField as editEditorShapePathField,
  enumerateShapePathComponents,
  interpolateShapePath as interpolateEditorShapePath,
  interpolateShapePathComponent,
  normalizeShapePathKey as normalizeEditorShapePathKey,
...
import {
  canConvertShapeDetailToCell as canConvertEditorShapeDetailToCell,
  editPolygonSides as editEditorPolygonSides,
  editShapePathAggregate as editEditorShapePathAggregate,
  shapeForAnchorComponentEdit as shapeForEditorAnchorComponentEdit,
  shapeForStaticPathEdit as shapeForEditorStaticPathEdit,
  shapePathAggregateMetrics as editorShapePathAggregateMetrics,
} from '../src/lib/shapePathEditing.ts';
import {
...
function cloneShapePathKey(value: unknown): TestShapePath {
  return cloneEditorShapePathKey(value) as unknown as TestShapePath;
}

function editShapePathField(
  value: unknown,
  field: Parameters<typeof editEditorShapePathField>[1],
  next: unknown,
  baseline?: unknown,
): TestShapePath {
  return editEditorShapePathField(value, field, next, baseline) as unknown as TestShapePath;
}

function interpolateShapePath(from: unknown, to: unknown, progress: unknown): TestShapePath {
  return interpolateEditorShapePath(from, to, progress) as unknown as TestShapePath;
}

function normalizeShapePathKey(
  value: unknown,
  expectedKind?: Parameters<typeof normalizeEditorShapePathKey>[1],
): TestShapePath {
  return normalizeEditorShapePathKey(value, expectedKind) as unknown as TestShapePath;
}

function pathValueFromShape(shape: ShapeFixture): TestShapePath {
  return pathValueFromEditorShape(shape as EditorShape) as unknown as TestShapePath;
}

function shapePathComponentValue(value: unknown, componentId: 'rotation'): number;
function shapePathComponentValue(value: unknown, componentId: string): EditorPoint;
function shapePathComponentValue(value: unknown, componentId: string): number | EditorPoint {
  return editorShapePathComponentValue(value, componentId) as number | EditorPoint;
}

function shapeWithPathValue(shape: ShapeFixture, value: unknown): TestShape {
  return editorShapeWithPathValue(shape as EditorShape, value) as TestShape;
}

function translateShapePathKey(value: unknown, dx: unknown, dy: unknown): TestShapePath {
  return translateEditorShapePathKey(value, dx, dy) as unknown as TestShapePath;
}
...
function withShapePathComponentValue(
  ...
  return withEditorShapePathComponentValue(
    value,
    componentId,
    componentValue,
  ) as unknown as TestShapePath;
}

function editPolygonSides(shape: ShapeFixture, sides: unknown, baseline?: ShapeFixture): TestShape {
  return editEditorPolygonSides(
    shape as EditorShape,
    sides,
    baseline as EditorShape | undefined,
  ) as TestShape;
}

function editShapePathAggregate(
  value: unknown,
  field: Parameters<typeof editEditorShapePathAggregate>[1],
  next: unknown,
): TestShapePath {
  return editEditorShapePathAggregate(value, field, next) as unknown as TestShapePath;
}
...
function shapeForAnchorComponentEdit(
  ...
  return shapeForEditorAnchorComponentEdit(shape as EditorShape, path, anchor) as TestShape;
}

function shapeForStaticPathEdit(shape: ShapeFixture, path: unknown): TestShape {
  return shapeForEditorStaticPathEdit(shape as EditorShape, path) as TestShape;
}

function shapePathAggregateMetrics(value: unknown): { cx: number; cy: number; w: number; h: number } {
  return editorShapePathAggregateMetrics(value)!;
}

function canConvertShapeDetailToCell(shape: ShapeFixture, geometryAnimated = false): boolean {
  return canConvertEditorShapeDetailToCell(shape as EditorShape, geometryAnimated);
}

function renderShapeToCells(shape: ShapeFixture): EditorCellMap {
  return renderEditorShapeToCells(shape as EditorShape);
}

function resolvedShapeVertices(shape: ShapeFixture): EditorPoint[] {
  return resolvedEditorShapeVertices(shape as EditorShape);
}

function geometryBounds(shape: { x0: number; y0: number; x1: number; y1: number }): number[] {
  return [
...
function renderedBounds(shape: ShapeFixture): number[] {
...
assert.deepEqual(normalizeShapePathKey({
  kind: 'rect', cx: 2, cy: 3, w: 5, h: 3, rotation: 45, rotationAspect: 0.5,
}), {
  kind: 'rect', cx: 2, cy: 3, w: 5, h: 3, rotation: 45, rotationAspect: 0.5,
}, 'rotation aspect survives path normalization');
assert.equal(normalizeShapePathKey({
  kind: 'rect', cx: 2, cy: 3, w: 5, h: 3, rotationAspect: 0,
}), null, 'rotation aspect must remain positive');
assert.equal(normalizeShapePathKey({
  kind: 'rect', cx: 2, cy: 3, w: 5, h: 3, rotation: 360,
}).rotation, 0, 'one full turn normalizes to zero');
assert.equal(normalizeShapePathKey({
  kind: 'rect', cx: 2, cy: 3, w: 5, h: 3, rotation: -1,
}).rotation, 359, 'negative rotation wraps into the displayed range');
assert.equal(normalizeShapePathKey({
  kind: 'rect', cx: 2, cy: 3, w: 5, h: 3, rotation: 721,
}).rotation, 1, 'multiple turns normalize into one revolution');
