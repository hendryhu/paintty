...
} from '../src/lib/gestureOwnership.ts';
...
const owner = captureGestureOwner(
  source as unknown as NonNullable<Parameters<typeof captureGestureOwner>[0]>,
  41,
);
...
const exact = { ...owner } as Omit<typeof owner, 'pointerId'> & {
  pointerId?: number | null;
};
delete exact.pointerId;
assert.equal(gestureOwnerMatches(owner, exact), true);

const replacements: Array<[keyof typeof exact, string | number]> = [
  ['layerId', 18],
  ['frameIndex', 24],
  ['tool', 'eraser'],
  ['layerPart', 'layer'],
...
for (const [field, replacement] of replacements) {
  assert.equal(
    gestureOwnerMatches(
      owner,
      { ...exact, [field]: replacement } as Parameters<typeof gestureOwnerMatches>[1],
    ),
    false,
