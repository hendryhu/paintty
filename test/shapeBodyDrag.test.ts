...
import {
  applyShapeBodyDrag as applyEditorShapeBodyDrag,
  applyShapeGeometryEdit as applyEditorShapeGeometryEdit,
  blankShapeLayerAcceptsKind as editorBlankShapeLayerAcceptsKind,
  captureShapeBodyDrag as captureEditorShapeBodyDrag,
  shapeDirectEditEnabled,
} from '../src/lib/shapeBodyDrag.ts';
import {
  editShapePathField,
  pathValueFromShape as pathValueFromEditorShape,
  shapePathComponentValue as editorShapePathComponentValue,
  shapeWithPathValue as editorShapeWithPathValue,
  translateShapePathKey as translateEditorShapePathKey,
  withShapePathComponentValue as withEditorShapePathComponentValue,
} from '../src/lib/shapePath.ts';
import { shapeForAnchorComponentEdit as shapeForEditorAnchorComponentEdit } from '../src/lib/shapePathEditing.ts';
import {
  shapeTransformHandles as shapeTransformHandlesForEditor,
  transformShapeFromCageHandle as transformEditorShapeFromCageHandle,
  type ShapeTransformHandle,
} from '../src/lib/shapeTransform.ts';
import {
  regularPolygonVertices,
  renderShapeToCells as renderEditorShapeToCells,
  resolvedShapeVertices as resolvedEditorShapeVertices,
...
type ShapeDrag = NonNullable<ReturnType<typeof captureEditorShapeBodyDrag>>;
interface TestShapePath {
...
function currentLayer(): EditorLayer {
  return get(G.layers)[0]!;
}

function currentShapeLayer(): EditorShapeLayer & { shape: TestShape } {
  return currentLayer() as EditorShapeLayer & { shape: TestShape };
}
...
function renderShapeToCells(shapeValue: ShapeFixture): EditorCellMap {
  return renderEditorShapeToCells(shapeValue as EditorShape);
}

function resolvedShapeVertices(shapeValue: ShapeFixture): EditorPoint[] {
  return resolvedEditorShapeVertices(shapeValue as EditorShape);
}

function pathValueFromShape(shapeValue: ShapeFixture): TestShapePath {
  return pathValueFromEditorShape(shapeValue as EditorShape) as unknown as TestShapePath;
}

function shapeWithPathValue(shapeValue: ShapeFixture, path: unknown): TestShape {
  return editorShapeWithPathValue(shapeValue as EditorShape, path) as TestShape;
}

function translateShapePathKey(path: unknown, dx: unknown, dy: unknown): TestShapePath {
  return translateEditorShapePathKey(path, dx, dy) as unknown as TestShapePath;
}

function withShapePathComponentValue(
  path: unknown,
  componentId: unknown,
  value: unknown,
): TestShapePath {
  return withEditorShapePathComponentValue(path, componentId, value) as unknown as TestShapePath;
}

function shapePathComponentValue(path: unknown, componentId: 'rotation'): number;
function shapePathComponentValue(path: unknown, componentId: string): EditorPoint;
function shapePathComponentValue(path: unknown, componentId: string): number | EditorPoint {
  return editorShapePathComponentValue(path, componentId) as number | EditorPoint;
}

function shapePathAt(layerId: string, frame: number): TestShapePath {
  return F.shapePathAt(layerId, frame) as unknown as TestShapePath;
}

function shapeForAnchorComponentEdit(
  shapeValue: ShapeFixture,
  path: unknown,
  anchor: EditorPoint | null,
): TestShape {
  return shapeForEditorAnchorComponentEdit(shapeValue as EditorShape, path, anchor) as TestShape;
}

function shapeTransformHandles(shapeValue: ShapeFixture): TestShapeHandles {
  return shapeTransformHandlesForEditor(shapeValue as EditorShape) as TestShapeHandles;
}
...
function transformShapeFromCageHandle(
  ...
  return transformEditorShapeFromCageHandle(shapeValue as EditorShape, handleId, target) as TestShape;
}

function blankShapeLayerAcceptsKind(
  layer: object | null,
  pathAnimated: boolean,
  currentKind: EditorShapeKind | null,
  requestedKind: EditorShapeKind,
  activeClip = true,
): boolean {
  return editorBlankShapeLayerAcceptsKind(
    layer as EditorLayer | null,
    pathAnimated,
    currentKind,
    requestedKind,
    activeClip,
...
function captureShapeBodyDrag(layerId: string, frame: number): ShapeDrag {
  return captureEditorShapeBodyDrag(layerId, frame)!;
}

function applyShapeBodyDrag(
  drag: ShapeDrag,
  frame: number,
  dx: number,
  dy: number,
): boolean {
  return applyEditorShapeBodyDrag(drag, frame, dx, dy);
}

function applyShapeGeometryEdit(
  layerId: string,
  frame: number,
  shapeValue: ShapeFixture,
  baseline: ShapeFixture,
): boolean {
  return applyEditorShapeGeometryEdit(
    layerId,
    frame,
    shapeValue as EditorShape,
    baseline as EditorShape,
  );
}
...
function shapeDocument(shapeValue: ShapeFixture = shape): string {
  G.setLayers([{
    name: 'Castle wall',
    type: 'shape',
    visible: true,
    opacity: 1,
    shape: structuredClone(shapeValue as EditorShape),
...
function compensationKeyCount(layerId: string): number {
...
for (const kind of ['line', 'rect', 'circle', 'polygon'] as EditorShapeKind[]) {
...
function gapShape(kind: EditorShapeKind): EditorShape {
  if (kind === 'polygon') {
    return {
      ...shape,
      kind,
  ...
  return { ...shape, kind } as EditorShape;
}
...
for (const kind of ['line', 'rect', 'circle', 'polygon'] as EditorShapeKind[]) {
  ...
  F.seekTick(0);
  const gapOwner = get(G.layers).find((layer) => layer.id === ownerId) as EditorShapeLayer;
...
const staticPolygonBeforeDrag = structuredClone(currentShapeLayer().shape);
assert.equal(F.isShapePathTrackEnabled(staticPolygonId), false,
  'a selected static polygon does not require Path animation');
const staticPolygonHandles = shapeTransformHandles(currentShapeLayer().shape);
...
G.endStroke();
assert.notDeepEqual(currentShapeLayer().shape.vertices, staticPolygon.vertices);
assert.equal(currentShapeLayer().shape.vertices.length, staticPolygon.sides);
assert.equal(currentShapeLayer().shape.vertices.every(({ x, y }) =>
  Number.isFinite(x) && Number.isFinite(y)), true,
'dragging a static polygon vertex retains safe finite geometry and side semantics');
G.undo();
assert.deepEqual(currentShapeLayer().shape, staticPolygonBeforeDrag,
  'one Undo restores the complete static polygon handle drag');
G.redo();
assert.deepEqual(currentShapeLayer().shape.vertices, editedStaticPolygon.vertices);
...
F.gotoFrame(1);
const localCells = structuredClone(currentShapeLayer().cells);
...
assert.deepEqual(currentShapeLayer().offset, { x: 3, y: 0 },
...
assert.deepEqual(currentShapeLayer().shape, shape,
  'dragging a keyed shape body must not rewrite its shared shape geometry');
assert.deepEqual(currentShapeLayer().cells, localCells,
...
F.gotoFrame(0);
assert.deepEqual(currentShapeLayer().shape, shape);
assert.deepEqual(currentShapeLayer().offset, { x: 0, y: 0 });
F.gotoFrame(2);
assert.deepEqual(currentShapeLayer().shape, shape);
assert.deepEqual(currentShapeLayer().offset, { x: 6, y: 0 });
...
G.endStroke();
assert.deepEqual(shapePathAt(pathAnimatedId, 1), {
...
const resizedPathStartShape = { ...currentShapeLayer().shape };
...
const pathBeforePositionDrag = shapePathAt(pathAnimatedId, 1);
G.beginStroke();
assert.equal(applyShapeBodyDrag(positionPriorityDrag, 1, 3, 2), true);
G.endStroke();
assert.deepEqual(shapePathAt(pathAnimatedId, 1), pathBeforePositionDrag,
  'Position animation takes priority over Path when both tracks are enabled');
assert.deepEqual(currentShapeLayer().offset, { x: 3, y: 2 });
...
F.gotoFrame(1);
assert.equal((currentShapeLayer().shape.x0 + currentShapeLayer().shape.x1) / 2, 8.5,
...
G.endStroke();
assert.equal(shapePathAt(evenPathId, 1).cx, 9,
...
const componentPathsBeforeDrag = [0, 1, 2].map((frame) =>
  shapePathAt(componentAnimatedId, frame));
...
const translatedComponentKeys = componentKeysBeforeDrag.map((key) => ({
  ...key,
  value: {
    x: (key.value as EditorPoint).x + 3,
    y: (key.value as EditorPoint).y - 1,
  },
}));
F.gotoFrame(1);
...
assert.deepEqual([0, 1, 2].map((frame) =>
  shapePathAt(componentAnimatedId, frame)), componentPathsBeforeDrag,
...
assert.deepEqual([0, 1, 2].map((frame) =>
  shapePathAt(componentAnimatedId, frame)), translatedComponentPaths,
...
for (const frame of [0, 1, 2]) {
  F.gotoFrame(frame);
  assert.deepEqual(shapePathAt(componentAnimatedId, frame),
...
const recoveredComponentId = activeId();
assert.deepEqual([0, 1, 2].map((frame) =>
  shapePathAt(recoveredComponentId, frame)), translatedComponentPaths,
...
const positionedComponentPaths = [0, 1, 2].map((frame) =>
  shapePathAt(positionedComponentId, frame));
...
G.endStroke();
assert.deepEqual([0, 1, 2].map((frame) =>
  shapePathAt(positionedComponentId, frame)), positionedComponentPaths,
...
F.gotoFrame(1);
const mixedPathBeforeHandle = shapePathAt(mixedPathId, 1);
...
const mixedHandleStart = currentShapeLayer().shape;
...
G.endStroke();
assert.deepEqual(
  shapePathComponentValue(shapePathAt(mixedPathId, 1), 'vertex:1'),
  { x: vertexOne.x + 3, y: vertexOne.y - 1 },
...
G.undo();
assert.deepEqual(shapePathAt(mixedPathId, 1), mixedPathBeforeHandle,
...
const anchorPathBefore = shapePathAt(mixedPathId, 1);
...
G.endStroke();
const anchorPathAfter = shapePathAt(mixedPathId, 1);
...
const visibleBeforeDisable = shapePathAt(mixedPathId, 1);
...
assert.deepEqual(shapePathAt(mixedPathId, 1), visibleBeforeDisable,
...
for (const frame of [0, 2]) {
  assert.deepEqual(
    shapePathComponentValue(shapePathAt(mixedPathId, frame), 'vertex:0'),
...
const endpointPathBefore = shapePathAt(endpointDragId, 0);
...
G.endStroke();
const endpointPathAfter = shapePathAt(endpointDragId, 0);
assert.deepEqual(
...
const implicitPathBefore = shapePathAt(implicitAnchorId, 0);
...
G.endStroke();
const implicitPathAfter = shapePathAt(implicitAnchorId, 0);
...
G.endStroke();
assert.deepEqual(shapePathAt(implicitAnchorId, 0).anchor, explicitAnchor,
...
const editedAnchorPath = withShapePathComponentValue(
  shapePathAt(implicitAnchorId, 0),
  'anchor',
  editedAnchor,
);
G.beginStroke();
assert.equal(applyShapeGeometryEdit(
  ...
  shapeWithPathValue(ordinaryLine, shapePathAt(implicitAnchorId, 0)),
), true);
G.endStroke();
assert.deepEqual(shapePathAt(implicitAnchorId, 0).anchor, editedAnchor,
...
F.gotoFrame(1);
const mixedLineBefore = shapePathAt(mixedLineId, 1);
...
G.endStroke();
const mixedLineAfter = shapePathAt(mixedLineId, 1);
...
G.undo();
assert.deepEqual(shapePathAt(mixedLineId, 1), mixedLineBefore,
...
G.endStroke();
assert.deepEqual(currentShapeLayer().shape, {
...
const translatedMetadataId = shapeDocument(componentShape);
assert.equal(F.setShapePathWholeTrackEnabled(translatedMetadataId, true), true);
assert.deepEqual(F.setShapePathWholeKeyInterpolation(
  translatedMetadataId,
  [0],
  'ease-in',
), [0]);
assert.deepEqual(F.setShapePathWholeKeyTemporalPreset(
  translatedMetadataId,
  [0],
  'ease-in-out',
), [0]);
const translatedMetadataBefore = F.shapePathWholeKeys(translatedMetadataId)[0]!;
G.beginStroke();
assert.equal(F.translateShapeLayerBaseById(translatedMetadataId, 2, -1), true);
G.endStroke();
const translatedMetadataAfter = F.shapePathWholeKeys(translatedMetadataId)[0]!;
assert.equal(translatedMetadataBefore.kind, 'rect');
assert.equal(translatedMetadataAfter.kind, 'rect');
if (translatedMetadataBefore.kind !== 'rect' || translatedMetadataAfter.kind !== 'rect') {
  throw new Error('translated metadata fixture must remain a rectangle');
}
assert.equal(translatedMetadataAfter.cx, translatedMetadataBefore.cx + 2);
assert.equal(translatedMetadataAfter.cy, translatedMetadataBefore.cy - 1);
assert.equal(translatedMetadataAfter.interpolation, translatedMetadataBefore.interpolation);
assert.deepEqual(translatedMetadataAfter.temporalEase, translatedMetadataBefore.temporalEase,
  'whole-Path translation preserves authored easing metadata');
...
F.gotoFrame(10);
const beforeLaterRotation = shapePathAt(compensatedAnchorId, 10);
...
for (let frame = 0; frame <= 10; frame++) {
  const actual = shapePathAt(compensatedAnchorId, frame);
  for (let index = 0; index < 4; index++) {
...
const lineAnchor = shapePathComponentValue(
  shapePathAt(compensatedLineId, 4),
  'anchor',
);
assert.deepEqual(F.setShapePathComponentValues(4, compensatedLineId, [{
  componentId: 'anchor',
...
const compensatedLinePaths = Array.from({ length: 5 }, (_, frame) =>
  shapePathAt(compensatedLineId, frame));
const assertSameLinePose = (
  actual: TestShapePath,
  expected: TestShapePath,
  message: string,
): void => {
  assert.deepEqual(
...
for (let frame = 0; frame < 5; frame++) {
  assertSameLinePose(shapePathAt(compensatedLineId, frame),
...
for (let frame = 0; frame < 5; frame++) {
  const actual = shapePathAt(compensatedLineId, frame);
