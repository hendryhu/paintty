...
import { readThemeColor, readThemeColors } from '../src/lib/themeColors.ts';
  ...
  for (const section of styleSections(file)) {
    for (const [index, line] of section.css.split('\n').entries()) {
      for (const match of line.matchAll(literalPattern)) {
        ...
        if (!approvals.has(key) || count > requireValue(approvals.get(key))) {
...
const computedStyle: CSSStyleDeclaration = Object.create(null);
computedStyle.getPropertyValue = (property: string): string =>
  property === '--waveform' ? '  rgba(1, 2, 3, 0.5)  ' : ' #abcdef ';
globalThis.getComputedStyle = () => computedStyle;
const themeTarget: Element = Object.create(null);
assert.equal(readThemeColor('--waveform', themeTarget), 'rgba(1, 2, 3, 0.5)');
assert.deepEqual(readThemeColors({ waveform: '--waveform', onion: '--onion-next' }, themeTarget), {
