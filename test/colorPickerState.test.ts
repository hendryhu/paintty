...
import { hexToOklch, oklchToHex } from '../src/lib/color.ts';
import {
  pickerStateFromHex,
  pickerStateFromOklch,
  pickerStateWithFixedChroma,
  setPickerChroma,
  setPickerHue,
  setPickerLightness,
} from '../src/lib/colorPickerState.ts';

function near(label: string, actual: number, expected: number, tolerance = 1e-6) {
  assert.ok(
...
function assertRgbNear(label: string, actual: string, expected: string, tolerance = 1) {
...
const knownVectors: Array<[string, number, number, number]> = [
...
function referenceLinearRgb(L: number, C: number, H: number): number[] {
...
function referenceMaxChroma(L: number, H: number, tolerance = 0): number {
  let lower = 0;
  let upper = 0.4;
...
for (const [L, H] of [[0.35, 30], [0.6, 140], [0.75, 260]] as Array<[number, number]>) {
...
near('chroma setter retains its requested picker value', saturated.C, 0.4, 1e-6);

const fixedChroma = pickerStateWithFixedChroma(0.25, 0.4, 140);
near('fixed chroma state preserves C through an out-of-gamut color', fixedChroma.C, 0.4, 1e-6);
near('fixed chroma state retains requested lightness', fixedChroma.L, 0.25, 1e-6);
near('fixed chroma state retains requested hue', fixedChroma.H, 140, 1e-6);
