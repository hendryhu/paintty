...
import { addLayer, layers, toggleGroupCollapsed } from '../src/lib/grid.ts';
import {
...
import type { MutableMediaRegistry, ProjectAssetRecord } from '../src/lib/types/media-types.ts';
import type {
  ProjectLayer,
  TimelineClip,
  TimelineTag,
...
const cell = (c: string, fg: string | null = '#ffffff', bg: string | null = null) => ({ c, fg, bg });
...
function layer(
  ...
  name: string,
  type: string,
  extra: Partial<ProjectLayer> & UnknownRecord = {},
): ProjectLayer {
  return {
...
function visualClip(
  ...
  extra: Partial<TimelineClip> & UnknownRecord = {},
): TimelineClip {
  return {
...
function fixtureAudioBuffer(): AudioBuffer {
  ...
  return Object.assign(Object.create(null) as AudioBuffer, {
    ...
    copyToChannel(): void {},
    getChannelData(channel: number) { return channel === 0 ? left : right; },
  });
}

function richProject(): RichProject {
  return {
...
const contentMaskProject = structuredClone(source) as RichProject;
const contentMaskTrack = requireValue(contentMaskProject.timeline.tracks.find((track) => track.kind === 'visual')) as Record<string, unknown>;
const contentMaskLayer = requireValue(contentMaskTrack['layer']) as Record<string, unknown>;
contentMaskLayer['contentMask'] = { defaultStrength: 0, cells: {}, offset: { x: 0, y: 0 } };
const contentMaskClip = requireValue(contentMaskProject.timeline.clips.find(
  (clip) => clip.trackId === contentMaskTrack['id'],
)) as Record<string, unknown>;
const contentMaskKeys = contentMaskClip['frameKeys'] as Array<Record<string, unknown>>;
requireValue(contentMaskKeys[0])['value'] = {
  ...(requireValue(contentMaskKeys[0])['value'] as Record<string, unknown>),
  contentMask: {
    defaultStrength: 0,
    cells: { '1,0': { bg: '#ffffff' } },
    offset: { x: 0, y: 0 },
...
loadJSON(JSON.stringify(contentMaskProject));
const serializedContentMaskProject = JSON.parse(serializeJSON()) as RichProject;
const serializedContentMaskTrack = requireValue(serializedContentMaskProject.timeline.tracks.find(
  (track) => track.id === contentMaskTrack['id'],
)) as Record<string, unknown>;
const serializedContentMaskClip = requireValue(serializedContentMaskProject.timeline.clips.find(
  (clip) => clip.trackId === contentMaskTrack['id'],
)) as Record<string, unknown>;
assert.deepEqual((serializedContentMaskTrack['layer'] as Record<string, unknown>)['contentMask'],
  { defaultStrength: 0, cells: {}, offset: { x: 0, y: 0 } },
  'content mask metadata survives canonical track serialization');
assert.deepEqual(((serializedContentMaskClip['frameKeys'] as Array<Record<string, unknown>>)[0]!['value'] as Record<string, unknown>)['contentMask'],
  { defaultStrength: 0, cells: { '1,0': { bg: '#ffffff' } }, offset: { x: 0, y: 0 } },
  'content mask cells survive canonical frame serialization');
loadJSON(JSON.stringify(source));
for (const [input, expected] of [[2, 1], [-2, 0]] as Array<[number, number]>) {
  const volumeProject = structuredClone(source);
  const track = requireValue(volumeProject.timeline.tracks.find((entry) => entry.kind === 'audio'));
  track.volume = input;
  track.muted = true;
  loadJSON(JSON.stringify(volumeProject));
  const loadedProject = JSON.parse(serializeJSON()) as RichProject;
  const loadedTrack = requireValue(loadedProject.timeline.tracks
...
seekTick(1);
const shapeLayer = requireValue(get(layers).find((layer) => layer.type === 'shape')) as EditorShapeLayer;
const resolvedShape = requireValue(shapeLayer.shape);
assert.deepEqual({ x0: resolvedShape.x0, x1: resolvedShape.x1 }, { x0: 1, x1: 3 });
assert.equal(requireValue(shapeLayer.cells['1,1']).c, '#');
const effectLayer = requireValue(get(layers).find((layer) => layer.type === 'effect')) as EditorEffectLayer;
assert.equal(requireValue(effectLayer.effect).intensity, 0.5);
assert.equal(requireValue(effectLayer.mask).opacity, 0.5);
assert.deepEqual(requireValue(get(layers).find((layer) => layer.type === 'group')).offset,
  { x: 1, y: 0 });
assert.equal(requireValue(requireValue(get(layers).find(
  (layer) => layer.type === 'text',
)).cells['1,2']).fg, '#ff00ff');
const preview = JSON.parse(serializeLivePreview()) as {
  format: string;
  version: number;
  ticks: Array<{ layers: Array<{ type: string; cells: Record<string, unknown> }> }>;
  tags: unknown;
  [field: string]: unknown;
};
...
assert.deepEqual(Object.keys(requireValue(requireValue(preview.ticks[1]).layers.find(
  (layer) => layer.type === 'shape',
)).cells),
  ['1,1', '2,1', '3,1'], 'resolved live preview rasterizes interpolated shape paths');

seekTick(4);
const videoLayer = requireValue(get(layers).find((layer) => layer.type === 'video')) as EditorVideoLayer;
assert.deepEqual({
...
const trimmedVideoProject = JSON.parse(serializeJSON()) as RichProject;
const trimmedVideo = requireValue(trimmedVideoProject.timeline.clips.find(
  (clip) => clip.id === IDs.videoClip,
));
assert.deepEqual({
...
assert.deepEqual(requireValue(recoveredKeyState.clips.find((clip) => clip.id === IDs.frontClip))
  .frameKeys.map((key) => key.tick), [0, 1]);
assert.deepEqual(requireValue(requireValue(recoveredKeyState.clips.find(
  (clip) => clip.id === IDs.frontClip,
)).propertyTracks['visibility']).map((key) => key.tick), [0, 1]);
assert.deepEqual(requireValue(requireValue(recoveredKeyState.clips.find(
  (clip) => clip.id === IDs.shapeClip,
)).propertyTracks['shapePath']).map((key) => key.tick), [0, 1],
  'recovery contents preserve mixed moved key source ticks across clips');
loadJSON(JSON.stringify(source));
seekTick(4);

const runtimeRaster = canvasElement();
runtimeRaster.width = 2;
runtimeRaster.height = 2;
setLayerRaster(IDs.imageLayer, runtimeRaster);
const imageLayer = requireValue(get(layers).find(
  (layer) => layer.id === IDs.imageLayer,
));
assert.equal('raster' in imageLayer ? imageLayer.raster : undefined, runtimeRaster);
...
Object.defineProperty(globalThis, 'navigator', {
  configurable: true,
  value: { clipboard: { async writeText(value: string) { copiedText = value; } } },
});
...
type InvalidProjectCase = [string, (project: RichProject) => void, RegExp];
for (const [label, mutate, pattern] of ([
  ['missing tags', (project) => {
    Reflect.deleteProperty(project.timeline, 'tags');
  }, /Timeline tags must be an array/],
  ['old-only shape', (project) => {
    Object.defineProperty(project, 'timeline', {
      configurable: true,
      value: { frameCount: 1, holds: [1], layers: [] },
    });
  ...
  ['duplicate key tick', (project) => {
    const clip = requireValue(project.timeline.clips[0]);
    clip.frameKeys.push(structuredClone(requireValue(clip.frameKeys[0])));
  }, /strictly increasing/],
  ['invalid clip bound', (project) => {
    requireValue(project.timeline.clips[0]).outTick = 0;
  }, /outTick/],
  ['unknown payload field', (project) => {
    record(requireValue(requireValue(project.timeline.clips[0]).frameKeys[0]).value)['offset'] = {
      x: 1, y: 2,
    };
  }, /unsupported field offset/],
  ['duplicate visual track name alias', (project) => {
    requireValue(project.timeline.tracks.find((track) => track.layer)).name = 'Duplicate';
  }, /duplicates its layer name/],
  ['unknown property authority', (project) => {
    requireValue(project.timeline.clips[0]).propertyTracks['legacyPosition'] = [];
  }, /unsupported property legacyPosition/],
  ['invalid property value', (project) => {
    requireValue(project.timeline.clips[0]).propertyTracks['position'] = [
      { tick: 0, value: { x: '1', y: 0 } },
    ];
  }, /integer x\/y/],
  ['unsupported tag field', (project) => {
    requireValue(project.timeline.tags[0])['extra'] = true;
  }, /unsupported field extra/],
  ['duplicate tag ID', (project) => {
    requireValue(project.timeline.tags[1]).id = requireValue(project.timeline.tags[0]).id;
  }, /Duplicate timeline tag ID/],
  ['duplicate loop singleton', (project) => {
    const tag = requireValue(project.timeline.tags[1]);
  ...
  ['untrimmed custom tag', (project) => {
    const tag = requireValue(project.timeline.tags[1]);
    if (tag.type !== 'custom') throw new Error('Expected custom tag fixture.');
    tag.value = ' event ';
  }, /must be trimmed/],
  ['out-of-range tag', (project) => {
    requireValue(project.timeline.tags[0]).tick = 22;
  }, /inside the sequence/],
] satisfies InvalidProjectCase[])) {
  const invalid = JSON.parse(baseline) as RichProject;
  mutate(invalid);
...
let packageBlob: Blob | undefined;
assert.equal(await saveJSONAs({
  checkpoint: async () => {},
  notifySaved: async () => {},
  chooseTarget: async () => saveTarget(
    async (blob: Blob) => { packageBlob = blob; },
    'rich.paintty',
  ),
}), true);
const savedPackage = await decodeProjectArchive(new Uint8Array(
  await requireValue(packageBlob).arrayBuffer(),
));
assert.deepEqual(savedPackage.document, source);
assert.equal(get(dirty), false);

let runtimeBlob: Blob | undefined;
await exportAnimation({
  chooseTarget: async () => saveTarget(async (blob: Blob) => { runtimeBlob = blob; }),
});
const runtime = JSON.parse(await requireValue(runtimeBlob).text()) as {
  format: string;
...
let audioRuntimeZip: Blob | undefined;
await exportAnimation({
  includeAudio: true,
  decodeAudio: async () => ({ buffer: fixtureAudioBuffer() }),
  chooseTarget: async () => saveTarget(async (blob: Blob) => { audioRuntimeZip = blob; }),
});
const entries = unzipSync(new Uint8Array(await requireValue(audioRuntimeZip).arrayBuffer()));
const embedded = JSON.parse(strFromU8(requireValue(entries['rich.json']))) as {
  audio: unknown;
  tags: unknown;
  [field: string]: unknown;
};
...
assert.equal(strFromU8(requireValue(entries['rich.json'])).includes('voice.wav'), false);
assert.equal(Object.keys(entries).some((path) => path.includes(IDs.audioAsset)), false);
const mixedWav = requireValue(entries['audio.wav']);
assert.equal(strFromU8(mixedWav.subarray(0, 4)), 'RIFF');
assert.equal(mixedWav.length, 44 + 105_600 * 4,
...
assert.equal(await saveJSON({
  ...
  reportRecoveryError(error: unknown) {
    assert.match(error instanceof Error ? error.message : String(error), /recovery unavailable/);
    recoveryReports++;
  },
  notifySaved: async () => {},
  chooseTarget: async () => ({
...
assert.ok(requireValue<Blob>(durableWrite) instanceof Blob,
  'disk save proceeds after checkpoint failure');
...
assert.deepEqual(
  (await decodeProjectArchive(new Uint8Array(
    await requireValue<Blob>(saveAsBlob).arrayBuffer(),
  ))).document,
...
async function staleArchiveOpen(mutateWhileCaching: () => void): Promise<void> {
  const input = Object.create(null) as HTMLInputElement;
  Object.defineProperty(input, 'click', { value() {} });
  const cacheStarted = deferred<void>();
  const cacheFinish = deferred<void>();
  let restored = 0;
  type OpenOptions = NonNullable<Parameters<typeof openFileDialog>[0]>;
  type AssetInputs = Parameters<NonNullable<OpenOptions['storeAssets']>>[0];
  let cachedRecords: AssetInputs | null = null;
  openFileDialog({
    createInput: () => input,
    decodeArchive: async (): Promise<Awaited<ReturnType<typeof decodeProjectArchive>>> => ({
      document: JSON.parse(serializeJSON()),
      mediaBlobs: new Map([
        ['a'.repeat(64), new Blob(['a'])],
        ['b'.repeat(64), new Blob(['b'])],
      ]),
      manifest: Object.create(null) as Awaited<ReturnType<typeof decodeProjectArchive>>['manifest'],
    }),
    storeAssets: async (records): Promise<ProjectAssetRecord[]> => {
    ...
    showError(error: unknown) { throw new Error(String(error)); },
  });
  const file = namedFile(new Uint8Array(), 'application/zip', 'large.paintty');
  const fileList = Object.assign(Object.create(null) as FileList, {
    0: file,
    length: 1,
    item(index: number) { return index === 0 ? file : null; },
  });
  Object.defineProperty(input, 'files', { configurable: true, value: fileList });
  const opening = Promise.resolve(requireValue(input.onchange).call(input, new Event('change')));
  await cacheStarted.promise;
  mutateWhileCaching();
  cacheFinish.resolve();
  await opening;
  assert.equal(requireValue<AssetInputs>(cachedRecords).length, 2,
    'archive media cache writes are one complete batch');
...
const finalVideo = requireValue(getClipTimelineState().clips.find((clip) => clip.kind === 'video'));
assert.deepEqual({
  startTick: finalVideo.startTick,
  inPoint: finalVideo.inPoint,
  outPoint: finalVideo.outPoint,
