...
} from '../src/lib/panelLayout.ts';
import { isTopPopup, popupCountForTests, popupFocus, popupOpen } from '../src/lib/popupFocus.ts';
import { MINIMUM_VIEWPORT, viewportGate } from '../src/lib/viewportGate.ts';
import { performDiscardedProjectAction } from '../src/lib/documentReplacement.ts';
import { testDocument, testHtmlElement } from './types/ui-test-types.ts';
...
class RecordingEventTarget extends EventTarget {
  installed: ListenerRecord | undefined;
  removed: ListenerRecord | undefined;
  ...
  override addEventListener(
    ...
    this.installed = { type, listener, options };
  }
  ...
  override removeEventListener(
    ...
    this.removed = { type, listener, options };
  }
}

const eventTarget = new RecordingEventTarget();
const uninstall = installUniversalTabBlock(eventTarget);
assert.equal(eventTarget.installed?.type, 'keydown');
assert.equal(eventTarget.installed?.options, true);
assert.equal(typeof eventTarget.installed?.listener, 'function');
let listenerPrevented = 0;
let listenerStopped = 0;
if (typeof eventTarget.installed?.listener === 'function') {
  eventTarget.installed.listener({
    key: 'Tab',
    preventDefault() { listenerPrevented++; },
    stopImmediatePropagation() { listenerStopped++; },
  } as unknown as Event);
}
assert.deepEqual({ listenerPrevented, listenerStopped }, {
  listenerPrevented: 1,
  listenerStopped: 1,
});
uninstall();
assert.deepEqual(eventTarget.removed, eventTarget.installed);
...
for (const key of ['+', '=', '-', '0']) {
  assert.equal(nativeBrowserZoomShortcut({ key, ctrlKey: true }), true,
    `Ctrl+${key} remains browser-owned while the viewport is blocked`);
  assert.equal(nativeBrowserZoomShortcut({ key, metaKey: true }), true,
    `Cmd+${key} remains browser-owned while the viewport is blocked`);
}
assert.equal(nativeBrowserZoomShortcut({ key: '-' }), false);
assert.equal(nativeBrowserZoomShortcut({ key: 's', ctrlKey: true }), false);
assert.equal(nativeBrowserZoomShortcut({ key: '-', ctrlKey: true, altKey: true }), false);
const rangeInput = {
  type: 'range',
  closest(selector: string) { return selector === 'input' ? this : null; },
};
const textInput = {
  type: 'text',
  closest(selector: string) { return selector === 'input' ? this : null; },
...
const previous = testHtmlElement({
  isConnected: true,
  focus() { restoredFocus++; },
});
globalThis.document = testDocument(previous);
const initial = testHtmlElement({ focus() { initialFocus++; } });
const node = testHtmlElement({
...
const outer = testHtmlElement();
const inner = testHtmlElement();
...
const convertPopup = source('src/components/ConvertPopup.svelte');
const shapeProperties = source('src/components/ShapeProperties.svelte');
const globalCss = source('src/styles/global.css');
for (const popup of [
  'ConvertPopup', 'DiscardChangesPopup', 'ExportPopup', 'HelpPopup', 'NewProjectPopup',
  'Preferences', 'ProjectAssets', 'ProjectSettings', 'PurgeUnusedMediaPopup', 'TuiHelperPopup',
]) {
  const popupSource = source(`src/components/${popup}.svelte`);
  assert.doesNotMatch(popupSource, /backdropClick|onclick=\{backdropClick\}/,
    `${popup} must require an explicit Close or Cancel control`);
}
assert.match(globalCss, /:where\(\*\):focus, :where\(\*\):focus-visible \{ outline: none; \}/);
assert.doesNotMatch(convertPopup, /Ctrl\+Enter|Glyph limit|Color limit|Auto uses|preview-meta/);
assert.match(convertPopup, /background = \$state(?:<[^>]+>)?\('source'\)/);
assert.match(convertPopup, /glyphLimit: 256/);
assert.match(convertPopup, /colorLimit: 16/);
assert.doesNotMatch(shapeProperties, /Background requires|disabled-hint/);
assert.match(shapeProperties, /option value="background" disabled=\{backgroundChannelBlocked\}/);
assert.match(globalCss, /:where\(button, input, select, textarea\):disabled[\s\S]*opacity: 0\.42/);
assert.match(canvas, /const CANVAS_BOUND = new Set(?:<[^>]+>)?\(\['crop'\]\)/);
assert.match(canvas, /if \(\$selection\.size && \$selection\.has\(selectionKey\(x, y\)\)\) return;/);
assert.match(canvas, /selDrag\.x0 !== selDrag\.x1 \|\| selDrag\.y0 !== selDrag\.y1/);
assert.doesNotMatch(canvas, /commitRect[\s\S]{0,300}inBounds\(x, y\)/);
assert.doesNotMatch(canvas, /commitLasso[\s\S]{0,800}inBounds\(x, y\)/);
assert.match(canvas, /event\.button === 2[\s\S]*\$activeTool === 'brush'[\s\S]*\$activeTool === 'subcell'/);
assert.doesNotMatch(toolOptions, /> Transform<\/button>/);
assert.doesNotMatch(canvas, />Transform selection<\/button>/);
assert.match(app, /tool === 'crop' && previousTool !== 'crop'\) toolBeforeCrop = previousTool/);
assert.match(app, /window\.addEventListener\('crop-finished', finishCrop\)/);
assert.match(app, /if \(e\.type === 'keyup'\) eyedropKeyHeld = false/);
assert.match(canvas, /const sampling = \$altEyedrop/);
assert.match(canvas, /activeImage && imgGizmo && \$activeTool !== 'crop'/);
assert.equal((canvas.match(/class="img-edge [nesw]"/g) || []).length, 4);
for (const handler of [
  'onActionPointerDown', 'dragImageBody', 'dragImageScale', 'dragImageRotate',
  'dragTextBox', 'dragShapeBody', 'dragShapeHandle', 'startCrop',
]) {
  assert.match(canvas,
    new RegExp(`function ${handler}[\\s\\S]{0,300}?\\{\\s*if \\([^)]*event\\.button !== 0[^)]*\\) return;`));
}
assert.match(menu, /\.menu-item:not\(:disabled\):focus-visible \{ background: var\(--accent-dim\); \}/);
assert.equal((canvas.match(/stopTextWrapSubscription\(\)/g) || []).length, 1,
  'text wrap subscription remains active until Canvas is destroyed');
assert.match(layersPanel, /title="Delete selected layers"[\s\S]*title="New group"/);
assert.match(layersPanel, /class="drop-end" class:active=\{dropGap && dropGap\.beforeId === null\}/);
assert.match(layersPanel, /\.drop-end \{ height: 8px;[\s\S]*border-top: 2px solid transparent;/);
assert.doesNotMatch(layersPanel, /layers\.drop-bottom/);
...
assert.match(app, /if \(viewportState\.blocked\) \{\s*if \(nativeBrowserZoomShortcut\(e\)\) return;\s*e\.preventDefault\(\);/,
  'the viewport blocker preserves native browser zoom but still blocks application input');
...
assert.match(app, /nativeInputOwnsKey\(shortcutEvent\)/);
...
assert.match(app, /function onCopy\(e:\s*ClipboardEvent\)/);
assert.match(app, /copyClipsForContext\(keyboardContext, e\.clipboardData\)/);
assert.match(app, /oncopy=\{onCopy\}/);
assert.match(app, /oncontextmenu=\{preventNativeContextMenu\}/);
assert.match(app, /ctrl && e\.key\.toLowerCase\(\) === 't'[\s\S]*activeTool\.set\('move'\)/);
assert.doesNotMatch(app, /ctrl && e\.key\.toLowerCase\(\) === 't'[\s\S]{0,300}beginTransformSelection/);
...
assert.match(app, /clipboardMediaPlacementSucceeded\(imported\)[\s\S]{0,100}activeTool\.set\('move'\)/,
  'a placed bitmap switches directly to Move for immediate positioning');
...
assert.match(timeline, /nativeInputOwnsKey\(\{/);
