...
import { isWide } from '../src/lib/width.ts';
import {
  applyTool,
...
function currentCellLayer(index = 0): EditorCellLayer {
  return currentLayer(index) as EditorCellLayer;
}

function currentEffectLayer(index = 0): EditorEffectLayer {
  return currentLayer(index) as EditorEffectLayer;
}
...
let positionGroup = currentLayer(0);
let positionedGlyph = currentLayer(1);
...
eq('grouped-offset-direct-write-stores-layer-local-coordinates', {
  local: currentCellLayer(1).cells['2,3'],
  doubled: currentCellLayer(1).cells['8,5'],
}, {
  local: { c: 'D', fg: '#222222' },
  doubled: undefined,
});
...
eq('grouped-offset-brush-stays-under-the-world-space-pointer',
  currentCellLayer(1).cells['3,4'], { c: 'B', fg: '#334455', bg: null });
applyTool(9, 6, {}, 'down', 0.5, 0.5, undefined, 'eraser');
eq('temporary eraser override clears without changing the selected brush', [
  currentCellLayer(1).cells['3,4'], get(activeTool),
], [undefined, 'brush']);
applyTool(9, 6, {}, 'down');
activeTool.set('eraser');
applyTool(9, 6, {}, 'down');
eq('grouped-offset-eraser-clears-the-same-local-cell', currentCellLayer(1).cells['3,4'], undefined);
...
let backgroundGroup = currentLayer(0);
let positionedBackground = currentLayer(1);
...
eq('grouped-offset-background-brush-stores-layer-local-coordinates',
  currentCellLayer(1).cells['1,1'], { c: '', fg: null, bg: '#556677' });
activeTool.set('eraser');
applyTool(0, 4, {}, 'down');
eq('grouped-offset-background-eraser-clears-the-same-local-cell',
  currentCellLayer(1).cells['1,1'], undefined);
...
let maskGroup = currentLayer(0);
let positionedMask = currentLayer(1);
...
eq('effect-mask-world-coordinate-mapping-still-includes-layer-group-and-mask-offsets', [
  currentEffectLayer(1).mask!.cells['2,1'],
  getCell(4, 5),
], [
  { mask: 0.25 },
  { mask: 0.25 },
...
applyTool(0, 0, {}, 'down');
endStroke();
const filled = currentCellLayer().cells;

eq('whole-canvas-fill-count', Object.keys(filled).length, 80 * 30);
eq('whole-canvas-fill-corners', [filled['0,0']?.c, filled['79,29']?.c], ['#', '#']);
undo();
eq('whole-canvas-fill-undo', Object.keys(currentCellLayer().cells).length, 0);

const border: EditorCellMap = {};
for (let x = 10; x <= 20; x++) {
...
const bounded = currentCellLayer().cells;
...
activeChar.set('+');
applyTool(1, 1, {}, 'down');
const selectedFill = currentCellLayer().cells;
...
applyTool(0, 0, {}, 'down');
eq('fill-outside-selection-is-no-op', Object.keys(currentCellLayer().cells).length, 0);
...
applyTool(0, 0, {}, 'down');
const colorBoundary = currentCellLayer().cells;
...
eq('half-fill-writes-logical-halves-through-the-real-tool-path', currentCellLayer().cells, {
...
eq('quarter-fill-samples-the-composite-and-writes-only-the-active-layer', [
  currentCellLayer(0).cells,
  currentCellLayer(1).cells,
], [
...
paintColor.set('#334455');
applyTool(0, 0, {}, 'down', 0.25, 0.25);
eq('background-fill-ignores-stale-subpixel-resolution', currentCellLayer().cells, {
...
applyTool(0, 0, {}, 'down');
eq('whole-glyph-fill-preserves-the-active-cell-background', currentCellLayer().cells, {
...
eq('whole-glyph-fill-ignores-background-when-matching-empty-cells', [
  currentCellLayer().cells['0,0'],
  currentCellLayer().cells['1,0'],
], [
  { c: 'X', fg: '#00ff00', bg: null },
...
applyTool(0, 0, {}, 'down');
eq('whole-fill-clears-the-replaced-wide-continuation', currentCellLayer().cells, {
...
globalThis.document = {
  createElement() {
    return { getContext: () => ({ measureText: (glyph: string) => ({ width: glyph === '界' ? 2 : 1 }) }) };
  },
...
applyTool(0, 0, {}, 'down');
eq('wide-brush-keeps-off-canvas-continuation', currentCellLayer().cells, {
...
applyTool(0, 0, {}, 'down');
eq('wide-fill-builds-complete-nonoverlapping-pairs', currentCellLayer().cells, {
...
eq('special-brush-clears-unmarked-wide-leader-to-its-left', currentCellLayer().cells, {
...
eq('effect-mask-fill-ignores-stale-subpixel-resolution',
  Math.round(currentEffectLayer().mask!.cells['0,0']!.mask! * 1e6) / 1e6, 1);
activeLayerPart.set('layer');
...
endStroke();
eq('special-brush-revises-the-turn-and-clears-wide-metadata', currentCellLayer().cells, {
...
undo();
eq('special-brush-is-one-undoable-stroke', currentCellLayer().cells, {
  '2,0': { c: '界', fg: '#111111', bg: '#010203' },
  '3,0': { c: '', fg: '#111111', bg: '#040506', cont: true },
});
...
eq('special-brush-paints-past-the-canvas-edge-without-a-marquee', currentCellLayer().cells, {
  '0,0': { c: '─', fg: '#e0a458', bg: null },
  '-1,0': { c: '─', fg: '#e0a458', bg: null },
  '-2,0': { c: '─', fg: '#e0a458', bg: null },
});
...
eq('selection-clips-cells-without-changing-segment-orientation', currentCellLayer().cells, {
...
for (const style of ['single', 'rounded', 'double', 'heavy'] as EditorBoxStyle[]) {
...
for (const [mode, fx, fy] of [
  ['half', 0.5, 0.25],
  ['quarter', 0.25, 0.25],
] as Array<['half' | 'quarter', number, number]>) {
  ...
  eq(`${mode} sub-cell brush click remains a one-cell edit`, {
    changed,
    cells: Object.keys(currentCellLayer().cells),
  }, {
    changed: true,
    cells: ['0,0'],
  });
...
const specialLine = {
  kind: 'line', style: 'special', x0: 1, y0: 2, x1: 7, y1: 6,
} as EditorShape;
...
const slopeLine = {
  kind: 'line', style: 'slope', x0: 2, y0: 2, x1: 6, y1: 6,
} as EditorShape;
