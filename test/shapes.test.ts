import {
  BOX_STYLES,
  constrainShape as constrainEditorShape,
  isSlopeLine as isEditorSlopeLine,
  lineStylePatch,
  lineStyleValue as editorLineStyleValue,
  maskShapeAppearance as editorMaskShapeAppearance,
  normalizeShapeThickness,
  normalizeStrokeAlign,
  orthogonalLinePoints,
  regularPolygonVertices,
  renderShapeToCells as renderEditorShapeToCells,
  resolvedShapeAnchor as resolvedEditorShapeAnchor,
  resolvedShapeVertices as resolvedEditorShapeVertices,
  shapeGlyphs as editorShapeGlyphs,
  specialBrushGlyphs,
  updateMaskShapeAppearance as updateEditorMaskShapeAppearance,
  updateShapeAppearance as updateEditorShapeAppearance,
...
type ShapeGlyph = ReturnType<typeof editorShapeGlyphs>[number];
interface MapConstructorWithGroupBy {
  groupBy<K, V>(items: Iterable<V>, keySelector: (item: V, index: number) => K): Map<K, V[]>;
}
...
function constrainShape(shape: ShapeFixture): EditorShape {
  return constrainEditorShape(shape as EditorShape);
}

function isSlopeLine(shape: ShapeFixture): boolean {
  return isEditorSlopeLine(shape as EditorShapeAppearance);
}

function lineStyleValue(shape: ShapeFixture): string {
  return editorLineStyleValue(shape as EditorShapeAppearance);
}

function maskShapeAppearance(shape: ShapeFixture): EditorShapeAppearance {
  return editorMaskShapeAppearance(shape as EditorShapeAppearance);
}

function updateMaskShapeAppearance(
  shape: ShapeFixture,
  patch: ShapeFixture,
): EditorShapeAppearance {
  return updateEditorMaskShapeAppearance(
    shape as EditorShapeAppearance,
    patch as EditorShapeAppearance,
  );
}

function updateShapeAppearance(
  shape: ShapeFixture,
  patch: ShapeFixture,
): EditorShapeAppearance {
  return updateEditorShapeAppearance(
    shape as EditorShapeAppearance,
    patch as EditorShapeAppearance,
  );
}
...
function resolvedShapeAnchor(shape: ShapeFixture): EditorPoint {
  return resolvedEditorShapeAnchor(shape as EditorShape);
}
...
function shapeGlyphs(shape: ShapeFixture): ReturnType<typeof editorShapeGlyphs> {
  return editorShapeGlyphs(shape as EditorShape);
}
...
function endpointPair(shape: ShapeCoordinates): number[][] {
  return [[shape.x0, shape.y0], [shape.x1, shape.y1]]
    .sort((left, right) => left[0]! - right[0]! || left[1]! - right[1]!);
}

function orderedCellMap(shape: ShapeFixture) {
...
const reversibleSlopeDrags: Array<[string, EditorPoint, EditorPoint]> = [
...
eq('slope-rendering-does-not-depend-on-which-handle-started-the-drag',
  reversedFallingSlope.map(({ x, y, ch }): [number, number, string] => [x, y, ch!])
    .sort((a, b) => a[1] - b[1] || a[0] - b[0]),
  shapeGlyphs(fallingSlope).map(({ x, y, ch }): [number, number, string] => [x, y, ch!])
    .sort((a, b) => a[1] - b[1] || a[0] - b[0]));
...
const diagonalDrags: Array<[number, number]> = [[1, 1], [4, 3], [7, 3], [-5, 2], [5, -4], [-8, -3]];
ok('diagonal-triangle-runs-are-nonempty-edge-sharing-lines',
  diagonalDrags.every(([x1, y1]) => {
    ...
    const byRow = (Map as unknown as MapConstructorWithGroupBy)
      .groupBy(glyphs, (point) => point.y);
...
const DIRECTIONS: Array<{ bit: number; dx: number; dy: number; opposite: number }> = [
  { bit: 1, dx: 0, dy: -1, opposite: 4 },
  { bit: 2, dx: 1, dy: 0, opposite: 8 },
  { bit: 4, dx: 0, dy: 1, opposite: 1 },
  { bit: 8, dx: -1, dy: 0, opposite: 2 },
];
const SINGLE_CONNECTIONS = new Map<string, number>([
  ['│', 1 | 4], ['─', 2 | 8],
  ['┌', 2 | 4], ['┐', 8 | 4], ['└', 2 | 1], ['┘', 8 | 1],
  ['┬', 2 | 8 | 4], ['┤', 1 | 4 | 8], ['┴', 2 | 8 | 1], ['├', 1 | 2 | 4],
  ['┼', 1 | 2 | 4 | 8],
]);
const bitCount = (value: number) => DIRECTIONS.reduce((count, { bit }) => count + ((value & bit) ? 1 : 0), 0);
const mirrorXConnections = (value: number) =>
  (value & 1) | (value & 4) | ((value & 2) ? 8 : 0) | ((value & 8) ? 2 : 0);
const mirrorYConnections = (value: number) =>
  (value & 2) | (value & 8) | ((value & 1) ? 4 : 0) | ((value & 4) ? 1 : 0);

function inspectSpecialCircle(width: number, height: number) {
  ...
  const map = new Map<string, number>(glyphs.map((point): [string, number] => [
    `${point.x},${point.y}`,
    SINGLE_CONNECTIONS.get(point.ch!)!,
  ]));
...
ok('special-circle-contours-form-one-closed-loop',
  inspectedCircles.every(({ glyphs, map }) => {
    ...
    const pending: EditorPoint[] = [start];
...
function localGlyphCoordinates(
  shape: ShapeCoordinates & Record<string, unknown>,
): Array<[number, number]> {
  const xa = Math.min(shape.x0, shape.x1);
  const ya = Math.min(shape.y0, shape.y1);
  return shapeGlyphs(shape).map(({ x, y }): [number, number] => [x - xa, y - ya])
...
const connectedPentagon = new Set<string>();
const pendingPentagon: string[] = [[...joinedPentagonCells][0]!];
...
function glyphKeySet(shape: ShapeFixture): Set<string> {
