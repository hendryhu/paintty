...
} from '../src/lib/timelineViewport.ts';
import {
  clipTimelineState,
  errorStack,
  requireValue,
  visualTimelineClip as clip,
  visualTimelineTrack,
...
test('row prefix indexes accept row records and skip zero-height rows', () => {
  interface SizeRow { size: number; height?: never }
  const sizeRows: SizeRow[] = [{ size: 12 }, { size: 0 }, { size: 18 }];
...
test('snapping chooses nearby playhead, clip edges, and visible frame keys', () => {
  ...
  assert.equal(requireValue(edge.target).kind, 'clip-end');
  ...
  assert.equal(requireValue(tie.target).kind, 'playhead');
});
...
test('common-edge start planning honors zero, neighbors, locks, and one-tick clips', () => {
  const state = clipTimelineState({
    tracks: [visualTimelineTrack({ id: 'a' }), visualTimelineTrack({ id: 'b', locked: true })],
    clips: [
