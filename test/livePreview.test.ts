...
import {
  ...
  sessionMarkerPath,
  writePreview,
} from '../src/lib/livePreview.ts';
import { serializeLivePreview } from '../src/lib/fileio.ts';
import { initTimeline, loadCanonicalTimeline } from '../src/lib/frames.ts';
import type {
...
interface VirtualFolderOptions {
  initial?: Record<string, string>;
  failClose?: string | null;
  name?: string;
}

function virtualFolder({
  initial = {}, failClose = null, name = 'watch-fixture',
}: VirtualFolderOptions = {}) {
  ...
  function fileHandle(path: string): FileSystemFileHandleLike {
    return {
      ...
      async createWritable(): Promise<FileSystemWritableLike> {
        ...
        let pending = '';
        return {
          async write(contents: Blob | string) {
            events.push('write:' + path);
            pending = typeof contents === 'string' ? contents : await contents.text();
          },
          async close() {
      ...
      async getFile() {
        ...
        return new File([requireValue(files.get(path))], path.split('/').at(-1) || path);
      },
    };
  }

  function directoryHandle(path: string): FileSystemDirectoryHandleLike {
    return {
      name: path ? path.split('/').at(-1) || name : name,
  ...
  return {
    ...
    files,
    events,
    setFailClose(path: string | null) { failedPath = path; },
  };
}
...
function fakeTimers() {
  ...
  const timeouts = new Map<TimerHandle, TimerRecord>();
  const intervals = new Map<TimerHandle, TimerRecord>();
  const handle = (): TimerHandle => Object.create(null) as TimerHandle;
  return {
    ...
    timeoutCount(delay: number): number {
      return [...timeouts.values()].filter((timer) => timer.delay === delay).length;
    },
    runTimeout(delay: number): void | Promise<void> {
    ...
    interval(): TimerRecord | undefined {
...

{
  ...
  assert.deepEqual(JSON.parse(requireValue(fs.files.get(path))), JSON.parse(project));
  ...
  assert.deepEqual(JSON.parse(requireValue(fs.files.get('.paintty-preview/sessions/tab-a.json'))), {
    version: 2,
    path,
    playheadTick: 4,
    updatedAt: 1234,
...

{
  ...
  assert.equal(JSON.parse(requireValue(fs.files.get(path))).revision, 20);
...

{
  ...
  const path = requireValue(sync.currentPath);
...

{
  ...
  requireValue(timers.interval()).callback();
...

{
  ...
  assert.equal(JSON.parse(requireValue(fs.files.get('.paintty-preview/sessions/tab-a.json'))).path, pathA);
  assert.equal(JSON.parse(requireValue(fs.files.get('.paintty-preview/sessions/tab-b.json'))).path, pathB);
  assert.equal(fs.files.get('.opened'), pathB);
...

{
  ...
  await sync.useWatchFolder(fs.root);
  const stop = sync.start();
  assert.equal(requireValue(timers.interval()).delay, PREVIEW_HEARTBEAT_MS);
  await sync.flushProject();

  const firstPath = '.paintty-preview/previews/live-tab-live-boss.json';
  assert.deepEqual(JSON.parse(requireValue(fs.files.get(firstPath))), project);
  assert.equal(Object.hasOwn(JSON.parse(requireValue(fs.files.get(firstPath))), 'playheadTick'), false);
  ...
  assert.equal(
    JSON.parse(requireValue(fs.files.get('.paintty-preview/sessions/live-tab.json'))).playheadTick,
    17,
  );
  ...
  await sync.flushMarker();
  assert.equal(
    JSON.parse(requireValue(fs.files.get('.paintty-preview/sessions/live-tab.json'))).updatedAt,
    clock,
  );
  assert.equal(
    fs.events.filter((event) => event === 'write:' + firstPath).length,
  ...
  assert.deepEqual(JSON.parse(requireValue(fs.files.get(renamedPath))), project);
  assert.equal(
    JSON.parse(requireValue(fs.files.get('.paintty-preview/sessions/live-tab.json'))).path,
    renamedPath,
  );

  content.set({ revision: 2 });
  requireValue(project.timeline.clips[0]).frameKeys.push({ tick: 1, value: { cells: {} } });
  await sync.flushProject();
  assert.deepEqual(JSON.parse(requireValue(fs.files.get(renamedPath))), project);
...

{
  ...
  assert.equal(JSON.parse(requireValue(fs.files.get(path))).revision, 1);
  ...
  await sync.flushMarker();
  assert.equal(JSON.parse(requireValue(fs.files.get(path))).revision, 2);
  assert.equal(JSON.parse(requireValue(fs.files.get(markerPath))).playheadTick, 9);
