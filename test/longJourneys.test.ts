...
import { loadJSON, serializeJSON, serializeTXT } from '../src/lib/fileio.ts';
import { renderShapeToCells } from '../src/lib/shapes.ts';
import { activeChar, activeTool, paintColor } from '../src/lib/stores.ts';
import { applyTool } from '../src/lib/tools.ts';
import { getClipTimelineState } from '../src/lib/clipTimelineState.ts';
import { validLoopRange } from '../src/lib/timelineTags.ts';
import type { EditorShape } from '../src/lib/types/editor-domain.ts';
import {
...
function resetDocument(
  layerDefs: Parameters<typeof G.setLayers>[0],
  width: number,
  height: number,
): void {
...
function stroke(action: () => void): void {
  G.beginStroke();
  action();
  G.endStroke();
}

function extendVisualClips(endTick: number): void {
  G.beginStroke();
...
function frameSummary(): Array<{ glyphs: string[]; backgrounds: string[] }> {
  ...
  return get(F.frames).map((frame) => {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const cell = requireValue(cells[y])[x];
...
function glyphPositions(glyph: string): string[][] {
  const { w, h } = get(G.dims);
  return get(F.frames).map((frame) => {
    const cells = F.compositeFrameCells(frame, w, h);
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        if (requireValue(cells[y])[x]?.c === glyph) positions.push(`${x},${y}`);
      }
    }
    return positions;
  });
...
function renderedForegrounds() {
  ...
  assert.equal(h, 1);
  return get(F.frames).map((frame) =>
    requireValue(F.compositeFrameCells(frame, w, h)[0]).map((cell) => cell?.fg ?? null));
}
...
function glyphBoundsByFrame() {
  ...
  return get(F.frames).map((frame) => {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        if (requireValue(cells[y])[x]?.c) points.push({ x, y });
      }
    }
...
function animatedGroupedSaveJourney() {
  ...
  const glyphId = requireValue(get(G.activeLayerId));
  ...
  const backgroundId = requireValue(get(G.activeLayerId));
  stroke(() => {
    G.setCell(1, 1, { c: '', fg: null, bg: '#101820' });
    G.setCell(2, 1, { c: '', fg: null, bg: '#203040' });
  });

  G.toggleLayerSelected(requireValue(glyphId));
  G.groupActiveLayer();
  const groupId = requireValue(get(G.activeLayerId));
  assert.deepEqual(
  ...
  F.gotoFrame(6);
  const restoredGlyph = requireValue(get(G.layers).find((layer) => layer.name === 'Glyphs'));
  G.selectLayer(restoredGlyph.id);
  stroke(() => G.setCell(5, 1, { c: 'B', fg: '#00ff00', bg: null }));
  F.commitLayersToActiveFrame();
  assert.deepEqual(requireValue(frameSummary()[6]).glyphs, [
    '4,1:A:#ff0000',
    '5,1:B:#00ff00',
  ]);
  G.undo();
  assert.deepEqual(requireValue(frameSummary()[6]).glyphs, ['4,1:A:#ff0000']);
  G.redo();
  assert.deepEqual(requireValue(frameSummary()[6]).glyphs, [
    '4,1:A:#ff0000',
    '5,1:B:#00ff00',
...
function wideSelectionCropJourney() {
  ...
  F.gotoFrame(1);
  assert.deepEqual(Array.from(requireValue(serializeTXT().split('\n')[1])), [
    ' ', ' ', ' ', ' ', ' ', '界', ' ',
  ]);
}
...
function animatedEffectJourney() {
  ...
  G.addLayer('effect');
  const effectId = requireValue(get(G.activeLayerId));
  G.setEffectProperties(requireValue(effectId), { kind: 'brightness', intensity: 0.25 });
  assert.equal(G.selectEffectMask(effectId), true);
  ...
  const restoredEffect = requireValue(get(G.layers).find((layer) => layer.type === 'effect'));
  ...
  const effectTrack = requireValue(
    restoredState.tracks.find((track) => track.layer?.id === restoredEffect.id),
  );
  const effectClip = requireValue(restoredState.clips.find((clip) => clip.trackId === effectTrack.id));
  const movedSelection = clipTimelineSelection({
    propertyKeys: [
  ...
  G.redo();
  loadJSON(serializeJSON());
  const movedEffect = requireValue(get(G.layers).find((layer) => layer.type === 'effect'));
...
async function shapePathLifecycleJourney() {
  const shape: EditorShape & { color: string } = {
    kind: 'rect',
    x0: 2,
    y0: 2,
    x1: 4,
  ...
  const shapeId = requireValue(get(G.activeLayerId));
  ...
  const originalClip = requireValue(
  ...
  loadJSON(serializeJSON());
  const restoredShape = requireValue(
    get(G.layers).find((layer) => layer.name === 'Path rectangle'),
  );
...
function sequenceTagLifecycleJourney() {
  ...
  assert.deepEqual(authored.map((tag) => [
    tag.tick,
    tag.type,
  ...
  const loopEnd = requireValue(authored.find((tag) => tag.type === 'loop-end'));
...
const journeys: Array<[string, TestRun]> = [
