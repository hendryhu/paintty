...
import { muxH264Mp4 } from '../src/lib/mp4.ts';
import { errorStack, requireValue, type TestRun } from './types/timeline-media-test-types.ts';
...
function readText(data: Uint8Array, offset: number, length: number): string {
  return String.fromCharCode(...data.subarray(offset, offset + length));
}

function parseBoxes(data: Uint8Array, start = 0, end = data.byteLength): Mp4Box[] {
  const boxes: Mp4Box[] = [];
  let offset = start;
  while (offset < end) {
...
function required(boxes: Mp4Box[], type: string): Mp4Box {
  const box = boxes.find((candidate) => candidate.type === type);
  assert.ok(box, `${type} box exists`);
  return box;
}

function children(data: Uint8Array, parent: Mp4Box): Mp4Box[] {
  return parseBoxes(data, parent.start + 8, parent.end);
}

function readDescriptor(data: Uint8Array, offset: number, limit: number): Mp4Descriptor {
  assert.ok(offset < limit, 'descriptor tag fits inside its parent');
  const tag = requireValue(data[offset++]);
  let size = 0;
  let terminated = false;
  for (let i = 0; i < 4; i++) {
    assert.ok(offset < limit, 'descriptor size fits inside its parent');
    const value = requireValue(data[offset++]);
...
function trackParts(data: Uint8Array, trak: Mp4Box) {
...
await test('muxes an independently parseable constant-frame-rate AVC file', () => {
  ...
  const output = muxH264Mp4({
    samples: [
      { data: requireValue(sampleData[0]), key: true },
      { data: requireValue(sampleData[1]), key: false },
      { data: requireValue(sampleData[2]), key: true },
    ],
  ...
  const ftyp = requireValue(top[0]);
  assert.equal(readText(output, ftyp.start + 8, 4), 'isom');
  assert.match(readText(output, ftyp.start + 16, ftyp.size - 16), /avc1/);

  const moovChildren = children(output, requireValue(top[1]));
  ...
  const avc1 = requireValue(parseBoxes(output, stsd.start + 16, stsd.end)[0]);
  assert.equal(avc1.type, 'avc1');
  assert.equal(readU16(output, avc1.start + 32), 80);
  assert.equal(readU16(output, avc1.start + 34), 30);
  const avcC = requireValue(parseBoxes(output, avc1.start + 86, avc1.end)[0]);
  ...
  const mdat = requireValue(top[2]);
  assert.equal(readU32(output, stco.start + 16), mdat.start + 8);
  assert.deepEqual(
    output.subarray(mdat.start + 8, mdat.end),
...
await test('muxes WebCodecs-style AAC chunks as an independent audio track', () => {
  ...
  const encodedChunk = (data: Uint8Array, timestamp: number, duration: number) => ({
    timestamp,
    duration,
  ...
  const output = muxH264Mp4({
    ...
    audio: {
      samples: [
        encodedChunk(requireValue(audioData[0]), 0, 21_333),
        encodedChunk(requireValue(audioData[1]), 21_333, 21_334),
        encodedChunk(requireValue(audioData[2]), 42_667, 21_333),
      ],
      decoderConfig: {
        codec: 'mp4a.40.2',
        sampleRate: 48_000,
  ...
  const video = trackParts(output, requireValue(tracks[0]));
  const audio = trackParts(output, requireValue(tracks[1]));
  ...
  assert.equal(readU32(output, stsd.start + 12), 1);
  const mp4a = requireValue(parseBoxes(output, stsd.start + 16, stsd.end)[0]);
  ...
  const esds = requireValue(parseBoxes(output, mp4a.start + 36, mp4a.end)[0]);
  ...
  const mdat = requireValue(top[2]);
  assert.equal(videoOffset, mdat.start + 8);
  ...
  assert.deepEqual(
    output.subarray(audioOffset, mdat.end),
...
await test('copies exact typed-array windows', () => {
  ...
  const top = parseBoxes(output);
  assert.deepEqual(output.subarray(requireValue(top[2]).start + 8), new Uint8Array([1, 2, 3]));
  const trak = required(children(output, requireValue(top[1])), 'trak');
...
await test('rejects malformed AAC configuration, chunks, and timelines', () => {
  ...
  const muxAudio = (audio: unknown) => muxH264Mp4({ ...valid, audio });
