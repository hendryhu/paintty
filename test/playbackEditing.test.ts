...
function unchanged(name: string, before: ReturnType<typeof snapshot>): void {
...
const layerId = requireValue(get(G.layers)[0]).id;
...
G.redo();
assert.equal(requireValue(get(G.layers)[0]).name, 'Ink authored',
  'playback keeps the prior Redo entry intact');
G.undo();
assert.equal(requireValue(get(G.layers)[0]).name, 'Ink',
  'Undo still reaches the same authored state after playback');
