...
} from '../src/lib/exportDestination.ts';
...
import { canvasElement, requireValue } from './types/timeline-media-test-types.ts';
...
let pickerOptions: PickerOptions | null = null;
  ...
  async createWritable(): Promise<FileSystemWritableLike> {
    ...
    return {
      async write(blob: Blob | string) {
        writes.push(typeof blob === 'string' ? blob : await blob.text());
      },
      async close() { closes++; },
      async abort() { aborts++; },
    };
  },
  async getFile() { return new File([], 'chosen.zip'); },
};
const browsed = await pickExportFileTarget(
  'suggested.zip',
  exportOutputSpec('animation-json', { includeAudio: true }),
  {
    async showSaveFilePicker(options: PickerOptions) {
...
const selectedTarget = requireValue(browsed);
assert.equal(selectedTarget.name, 'chosen.zip');
...
await selectedTarget.write(new Blob(['one export']));
...
assert.equal(compatibleRetainedTarget(selectedTarget, 'chosen.zip', '.zip'), selectedTarget);
assert.equal(compatibleRetainedTarget(selectedTarget, 'chosen.json', '.json'), null,
  'audio exclusion invalidates a ZIP handle');
assert.equal(compatibleRetainedTarget(selectedTarget, 'renamed.zip', '.zip'), null,
  'editing the filename invalidates a retained handle');
assert.equal(compatibleRetainedTarget(selectedTarget, 'chosen.zip', '.json'), null,
...
writeCompletion.resolve(undefined);
await assert.rejects(cancelledWrite,
...
try {
  ...
  const pickerWindow = Object.assign(Object.create(null) as Window, {
    async showSaveFilePicker(options: PickerOptions): Promise<FileSystemFileHandleLike> {
      defaultPickerCalls++;
      ...
      return {
        ...
        async getFile() { return new File([], 'native.png'); },
      };
    },
  });
  ...
  assert.equal(requireValue(nativeDefault).name, 'native.png');

  Reflect.deleteProperty(globalThis, 'window');
  assert.equal(exportPickerAvailable(), false);
  const fallback = await chooseSaveTarget('fallback.txt', 'text/plain', 'Text');
  assert.equal(requireValue(fallback).name, 'fallback.txt');
  assert.equal(requireValue(fallback).durable, false,
    'unsupported File System Access falls back to download');
} finally {
...
const targetFor = async (filename: string): Promise<SaveTarget> => {
  selectedNames.push(filename);
  return { name: filename, durable: true, async write() {} };
};
...
assert.equal(await exportVideo(8, false, {
  ...
  preflight: async () => Object.create(null) as Awaited<ReturnType<
    NonNullable<NonNullable<Parameters<typeof exportVideo>[2]>['preflight']>
  >>,
