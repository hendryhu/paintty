...
function snapshot(result: ReturnType<typeof selectTimelineKey>) {
  return {
...
const timelineNode = Object.assign(Object.create(null) as Node, { nodeType: 1 });
const commandTarget = Object.assign(Object.create(null) as Node, { type: 'menu-command' });
const timelineContainer = Object.assign(Object.create(null) as Node, {
...
eq('timeline shortcuts ignore command events without a DOM target', [
  ...
  timelineContainsTarget(timelineContainer, commandTarget),
  timelineContainsTarget(timelineContainer, null),
], [true, false, false]);

interface TestElement extends Element {
  type?: string;
  blur(): void;
}
function testElement(context: string | null = null, type?: string): TestElement {
  const element = Object.create(null) as TestElement;
  if (type !== undefined) element.type = type;
  element.getAttribute = (name: string) =>
    name === 'data-keyboard-context' ? context : null;
  element.closest = (selector: string) =>
    selector === '[data-keyboard-context]' && context !== null ? element : null;
  element.blur = () => {};
  return element;
}
const outsideTarget = testElement();
const timelineTarget = testElement('timeline');
const layersTarget = testElement('layers');
const canvasTarget = testElement('canvas');
const neutralTarget = testElement('neutral');
...
let activeElement: Element = timelineTarget;
const timelineTargetWithBlur = testElement('timeline');
timelineTargetWithBlur.blur = () => {
...
const editingLayersTarget = testElement('layers', 'text');
editingLayersTarget.closest = (selector: string) =>
  selector.startsWith('input') || selector === '[data-keyboard-context]'
    ? editingLayersTarget
    : null;
noteKeyboardContext({ target: editingLayersTarget });
eq('editing targets release explicit layer context', [
  keyboardContextOwns('layers', { target: layersTarget }),
], [false]);
eq('handled-delete-cannot-leak-into-the-other-panel-handler', [
  keyboardContextOwns('timeline', { target: timelineTarget, defaultPrevented: true }),
  keyboardContextOwns('layers', { target: layersTarget, defaultPrevented: true }),
], [false, false]);

eq('delete-contract-leaves-timeline-selection-to-the-canonical-planner', [
  keyboardDeleteAction('timeline', { activeLayerId: 9 }),
...
eq('playback-shortcut-requires-timeline-context', [
  isPlaybackShortcut({ key: 'k' }, false, 'timeline'),
  isPlaybackShortcut({ key: 'k' }, false, 'canvas'),
  isPlaybackShortcut({ key: 'k' }),
  isPlaybackShortcut({ key: 'K', shiftKey: true }, false, 'timeline'),
...
const editingControl = testElement();
editingControl.closest = () => editingControl;
eq('playback-shortcuts-yield-to-every-editing-control', [
  ...
  isEditingTarget(outsideTarget),
], [true, false, false]);

const rangeInput = testElement(null, 'range');
rangeInput.closest = (selector: string) => selector === 'input' ? rangeInput : null;
const textInput = testElement(null, 'text');
textInput.closest = (selector: string) => selector === 'input' ? textInput : null;
...
let selection: Parameters<typeof selectTimelineKey>[0] = {
  layerId: null, kind: null, frames: new Set<number>(),
};
let anchor: Parameters<typeof selectTimelineKey>[1] = null;
...
const motionState = clipTimelineState({
  tracks: [
    visualTimelineTrack({ id: 'motion-track' }),
    visualTimelineTrack({ id: 'locked-track', locked: true }),
  ],
...
const mixedMotion = planTimelineKeyMotion(motionState, clipTimelineSelection({
  ...
  propertyKeys: ([
    ...
    ['maskOpacity', 4],
    ['shapePath', 5],
  ] satisfies Array<[string, number]>).map(([propertyName, sourceTick]) => ({
    clipId: 'motion-clip', propertyName, sourceTick,
  })),
}), 99);
if (!mixedMotion.valid) throw new Error('Expected mixed key motion to be valid.');
...
const crossClipMotion = planTimelineKeyMotion(clipTimelineState({
  tracks: [...motionState.tracks, visualTimelineTrack({ id: 'offset-track' })],
  clips: [...motionState.clips, visualTimelineClip({
...
eq('unselected-destination-collision-rejects-the-complete-key-group',
  planTimelineKeyMotion(motionState, clipTimelineSelection({
...
eq('selected-key-vacates-a-destination-before-another-selected-key-enters-it',
  planTimelineKeyMotion({
    ...motionState,
    clips: [{
      ...requireValue(motionState.clips[0]),
      propertyTracks: { position: [{ tick: 3 }, { tick: 4 }] },
    }],
  }, clipTimelineSelection({
    propertyKeys: [
      { clipId: 'motion-clip', propertyName: 'position', sourceTick: 3 },
...
eq('opening-frame-motion-remains-valid-for-canonical-boundary-advance', {
  valid: planTimelineKeyMotion(motionState, clipTimelineSelection({
    frameKeys: [{ clipId: 'motion-clip', sourceTick: 2 }],
  }), 1).valid,
  destination: requireValue(planTimelineKeyMotion(motionState, clipTimelineSelection({
    frameKeys: [{ clipId: 'motion-clip', sourceTick: 2 }],
