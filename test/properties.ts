...
import { cmClone, cmKey, cmTranslate } from '../src/lib/cellmap.ts';
import { blendHex, nearest256 } from '../src/lib/color.ts';
import type {
  EditorCellMap,
  EditorLayer,
...
function compositeFixture(
  items: unknown[],
  viewport: Parameters<typeof G.compositeWorld>[1],
) {
  return G.compositeWorld(items as EditorLayer[], viewport);
}
...
function referenceBlend(foreground: string, background: string, alpha: number): string {
...
const XTERM_PALETTE: Array<{ index: number; rgb: [number, number, number] }> = [];
...
function referenceNearest256(hex: string): Array<{ index: number; hex: string }> {
  const source = parseHex(hex);
  let bestDistance = Infinity;
  let best: Array<{ index: number; rgb: [number, number, number] }> = [];
...
function randomCellMap(random: RandomSource, count: number): EditorCellMap {
...

{
  ...
  for (let run = 0; run < 16; run++) {
    ...
    const grouped = compositeFixture(stack, viewport);
    const flatten = (layer: typeof glyph | typeof background) => ({
      ...layer,
      groupId: null,
      offset: {
    ...
    check(
      ...
      sameValue(grouped, compositeFixture([
        flatten(glyph),
        flatten(background),
      ], viewport)),
      'run ' + run,
    ...
    check(
      ...
      sameValue(grouped, compositeFixture(shifted, {
        ...viewport,
        x: viewport.x + dx,
        y: viewport.y + dy,
      })),
    ...
    check(
      ...
      sameValue(grouped, compositeFixture([hiddenNoise, ...stack], viewport)),
      'run ' + run,
    );
    ...
    check(
      ...
      sameValue(
        compositeFixture([plainGlyph, plainBackground], viewport),
        compositeFixture([plainBackground, plainGlyph], viewport),
      ),
      'run ' + run,
    );
    check(
      ...
      sameValue(
        compositeFixture([{ ...plainGlyph, opacity: 0 }, plainBackground], viewport),
        compositeFixture([plainBackground], viewport),
      ),
      'run ' + run,
    );
  }
