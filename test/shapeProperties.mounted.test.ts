...
import { regularPolygonVertices, renderShapeToCells } from '../src/lib/shapes.ts';
import {
  activeChar,
  activeTool,
  dirty,
...
import { requireInstance, requireValue } from './types/ui-test-types.ts';
...
type TestShapeLayer = EditorShapeLayer & { shape: EditorShape };

function polygonShape(sides: number): EditorShape {
...
function mountPolygon(sides: number): void {
...
function polygonLayer(): TestShapeLayer {
  return requireValue(get(layers).find(
    (layer): layer is TestShapeLayer => layer.id === POLYGON_ID &&
      layer.type === 'shape' && layer.shape !== undefined,
  ));
}
...
function option(select: HTMLSelectElement, value: string): HTMLOptionElement {
  return requireValue([...select.options].find((candidate) => candidate.value === value));
}
...
describe('mounted polygon detail editing', () => {
  test.each([3, 5, 8])(
    ...
    async (sides) => {
      ...
      const detail = requireInstance(
        screen.getByRole('combobox', { name: 'Detail' }),
        HTMLSelectElement,
      );
      const style = requireInstance(
      ...
      expect(polygonLayer().shape.channel).toBe('glyph');
...
describe('mounted effect intensity animation', () => {
  test('exposes pressed identity and disables the complete intensity track', async () => {
    const effectLayer: EditorEffectLayer = {
      id: EFFECT_ID,
    ...
    const currentEffect = requireValue(get(layers).find(
      (layer): layer is EditorEffectLayer => layer.id === EFFECT_ID && layer.type === 'effect',
    )).effect;
    expect(requireValue(currentEffect).intensity).toBe(0.25);
  });
});
