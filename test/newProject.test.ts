...
} from '../src/lib/projectPresets.ts';
import {
...
import { replaceWithBlankProject } from '../src/lib/newProjectLifecycle.ts';
import {
  editorModalOpen,
...
import { mediaResourceManager } from '../src/lib/mediaRuntime.ts';
import {
  recentProjectIdentity,
  recentProjectRecord,
  recentProjects,
} from '../src/lib/recentProjects.ts';
import { moveState, selectMode } from '../src/lib/selection.ts';
import { activeTool, altEyedrop, colorDepth, dirty, fileName } from '../src/lib/stores.ts';
import { audioAssets, audioClips, createAudioTrack } from '../src/lib/audio.ts';
import { applyProjectSettings } from '../src/lib/projectSettings.ts';
import type { MediaRuntimeStatus } from '../src/lib/types/media-types.ts';
import type {
  ProjectLoadedDetail,
...
import { audioBuffer } from './types/timeline-media-test-types.ts';
...
function isSerializedTrack(value: unknown): value is SerializedTrack {
  if (!isUnknownRecord(value) || typeof value['id'] !== 'string') return false;
  const layer = value['layer'];
  return layer === undefined || (isUnknownRecord(layer) && typeof layer['id'] === 'string');
}

function isSerializedClip(value: unknown): value is SerializedClip {
  return isUnknownRecord(value) && typeof value['id'] === 'string' &&
    Array.isArray(value['frameKeys']);
}

function parseSerializedProject(contents: string): SerializedProject {
  const value: unknown = JSON.parse(contents);
  assert.ok(isUnknownRecord(value), 'project JSON must be an object');
  const timeline = value['timeline'];
  assert.ok(isUnknownRecord(timeline), 'project timeline must be an object');
  ...
  const projectFps = value['fps'];
  assert.ok(Array.isArray(tracks) && tracks.every(isSerializedTrack), 'project tracks must be valid');
  assert.ok(Array.isArray(clips) && clips.every(isSerializedClip), 'project clips must be valid');
  assert.ok(Array.isArray(tags), 'project tags must be an array');
  assert.equal(typeof projectId, 'string');
  assert.equal(typeof width, 'number');
  assert.equal(typeof height, 'number');
  assert.equal(typeof projectFps, 'number');
  if (typeof projectId !== 'string' || typeof width !== 'number' ||
    typeof height !== 'number' || typeof projectFps !== 'number') {
    throw new TypeError('project identity and dimensions must be typed');
  }
  return {
    projectId,
    width,
    height,
    fps: projectFps,
    timeline: { tracks, clips, tags },
    media: value['media'],
  };
}

const uuidByKind: Record<string, string> = {
...
const idsOf = (project: SerializedProject): Set<string> => new Set([
  project.projectId,
...
const invalidDrafts: ReadonlyArray<readonly [draft: unknown, pattern: RegExp]> = [
...
for (const [draft, pattern] of invalidDrafts) {
...
const firstBuiltInPreset = requireValue(BUILT_IN_PROJECT_PRESETS[0]);
assert.equal(firstBuiltInPreset.name, '80x24 · 24 fps');
assert.throws(() => Object.defineProperty(firstBuiltInPreset, 'columns', { value: 1 }), TypeError);

const defaultSettings: ProjectPresetSettings = normalizeProjectPresetSettings(null);
...
let settings: ProjectPresetSettings = defaultSettings;
...
assert.equal(requireValue(projectPresetById(settings, smallId)).name, 'Tiny');
...
let fallbackSettings: ProjectPresetSettings = defaultSettings;
...
assert.deepEqual(loadProjectPresetSettings(blockedStorage), defaultSettings,
...
} satisfies StorageLike), /quota/);

const colorDepthValues = new Map<string, string>();
const colorDepthStorage: StorageLike = {
  getItem(key: string) { return colorDepthValues.get(key) ?? null; },
  setItem(key: string, value: string) { colorDepthValues.set(key, value); },
};
...
assert.equal(loadColorDepthPreference(blockedStorage), 'truecolor',
...
assert.doesNotThrow(() => persistColorDepthPreference('256', {
  getItem() { return null; },
  setItem() { throw new Error('quota'); },
} satisfies StorageLike), 'a storage write failure must not break the current editor session');
...
const shortcut = (
  event: Parameters<typeof newProjectShortcutAction>[0],
  state: NonNullable<Parameters<typeof newProjectShortcutAction>[1]>,
) => newProjectShortcutAction(event, state);
...
let injectedRecent: string | null = 'recent-old';
...
const startupBlank = parseSerializedProject(serializeJSON());
assert.deepEqual({
...
assert.deepEqual(requireValue(startupBlank.timeline.clips[0]).frameKeys, [{ tick: 0, value: { cells: {} } }]);
...
const staleTarget: SaveTarget = {
  name: 'authored.paintty',
  durable: true,
  async write(_blob: Blob) { staleTargetWrites++; },
};
await saveJSON({
...
if (videoAsset.kind !== 'video') throw new Error('Expected a video asset');
...
if (audioAsset.kind !== 'audio') throw new Error('Expected an audio asset');
createAudioTrack({
  ...
  blob: audioBlob,
  buffer: audioBuffer(audioAsset.duration),
...
trimClip(requireValue(
  getClipTimelineState().clips.find((clip) => clip.kind === 'visual'),
).id, 'end', 2);
...
const movingLayer = requireValue(get(layers).find((layer) => layer.id === get(activeLayerId)));
moveState.set({
  mode: 'move',
  layerId: movingLayer.id,
  target: 'layer',
  layerType: movingLayer.type,
  hasCellSelection: true,
  offset: { x: 0, y: 0 },
  lifted: [],
  sourceKeys: new Set(['2,2']),
  sourceBounds: { x: 2, y: 2, w: 1, h: 1 },
...
mediaRuntimeStatus.set(new Map<string, MediaRuntimeStatus>([[
  'stale',
  { state: 'ready', hash: 'stale', generation: 0 },
]]));
const staleClip = requireValue(getClipTimelineState().clips[0]);
setClipSelection({ clipIds: [staleClip.id] });
dirty.set(true);

const dirtySnapshot = serializeJSON();
const oldIds = idsOf(parseSerializedProject(dirtySnapshot));
...
let recoveryBaseline: ProjectLoadedDetail | null = null;
...
const created = parseSerializedProject(serializeJSON());
...
assert.deepEqual(requireValue(created.timeline.clips[0]).frameKeys, [{ tick: 0, value: { cells: {} } }]);
...
assert.equal(get(activeLayerId), requireValue(get(layers)[0]).id);
...
await saveJSON({
  chooseTarget: async () => {
    ...
    return {
      name: 'new.paintty',
      durable: true,
      async write(_blob: Blob) { newTargetWrites++; },
    } satisfies SaveTarget;
  },
  checkpoint: async () => {},
  notifySaved: async () => {},
});
...
const storesSource = readSource('src/lib/stores.ts');
