...
import type { EditorGroupLayer, EditorVideoLayer } from '../src/lib/types/editor-domain.ts';
import type { ProjectLayer, VideoClip } from '../src/lib/types/project-types.ts';
import {
  errorStack,
  namedFile,
  requireValue,
...
assert.equal(videoRasterReadyAt({ state: 'ready', token: 1, ...readyIdentity }, readyIdentity), true);
assert.equal(videoRasterReadyAt(
  { state: 'ready', token: 1, ...readyIdentity, projectTick: 4 },
  readyIdentity,
), false);
function referenceVideoLayer(assetId: string, videoElement?: HTMLVideoElement): EditorVideoLayer {
  return {
    id: 'video',
    name: 'Video',
    type: 'video',
    ...
    ...(videoElement === undefined ? {} : { videoElement }),
  };
}
const referenceElement = Object.create(null) as HTMLVideoElement;
assert.deepEqual([
  videoReferenceState(referenceVideoLayer('source', referenceElement)),
  videoReferenceState(
    referenceVideoLayer('source', referenceElement),
    { state: 'error', clipId: 'video', assetId: 'source' },
  ),
  videoReferenceState(
    referenceVideoLayer('new-source', referenceElement),
    { state: 'error', clipId: 'video', assetId: 'old-source' },
  ),
  videoReferenceState(referenceVideoLayer('source')),
], ['ready', 'error', 'ready', 'missing']);
const collapsedGroup: EditorGroupLayer = {
  id: 'collapsed', name: 'Collapsed', type: 'group', visible: true, cells: {}, collapsed: true,
};
const expandedGroup: EditorGroupLayer = {
  id: 'expanded', name: 'Expanded', type: 'group', visible: true, cells: {}, collapsed: false,
};
assert.deepEqual(visibleVideoReferenceLayers([
  { ...referenceVideoLayer('source'), id: 'open', groupId: 'expanded' },
  { ...referenceVideoLayer('source'), id: 'hidden', groupId: 'collapsed' },
  collapsedGroup,
  expandedGroup,
...
class FakeVideo {
  ...
  readonly listeners = new Map<string, Array<{
    listener: EventListenerOrEventListenerObject;
    once: boolean;
  }>>();
  private _currentTime = 0;
  private _decodedTime = 0;
  seeking = false;
  private _src = '';
  preload = '';
  muted = false;
  ...
  constructor({
    duration = 4, width = 640, height = 360, readyState = 0,
  }: { duration?: number; width?: number; height?: number; readyState?: number } = {}) {
  ...
  addEventListener(
    ...
    entries.push({ listener, once: typeof options === 'object' && Boolean(options.once) });
...
interface RasterDraw { time: number; x: number; y: number; width: number; height: number }
class FakeRaster {
  width: number;
  height: number;
  readonly draws: RasterDraw[] = [];
  readonly context: CanvasRenderingContext2D;

  constructor(width = 1, height = 1) {
    this.width = width;
    this.height = height;
    this.context = Object.assign(Object.create(null) as CanvasRenderingContext2D, {
      drawImage: (video: FakeVideo, x: number, y: number, width: number, height: number) => {
...
function browserVideo(video: FakeVideo): FakeVideo & HTMLVideoElement {
  return Object.assign(video, Object.create(null) as HTMLVideoElement);
}

function browserRaster(raster: FakeRaster): FakeRaster & HTMLCanvasElement {
  return Object.assign(raster, Object.create(null) as HTMLCanvasElement);
}

let pendingVideo: FakeVideo | null = null;
const createdRasters: FakeRaster[] = [];
const createdUrls: Array<{ file: Blob | MediaSource; url: string }> = [];
const revokedUrls: string[] = [];

const fakeDocument = Object.create(null) as Document;
Object.defineProperty(fakeDocument, 'createElement', {
  value(tag: string) {
    if (tag === 'video') {
      assert.ok(pendingVideo, 'test must provide the next video element');
      const video = pendingVideo;
      pendingVideo = null;
      return browserVideo(video);
    }
    if (tag === 'canvas') {
      const raster = new FakeRaster(0, 0);
      createdRasters.push(raster);
      return browserRaster(raster);
    }
...
const fakeUrl = Object.assign(Object.create(null) as typeof URL, {
...
type TestVideoLayer = ProjectLayer & {
  type: 'video';
  videoElement: HTMLVideoElement | null;
  raster: HTMLCanvasElement | null;
  videoClip: VideoClip & { assetId: string };
};

function videoLayer(
  video: FakeVideo | null,
  raster: FakeRaster | null,
  clip: Partial<VideoClip> = {},
): TestVideoLayer {
  return {
    id: 'video',
    name: 'Video',
    ...
    visible: true,
    cells: {},
    videoElement: video ? browserVideo(video) : null,
    raster: raster ? browserRaster(raster) : null,
    videoClip: {
      assetId: 'source',
      startTick: 0,
      inPoint: 0,
...
function rasterStatus(layerId = 'video') {
  return requireValue(get(videoRasterStatus).get(layerId));
}

function editorVideoLayer(layer: TestVideoLayer): EditorVideoLayer {
  return {
    id: layer.id,
    name: layer.name,
    type: 'video',
    ...
    videoClip: {
      ...layer.videoClip,
      assetId: layer.videoClip.assetId,
      width: 1,
      height: 1,
    },
    ...(layer.videoElement ? { videoElement: layer.videoElement } : {}),
    ...(layer.raster ? { raster: layer.raster } : {}),
  };
}
...
await test('metadata load publishes the decoded video and correctly sized raster', async () => {
  const file = namedFile(new Uint8Array([1]), 'video/mp4', 'reference.mp4');
  ...
  requireValue(video.onloadedmetadata)();
...
await test('metadata with an initially unknown duration waits for the finite duration', async () => {
  ...
  pendingVideo = video;
  let resolved = false;
  const loading = loadVideoSource(
    namedFile(new Uint8Array([1]), 'video/webm', 'browser-recording.webm'),
  ).then((source) => {
    resolved = true;
    return source;
  });
  ...
  video.duration = 3.5;
  requireValue(video.ondurationchange)();
  const source = await loading;
...
await test('decode failure rejects and releases its object URL', async () => {
  ...
  const loading = loadVideoSource(namedFile(new Uint8Array([1]), 'video/quicktime', 'broken.mov'));
  requireValue(video.onerror)();
...
await test('frame sync waits for seek before publishing a resized raster', async () => {
  ...
  assert.equal(raster.draws.length, 0);
  assert.equal(get(videoFrameRevision), 0);
  const pending = rasterStatus();
...
await test('frame sync seeks every output tick inside one held frame', async () => {
  ...
  video.dispatch('seeked');
  await flushAsyncWork();
  const firstToken = rasterStatus().token;
...
await test('a superseded seek cannot replace the preview with its stale frame', async () => {
  ...
  const firstPending = rasterStatus();
  ...
  const secondPending = rasterStatus();
  ...
  assert.deepEqual(rasterStatus(), secondPending);
...
await test('rapid held-frame requests coalesce to the newest tick', async () => {
  ...
  const latestPending = rasterStatus();
  ...
  assert.deepEqual(rasterStatus(), latestPending);

  video.dispatch('seeked');
  await flushAsyncWork();
  assert.deepEqual(raster.draws.map((draw) => draw.time), [0.4]);
  assert.equal(videoRasterReadyAt(rasterStatus(), {
    clipId: 'video', assetId: 'source', projectTick: 2,
  }), true);
  assert.equal(requireValue(video.listeners.get('seeked')).length, 0);
});
...
await test('continuous playback publishes decodable intermediate frames instead of freezing', async () => {
  ...
  assert.equal(rasterStatus().projectTick, 1);
  assert.equal(rasterStatus().state, 'pending');
});
...
await test('adjacent 24 fps frames are not mistaken for the same decoded time', async () => {
  ...
  assert.ok(Math.abs(requireValue(video.currentTimeWrites[0]) - 1 / 24) < 1e-9);
  assert.equal(rasterStatus().state, 'pending');

  video.dispatch('seeked');
  await flushAsyncWork();
  assert.equal(raster.draws.length, 1);
  assert.ok(Math.abs(requireValue(raster.draws[0]).time - 1 / 24) < 1e-9);
});
...
await test('an inactive frame invalidates an older pending seek', async () => {
  ...
  assert.equal(raster.draws.length, 0);
  assert.equal(get(videoFrameRevision), 0);
  assert.deepEqual(rasterStatus(), inactive);
});
...
await test('an out-point change invalidates a ready raster at the same tick', async () => {
  ...
  assert.equal(rasterStatus().state, 'inactive');
});
...
await test('a seek watchdog releases the newest queued tick', async () => {
  ...
  let nextTimer = 1;
  const fakeSetTimeout = (callback: () => void): number => {
    const id = nextTimer++;
    timers.set(id, callback);
    return id;
  };
  const fakeClearTimeout = (id: unknown): void => { timers.delete(Number(id)); };
  ...
  try {
    ...
    syncVideoLayerFrames([layer], 1, 10);
    assert.deepEqual(video.currentTimeWrites, [0]);
    const [watchdogId, watchdog] = requireValue(timers.entries().next().value);
...
await test('hidden video references and references in hidden groups do not decode', async () => {
  ...
  const group: ProjectLayer = {
    id: 'hidden-group', name: 'Hidden group', type: 'group', visible: false, cells: {},
  };
  const grouped = {
...
await test('a hidden video decodes only while conversion explicitly requests its frame', async () => {
  ...
  try {
    ...
    assert.deepEqual(video.currentTimeWrites, [0]);
    assert.equal(rasterStatus(hidden.id).state, 'pending');
  } finally {
    release();
  }
...
await test('a failed decoded frame stays recoverable while hidden and clears when removed', async () => {
  ...
  assert.equal(canRelinkVideo(editorVideoLayer(layer), status), true);

  const hidden = { ...layer, visible: false };
  syncVideoLayerFrames([hidden], 0, 10);
  const hiddenStatus = rasterStatus();
  assert.equal(hiddenStatus.state, 'error');
  assert.equal(canRelinkVideo(editorVideoLayer(hidden), hiddenStatus), true);
...
await test('removing and immediately reusing a layer id rejects the old completion', async () => {
  ...
  const oldToken = rasterStatus().token;
  syncVideoLayerFrames([], 0, 10);
  syncVideoLayerFrames([videoLayer(newVideo, newRaster)], 0, 10);
  await flushAsyncWork();
  const newToken = rasterStatus().token;
  ...
  assert.equal(rasterStatus().token, newToken);
});
...
await test('missing video, missing raster, and inactive clips publish nothing', async () => {
  ...
  syncVideoLayerFrames([
    ...
    {
      id: 'glyph', name: 'Glyph', type: 'glyph', visible: true, cells: {},
      videoElement: browserVideo(unusedVideo), raster: browserRaster(untouchedRaster),
    },
  ], 0, 10);
