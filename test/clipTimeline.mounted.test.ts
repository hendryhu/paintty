...
import { dirty } from '../src/lib/stores.ts';
import {
  keyboardContextOwns,
  resetKeyboardContext,
} from '../src/lib/timelineKeys.ts';
import { projectMediaRegistry } from '../src/lib/mediaRegistry.ts';
import type {
  EditorCell,
  EditorCellLayer,
  EditorPoint,
...
interface MountedPropertyTracks extends TimelinePropertyTracks {
  position: Array<TimelineStoredKey<EditorPoint>>;
  visibility: Array<TimelineStoredKey<boolean>>;
  effectIntensity: Array<TimelineStoredKey<number>>;
  maskPosition: Array<TimelineStoredKey<EditorPoint>>;
  maskOpacity: Array<TimelineStoredKey<number>>;
  shapePath: Array<TimelineStoredKey<{ path: { kind: string } }>>;
}

interface MountedClip extends VisualTimelineClip {
  name?: string;
  frameKeys: Array<TimelineStoredKey<{ cells: Record<string, EditorCell> }>>;
  propertyTracks: MountedPropertyTracks;
}

interface MountedTimelineState extends ClipTimelineState {
  fps: number;
...
type FireEventTarget = Node | Element | Document | Window;

function cell(character: string): EditorCell {
  return { c: character, fg: '#ffffff', bg: null };
}

function timelineState(): MountedTimelineState {
...
function denseOpeningKeyState({ locked = false }: { locked?: boolean } = {}): MountedTimelineState {
  const state = timelineState();
  state.tracks[0].locked = locked;
  const clip = requireValue(state.clips[0]);
  clip.frameKeys = [
    { tick: 0, value: { cells: { '0,0': cell('A') } } },
...
function isMountedClip(clip: TimelineClip): clip is MountedClip {
  return clip.id === 'mounted-clip' && clip.kind === 'visual';
}

function currentClip(): MountedClip {
  return requireValue(getClipTimelineState().clips.find(isMountedClip));
}

function completeSelection(): ClipTimelineSelection {
  return {
...
function expectNoTimelineSelection(): void {
...
function setTimelineViewportRect(width = 640, height = 220): HTMLElement {
  const viewport = requireElement<HTMLElement>('.timeline-viewport');
  viewport.getBoundingClientRect = () => testRect(width, height);
  return viewport;
}

function wheelEvent(properties: WheelEventInit): Event {
  const event = new Event('wheel', { bubbles: true, cancelable: true });
  Object.defineProperties(event, {
    deltaY: { configurable: true, value: properties.deltaY ?? 0 },
    clientX: { configurable: true, value: properties.clientX ?? 0 },
    ctrlKey: { configurable: true, value: properties.ctrlKey ?? false },
    metaKey: { configurable: true, value: properties.metaKey ?? false },
  });
  return event;
}
...
async function selectThreeMixedKeys(): Promise<void> {
...
beforeAll(() => {
  ...
  vi.spyOn(HTMLCanvasElement.prototype, 'getContext').mockImplementation(() => testCanvasContext({
    clearRect: vi.fn(),
    fillRect: vi.fn(),
    beginPath: vi.fn(),
    moveTo: vi.fn(),
...
describe('mounted timeline background selection release', () => {
  test.each(['Delete', 'Backspace'])(
    ...
    async (key) => {
      ...
      const revision = get(authoredRevision);
      const root = screen.getByRole('application', { name: 'Clip timeline' });
      const background = requireElement<HTMLElement>('.timeline-surface');
  ...
  test('scrollbar, resizer, ruler, row, clip, key, tag, and editor targets are not blank', async () => {
    const unchangedTargets = [
      requireElement<HTMLElement>('.timeline-viewport'),
      screen.getByRole('separator', { name: 'Resize track headers' }),
      requireElement<HTMLElement>('.ruler-active-range'),
      requireElement<HTMLElement>('.timeline-row'),
...
describe('mounted sequence tag targets', () => {
  test('track lanes open Tag editing on the first and final active ticks only on release', async () => {
    ...
    const lane = requireElement<HTMLElement>('.track-lane');
  ...
  test('zero-track Tag mode exposes a global lane with exact drag, focus, cancel, and Undo', async () => {
    ...
    const ruler = requireElement<HTMLElement>('.ruler-active-range');
    const laneLeft = Number.parseFloat(lane.style.left);
    ...
    expect(getClipTimelineState().tags.map((tag) => {
      if (tag.type !== 'custom') throw new Error('Expected a custom tag');
      return { tick: tag.tick, type: tag.type, value: tag.value };
...
describe('mounted movable sequence tags', () => {
  test('Select drag moves the exact tag once and supports cancel, Undo, and Redo', async () => {
    ...
    expect(get(canUndo)).toBe(true);
    undo();
    expect(requireValue(getClipTimelineState().tags[0])).toEqual({
...
describe('mounted draggable frame and property keys', () => {
  async function selectMovableMixedKeys() {
    const targets: ReadonlyArray<readonly [name: string, ctrlKey: boolean]> = [
  ...
  test('mixed keys preview and move together in one recoverable history entry', async () => {
    ...
    expect(requireValue(getClipTimelineSelection().frameKeys[0]).sourceTick).toBe(2);
    redo();
    expect(currentClip().frameKeys.map((key) => key.tick)).toEqual([0, 3, 4]);
    expect(currentClip().propertyTracks.shapePath.map((key) => key.tick)).toEqual([3]);
  });
  ...
  test('collision preview is visibly invalid and commits no history', async () => {
    ...
    const preview = requireValue(screen.getAllByRole('button', { name: 'Position key at tick 4' })
...
describe('mounted Shift-drag clip duplication', () => {
  async function mountTwoClips() {
    ...
    const state = timelineState();
    state.clips.push({
      ...structuredClone(requireValue(state.clips[0])),
      id: 'second-clip',
      name: 'Second',
      startTick: 6,
    });
  ...
  test('captures Shift at pointer-down, renders a ghost, and commits one selectable copy', async () => {
    ...
    const ghost = requireElement<HTMLElement>('.timeline-clip.duplicate-ghost');
    ...
    const copied = requireValue(clips.find((clip) => clip.id !== 'mounted-clip'));
  ...
  test('Shift-dragging an unselected clip duplicates only that target', async () => {
    ...
    const clips = getClipTimelineState().clips;
    const copy = requireValue(clips.find(
      (clip) => clip.id !== 'mounted-clip' && clip.id !== 'second-clip',
    ));
...
describe('mounted timeline presentation and input geometry', () => {
  ...
  test('Ctrl/Cmd+wheel zooms around the pointer while ordinary and input wheel stay native', async () => {
    ...
    const surface = requireElement<HTMLElement>('.timeline-surface');
  ...
  test.each([
    ...
    await fireEvent.scroll(viewport);
    const surface = requireElement<HTMLElement>('.timeline-surface');
    const laneLeft = Number.parseFloat(requireElement<HTMLElement>('.track-lane').style.left);
  ...
  test('filmstrip mode expands and restores complete row geometry', async () => {
    const row = requireElement<HTMLElement>('.timeline-row');
    ...
    expect(requireElement<HTMLElement>('.clip-timeline').classList.contains('filmstrip-mode')).toBe(true);
    expect(requireValue(requireElement<HTMLElement>('.track-lane').parentElement).style.height).toBe('56px');
  ...
  test.each([4, 14, 48])(
    ...
    async (zoom) => {
      ...
      await tick();
      setTimelineViewportRect();
      const head = requireElement<HTMLElement>('.cti-head');
      const rulerLine = requireElement<HTMLElement>('.cti-ruler-line');
      const marker = screen.getByRole('button', { name: /Custom tag/ });
      const line = requireElement<HTMLElement>('.playhead-line');
      ...
      expect(requireValue(getClipTimelineState().tags[0]).tick).toBe(2);
  ...
  test.each([
    ...
    const keys = [...document.querySelectorAll<HTMLElement>('.frame-key, .property-key')];
    ...
    for (let first = 0; first < boxes.length; first++) {
      const firstBox = requireValue(boxes[first]);
      expect(firstBox.width).toBeGreaterThanOrEqual(4);
      expect(firstBox.height).toBeGreaterThanOrEqual(6);
      for (let second = first + 1; second < boxes.length; second++) {
        const secondBox = requireValue(boxes[second]);
        const overlaps = firstBox.left < secondBox.right &&
          secondBox.left < firstBox.right &&
          firstBox.top < secondBox.bottom &&
          secondBox.top < firstBox.bottom;
        expect(overlaps, `${firstBox.name} overlaps ${secondBox.name}`).toBe(false);
      }
    }
    expect(keys.filter((key) => key.getAttribute('aria-label')?.endsWith('tick 0') === true))
      .toHaveLength(4);
  });
  ...
  test.each([4, 14, 48])(
    ...
    async (zoom) => {
      ...
      try {
        ...
        const end = requireInstance(
          screen.getByRole('button', { name: 'Trim end of Mounted' }),
          HTMLButtonElement,
        );
  ...
  const directKeyCases: ReadonlyArray<readonly [
    name: string,
    target: 'frameKeys' | keyof MountedPropertyTracks,
  ]> = [
  ...
  test.each(directKeyCases)('%s can start its own direct drag', async (name, target) => {
    cleanup();
    loadCanonicalTimeline(denseOpeningKeyState());
    resetEditorStateForProjectLoad();
    render(ClipTimeline, { expanded: true, pixelsPerTick: 14 });
    ...
    const keys = requireValue(
      target === 'frameKeys' ? clip.frameKeys : clip.propertyTracks[target],
    );
  ...
  test('start-only loop band reaches the inclusive final sequence tick', async () => {
    ...
    render(ClipTimeline, { expanded: true, pixelsPerTick: 14 });
    await tick();
    const band = requireElement<HTMLElement>('.loop-range-band');
...
describe('mounted header and media empty states', () => {
  test('expanded controls end with Zoom, Loop, thumbnails, and Onion', async () => {
    cleanup();
    loadCanonicalTimeline(timelineState());
    resetEditorStateForProjectLoad();
    render(TimelineV2, { expanded: true });
    await tick();
    const headerChildren = [...requireElement<HTMLElement>('.timeline-header').children];
    const zoom = requireElement<HTMLElement>('.zoom-control');
    const loop = screen.getByRole('button', { name: /^Loop / });
    const thumbnails = screen.getByRole('button', { name: 'Show frame thumbnails' });
    const onion = screen.getByRole('button', { name: /^Onion:/ });
    expect(headerChildren.indexOf(zoom)).toBeLessThan(headerChildren.indexOf(loop));
    expect(headerChildren.indexOf(loop)).toBeLessThan(headerChildren.indexOf(thumbnails));
    expect(headerChildren.indexOf(thumbnails)).toBeLessThan(headerChildren.indexOf(onion));
    expect(headerChildren.at(-1)).toBe(onion);
    expect(zoom.title).toBe('Zoom');
    expect(screen.getByRole('button', { name: 'Zoom out' }).hasAttribute('title')).toBe(false);
    expect(screen.getByRole('button', { name: 'Zoom in' }).hasAttribute('title')).toBe(false);
  });
  ...
  test('Timeline zoom range keeps native navigation local while command keys bubble', async () => {
    ...
    const recordGlobalKey = (event: KeyboardEvent): void => { globalKeys.push(event.key); };
  ...
  test('onion Off, Layer, and All have distinct resting icon structures', async () => {
    ...
    expect(screen.queryByRole('button', { name: /frame thumbnails/ })).toBeNull();
  ...
  test('empty Project Assets names the state above the three import commands', async () => {
    ...
    const listener: EventListener = (event): void => {
      if (!(event instanceof CustomEvent)) throw new TypeError('Expected a custom import event');
      const detail: unknown = event.detail;
      if (!isUnknownRecord(detail) || typeof detail['kind'] !== 'string') {
        throw new TypeError('Expected an import kind');
      }
      requested.push(detail['kind']);
    };
