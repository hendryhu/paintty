...
import { notifyProjectReplaced } from '../src/lib/documentLifecycle.ts';
import type { NumberFieldDetail } from '../src/lib/types/canvas-components.ts';
import { requireInstance } from './types/ui-test-types.ts';
...
function outsideTarget(onPointerDown: EventListener | null = null): HTMLButtonElement {
...
describe('mounted number draft pointer policy', () => {
  test('same-field pointer preserves a draft and the next outside pointer sees one commit', async () => {
    ...
    render(NumberField, {
      ...
      onInput: ({ value }: NumberFieldDetail) => published.push(value),
      onChange: ({ value }: NumberFieldDetail) => changes.push(value),
    });
    const input = requireInstance(screen.getByRole('spinbutton', { name: 'Amount' }), HTMLInputElement);
  ...
  test('Escape and project replacement discard drafts before later pointers', async () => {
    ...
    const input = requireInstance(
      screen.getByRole('spinbutton', { name: 'Replace-safe amount' }),
      HTMLInputElement,
    );
    const outside = outsideTarget();
  ...
  test('Export cell-size preview updates on the first outside pointer-down', async () => {
    render(ExportPopup);
    await tick();
    const input = requireInstance(screen.getByRole('spinbutton', { name: 'Cell size' }), HTMLInputElement);
    await fireEvent.focus(input);
    await fireEvent.input(input, { target: { value: '23' } });
    expect(screen.getByText(/16px wide/)).not.toBeNull();

    await fireEvent.pointerDown(screen.getByRole('button', { name: /^Export$/ }), {
      button: 0,
      pointerId: 31,
    });
    await tick();
