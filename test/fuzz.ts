...
import * as S from '../src/lib/selection.ts';
import { applyTextColor, cutTextToBox, normalizeTextRuns, renderTextToCells, textOverflowsBox } from '../src/lib/textLayer.ts';
import { applyTool, paintSpecialBrushPath, visibleColorFromCell } from '../src/lib/tools.ts';
import { activeChar, activeTool, paintColor, toolOptions } from '../src/lib/stores.ts';
import {
  BOX_STYLES,
  renderShapeToCells as renderEditorShapeToCells,
} from '../src/lib/shapes.ts';
import { colorEditSession } from '../src/lib/colorEditSession.ts';
import { textSelection } from '../src/lib/textEditing.ts';
import { applyShapeBodyDrag, captureShapeBodyDrag } from '../src/lib/shapeBodyDrag.ts';
import { applyRasterBodyDrag, captureRasterBodyDrag } from '../src/lib/rasterBodyDrag.ts';
import type {
  EditorBounds,
  EditorEffectLayer,
  EditorImageLayer,
...
interface ShapeDragCase {
  drag: NonNullable<ReturnType<typeof captureShapeBodyDrag>>;
  frame: number;
  dx: number;
  dy: number;
  shape: EditorShape;
}
interface RasterDragCase {
  drag: NonNullable<ReturnType<typeof captureRasterBodyDrag>>;
  frame: number;
  dx: number;
  dy: number;
  transform: EditorImageLayer['transform'];
  keys: Array<{ frame: number; x: number; y: number }>;
}
type ColorFlow = 'preview-commit' | 'preview-cancel' | 'sample-finish' | 'sample-cancel' | 'abort';
type ColorCase =
  | { kind: 'toolbar'; flow: ColorFlow; target: { kind: 'toolbar' } }
  | { kind: 'shape'; flow: ColorFlow; target: { kind: 'shape'; layerId: string } }
  | {
      kind: 'text';
      flow: ColorFlow;
      target: {
        kind: 'text';
        layerId: string;
        selection: { layerId: string; start: number; end: number };
      };
    };
interface ShapePathCase {
  layerId: string;
  enabled: boolean;
  frame: number;
  path: unknown;
  ...
  delta: number;
  destination: number;
  payload: ReturnType<typeof F.copyShapePathKeys>;
}
...
function renderShapeToCells(shape: object) {
  return renderEditorShapeToCells(shape as EditorShape);
}

function addLayer(type: string): void {
  G.addLayer(type as Parameters<typeof G.addLayer>[0]);
}
...
function shapeLayer(id: string): EditorShapeLayer {
  return G.getLayer(id) as EditorShapeLayer;
}

function rasterLayer(id: string): EditorImageLayer | EditorVideoLayer {
  return G.getLayer(id) as EditorImageLayer | EditorVideoLayer;
}

function captureShapeDrag(layerId: string, frame = get(F.activeFrameIndex)) {
  return captureShapeBodyDrag(layerId, frame)!;
}

function captureRasterDrag(layerId: string) {
  return captureRasterBodyDrag(layerId)!;
}
...
const fuzzWide = (glyph: string) => glyph === '😀';
const specialGlyphs = new Set<string>(
  Object.values(BOX_STYLES).flatMap((style) => Object.values(style)),
);
const shapeKinds = new Set(['line', 'rect', 'circle']);
const interpolationPresets = new Set(['linear', 'ease-in', 'ease-out', 'ease-in-out']);
let colorSessionFailure: string | null = null;
const renderFuzzText = (
  text: string,
  box: EditorBounds,
  fg: string,
  wrap: boolean,
...
function shapeGeometryValid(shape: unknown): boolean {
  return isUnknownRecord(shape)
    && shapeKinds.has(String(shape['kind']))
    && [shape['x0'], shape['y0'], shape['x1'], shape['y1']].every(Number.isFinite);
}

function shapePathGeometryValid(path: unknown, kind: unknown): boolean {
  if (!isUnknownRecord(path) || path['kind'] !== kind || !shapeKinds.has(String(path['kind']))) return false;
  if (path['kind'] === 'line') {
...
function temporalEaseValid(ease: unknown): boolean {
  if (ease == null) return true;
  if (!isUnknownRecord(ease)) return false;
  return ['in', 'out'].every((side) => {
    const handle = ease[side];
    return handle == null || (
      isUnknownRecord(handle)
      && Number.isFinite(handle['time'])
      && Number.isFinite(handle['value'])
      && Number(handle['time']) >= 0
      && Number(handle['time']) <= 1
      && Number(handle['value']) >= 0
      && Number(handle['value']) <= 1
    );
  });
}
...
function firstDifference(expected: unknown, actual: unknown, path = '$'): string | null {
  ...
  if (Array.isArray(expected) && Array.isArray(actual)) {
    ...
    return null;
  } else if (isUnknownRecord(expected) && isUnknownRecord(actual)) {
...
function stateDifference(expected: string, actual: string): string | null {
...
function invariant(name: string, condition: boolean, detail: string): InvariantFailure | null {
...
function checkInvariants() {
        ...
        return {
          ...
          detail: layer.id + ': ' + JSON.stringify({
            ...
            frames: get(F.frames).map((frame) => {
              const textLayer = frame.layers.find(
  ...
  for (const frame of frameViews) {
    for (const layer of frame.layers.filter(
      (candidate): candidate is EditorShapeLayer =>
    ...
    if (row.shapePathTrackEnabled) {
      for (let frame = 0; frame < frameViews.length; frame++) {
        ...
        const resolvedShape = frameViews[frame]!.layers
          .find((layer): layer is EditorShapeLayer =>
            layer.id === row.id && layer.type === 'shape')?.shape;
...
function randomLayer<T extends EditorLayer>(
  random: RandomSource,
  predicate: (layer: EditorLayer) => layer is T,
): T | null;
function randomLayer(
  random: RandomSource,
  predicate?: (layer: EditorLayer) => boolean,
): EditorLayer | null;
function randomLayer(
  random: RandomSource,
  predicate: (layer: EditorLayer) => boolean = () => true,
): EditorLayer | null {
  const candidates = get(layers).filter(predicate);
  return candidates.length ? pick(random, candidates) : null;
}

function prepareShapePathLayer(random: RandomSource): EditorShapeLayer {
  ...
  if (!layer) {
    const kind = pick(random, ['line', 'rect', 'circle'] as const);
    ...
    layer = G.getLayer(id as string) as EditorShapeLayer;
  } else {
...
function mutateShapePath(
  path: unknown,
  random: RandomSource,
): Record<string, unknown> | null {
  if (!isUnknownRecord(path)) return null;
  const delta = 1 + Math.floor(random() * 4);
  if (path['kind'] === 'line') {
    const field = pick(random, ['x0', 'y0', 'x1', 'y1']);
    return { ...path, [field]: Number(path[field]) + (random() < 0.5 ? -delta : delta) };
  }
  const field = pick(random, ['cx', 'cy', 'w', 'h']);
  if (field === 'w' || field === 'h') return { ...path, [field]: Number(path[field]) + delta };
  return { ...path, [field]: Number(path[field]) + (random() < 0.5 ? -delta : delta) };
}

function offsetShapePath(
  path: unknown,
  amount: number,
): Record<string, unknown> | null {
  if (!isUnknownRecord(path)) return null;
  if (path['kind'] === 'line') {
    return {
      ...path,
      x1: Number(path['x1']) + amount,
      y1: Number(path['y1']) + Math.round(amount / 2),
    };
  }
  return {
    ...path,
    cx: Number(path['cx']) + amount,
    w: Number(path['w']) + Math.abs(amount),
  };
}

function ensureFrameCount(count: number): void {
  while (get(F.frames).length < count) F.addFrame();
}

function resetShapePathKeys(layerId: string): void {
  const keys = F.shapePathKeys(layerId).map((key) => key.frame);
  if (keys.length) F.deleteShapePathKeys(layerId, keys);
}

function prepareSelectedCell(random: RandomSource): boolean {
...
function prepareSelectedMaskCell(random: RandomSource): boolean {
...
function operationTable(random: RandomSource): FuzzOperation[] {
  const frameCount = () => get(F.frames).length;
  let colorCase: ColorCase = null!;
  let shapeDragCase: ShapeDragCase = null!;
  let shapePathCase: ShapePathCase = null!;
  let rasterDragCase: RasterDragCase = null!;
  let maskPositionLayerId: string = null!;
  return [
    {
      name: 'add layer',
      undoable: true,
      run() {
        addLayer(pick(random, ['cell', 'cell', 'background', 'text', 'shape', 'video', 'effect']));
      },
    },
      ...
      prepare() {
        ...
        G.layers.update((items) => items.map((layer) => (
          layer.id === sourceId
            ? {
                ...layer,
                raster: { width: 2, height: 2 } as unknown as EditorImageLayer['raster'],
              }
            : layer
        )));
        return true;
    ...
    },
    {
      ...
      prepare() {
        ...
        if (!dx && !dy) dx = 1;
        shapeDragCase = {
          drag: captureShapeDrag(layer.id),
          frame,
          dx,
          dy,
          shape: { ...shapeLayer(layer.id).shape! },
        };
      ...
      run() {
        ...
        G.endStroke();
        const layer = shapeLayer(shapeDragCase.drag.layerId);
    ...
    },
    {
      ...
      prepare() {
        let layer = randomLayer(
          random,
          (candidate): candidate is EditorImageLayer | EditorVideoLayer =>
            candidate.type === 'image' || candidate.type === 'video',
        );
        if (!layer) {
          addLayer(pick(random, ['image', 'video']));
          layer = rasterLayer(activeId());
        } else {
          G.selectLayer(layer.id);
        }
        ...
        if (!dx && !dy) dx = 1;
        rasterDragCase = {
          drag: captureRasterDrag(layer.id),
          frame,
          dx,
          dy,
      ...
      run() {
        ...
        G.endStroke();
        const layer = rasterLayer(rasterDragCase.drag.layerId);
    ...
    },
    {
      ...
      undoable: true,
      prepare() {
        const kind = pick(random, ['toolbar', 'shape', 'text'] as const);
        ...
        if (kind === 'shape') {
          ...
          if (!layer) {
            ...
            layer = get(layers).find(
              (candidate): candidate is EditorShapeLayer =>
                candidate.id === id && candidate.type === 'shape',
            )!;
          } else {
            G.selectLayer(layer.id);
          }
      ...
      run() {
        ...
        const fail = (detail: string) => { colorSessionFailure ||= detail; };
        if (!colorEditSession.open(colorCase.target, { x: 20, y: 30 })) {
          fail('picker did not open for ' + colorCase.kind);
          return;
        }
        if (colorCase.kind === 'text') {
          const layer = G.getLayer(colorCase.target.layerId) as EditorTextLayer;
        ...
        if (colorCase.flow.startsWith('sample')) {
          ...
          const layer = colorCase.kind === 'shape' ? shapeLayer(colorCase.target.layerId) : null;
          ...
          if (colorEditSession.sample(transparent as string)) fail('transparent sample changed the target');
    ...
    },
    {
      ...
      run() {
        ...
        const style = pick(random, ['single', 'rounded', 'double', 'heavy'] as const);
    ...
    },
    {
      ...
      prepare() {
        ...
        G.endStroke();
        shapePathCase = { layerId: layer.id, frames: [0], delta: 1 } as ShapePathCase;
        return true;
      },
      run() {
        F.moveShapePathKeys(
    ...
    },
    {
      ...
      prepare() {
        ...
        G.endStroke();
        shapePathCase = { layerId: layer.id, frames: [frame] } as ShapePathCase;
        return true;
      },
      run() {
    ...
    },
    {
      ...
      prepare() {
        ...
        shapePathCase = { layerId: layer.id, frames: [0, 3] } as ShapePathCase;
        return true;
      },
      run() {
    ...
    },
    {
      ...
      prepare() {
        let layer = randomLayer(
          random,
          (candidate): candidate is EditorEffectLayer =>
            candidate.type === 'effect' && !!candidate.mask,
        );
        if (!layer) {
          addLayer('effect');
          const created = G.getLayer(activeId());
          if (created?.type !== 'effect' || !created.mask) {
            throw new Error('New effect layer must include a mask.');
          }
          layer = created;
        }
...
function prepareOperation(operation: FuzzOperation): boolean {
...
const operationCoverage = new Map<string, { attempted: number; effective: number }>();

function availableOperations(random: RandomSource): FuzzOperation[] {
...
function coverageFor(name: string): { attempted: number; effective: number } {
...
function checkUndoRedo(seed: number) {
  const random = rng(seed);
  resetDocument();
