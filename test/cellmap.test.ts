import {
  ...
  cmFromGrid,
  cmToGrid,
} from '../src/lib/cellmap.ts';
import type { EditorCell, EditorCellMap, EditorCellValue } from '../src/lib/types/editor-domain.ts';
...
const cell = (c: string): EditorCell => ({ c, fg: '#fff', bg: null });
...
const seededMap: EditorCellMap = {};
const expected = new Map<string, { x: number; y: number; cell: EditorCell }>();
...
const normalizeEntries = (entries: Array<{ x: number; y: number; cell: EditorCellValue }>) => entries
