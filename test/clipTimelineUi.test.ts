...
} from '../src/lib/clipTimelineUi.ts';
import {
  clipTimelineState,
  audioTimelineClip,
  effectTimelineClip,
  ...
  timelineTrack,
  videoTimelineClip,
  visualTimelineClip as canonicalVisualClip,
  visualTimelineTrack,
...
function selectionState(): ClipTimelineState {
  return clipTimelineState({
    tracks: [
      visualTimelineTrack({ id: 'a' }),
      visualTimelineTrack({ id: 'b' }),
      { id: 'group', kind: 'group', locked: false },
    ],
    clips: [
      visualClip('a1', 'a', 0, 3),
      visualClip('a2', 'a', 6, 2),
...
test('clip context targets one, two, or three selected clips without discarding other selection kinds', () => {
  const state = selectionState();
  requireValue(state.clips[0]).propertyTracks = { position: [{ tick: 0 }] };
...
test('clip context excludes locked selections and disables a locked target', () => {
  const state = selectionState();
  requireValue(state.tracks.find((track) => track.id === 'b')).locked = true;
...
test('frame-key clicks form sparse ranges and replace clip context', () => {
  ...
  clip.frameKeys = [{ tick: 0 }, { tick: 2 }, { tick: 5 }];
  clip.outTick = 6;
...
test('frame and property hit zones stay disjoint at tick zero and adjacent ticks', () => {
  ...
  for (const { zoom, rowHeight } of [
    ...
    const boxes = layout.map((entry) => ({
      ...entry,
      left: entry.timelineTick * zoom + Number(entry['left']),
      right: entry.timelineTick * zoom + Number(entry['left']) + Number(entry['width']),
      top: Number(entry['top']),
      width: Number(entry['width']),
      height: Number(entry['height']),
      bottom: Number(entry['top']) + Number(entry['height']),
    }));
    for (let first = 0; first < boxes.length; first++) {
      const firstBox = requireValue(boxes[first]);
      assert.ok(firstBox.width >= 4, `zoom ${zoom} keeps a usable horizontal target`);
      assert.ok(firstBox.height >= 6, `row ${rowHeight} keeps a usable vertical target`);
      for (let second = first + 1; second < boxes.length; second++) {
        ...
        assert.equal(overlaps, false,
          `zoom ${zoom}, row ${rowHeight}: ${firstBox.kind} and ${secondBox.kind} overlap`);
      }
    }
...
test('clip trim targets stay inside a final one-tick clip at every zoom and DPR', () => {
  ...
  assert.equal(Math.max(...denseKeys.map((key) => Number(key['top']) + Number(key['height']))), trimTop,
...
test('one, two, and three mixed key selections survive Ctrl-click and context planning', () => {
  ...
  clip.frameKeys = [{ tick: 0 }, { tick: 2 }];
  clip.propertyTracks = Object.fromEntries([
    'position',
    'visibility',
    'effectIntensity',
    'maskPosition',
...
test('key context excludes locked keys and disables a locked target', () => {
  const state = selectionState();
  requireValue(state.tracks.find((track) => track.id === 'b')).locked = true;
  requireValue(state.clips[0]).frameKeys = [{ tick: 0 }, { tick: 2 }];
  requireValue(state.clips[2]).frameKeys = [{ tick: 0 }, { tick: 2 }];
  requireValue(state.clips[0]).propertyTracks = { position: [{ tick: 0 }] };
  requireValue(state.clips[2]).propertyTracks = { position: [{ tick: 0 }] };
  const selection = {
    frameKeys: [
      { clipId: 'a1', sourceTick: 0 },
...
test('gap planning excludes locked headers and never selects a locked hovered row alone', () => {
  const state = selectionState();
  requireValue(state.tracks[1]).locked = true;
...
function keyboardTarget(tagName: string, isContentEditable = false): EventTarget {
...
test('Delete and Backspace require live Timeline context and yield to editing controls', () => {
  ...
  const deletion = planTimelineDeleteKey(
    { key: 'Delete', target: keyboardTarget('DIV') },
    selection,
    { contextOwned: true },
  );
  ...
  for (const tagName of ['INPUT', 'TEXTAREA', 'SELECT']) {
    const native = planTimelineDeleteKey(
      { key: 'Backspace', target: keyboardTarget(tagName) },
      selection,
      { contextOwned: true },
    );
  ...
  assert.equal(planTimelineDeleteKey(
    { key: 'Delete', target: keyboardTarget('DIV', true) },
    selection,
    { contextOwned: true },
  ).handled, false);
  assert.equal(planTimelineDeleteKey(
  ...
  assert.equal(planTimelineDeleteKey(
    { key: 'Delete', defaultPrevented: true, target: keyboardTarget('DIV') },
    selection,
    { contextOwned: true },
  ).handled, false);
  assert.equal(planTimelineDeleteKey(
    { key: 'Backspace', target: keyboardTarget('DIV') },
    {},
    { contextOwned: true },
  ).kind, 'none');
});
...
test('Razor targets one hovered clip and distinguishes locked, edge, and gap points', () => {
  const state = clipTimelineState({
    tracks: [
      timelineTrack('a'),
      timelineTrack('b', 'visual', true),
      timelineTrack('c', 'effect'),
      timelineTrack('group', 'group'),
    ],
    clips: [
      visualClip('a1', 'a', 0, 5),
      visualClip('b1', 'b', 0, 5),
      effectTimelineClip({
        id: 'c1', trackId: 'c', startTick: 1, inTick: 0, outTick: 4, sourceDuration: 4,
      }),
    ],
  });
  ...
  const shiftedRazorOptions = {
    tick: 2,
    hoveredClipId: 'a1',
    shiftKey: true,
  };
  assert.equal(planRazorClick(state, shiftedRazorOptions).clipId, 'a1',
    'Shift does not widen a Razor click');
...
test('Select marquee intersects unlocked clips and sparse keys across rows', () => {
  ...
  requireValue(state.tracks[1]).locked = true;
  requireValue(state.clips[0]).frameKeys = [{ tick: 0 }, { tick: 2 }];
  requireValue(state.clips[0]).propertyTracks = { position: [{ tick: 1 }, { tick: 2 }] };
...
test('Razor path follows horizontal ticks and vertical rows without duplicate or locked cuts', () => {
  const state = clipTimelineState({
    tracks: [
      timelineTrack('a'),
      timelineTrack('b'),
      timelineTrack('locked', 'visual', true),
    ],
    clips: [
      visualClip('a1', 'a', 0, 6),
      visualClip('b1', 'b', 0, 6),
  ...
  assert.equal(requireValue(vertical.current).reason, 'locked');
...
test('an unambiguous Timeline target identifies the active layer and property part', () => {
  const state = selectionState();
  requireValue(state.tracks[0]).layer = {
    id: 'layer-a', name: 'Layer A', type: 'cell', visible: true, cells: {},
  };
  requireValue(state.tracks[1]).layer = {
    id: 'layer-b', name: 'Layer B', type: 'effect', visible: true, cells: {}, mask: { cells: {} },
  };
  requireValue(state.clips[2]).propertyTracks = { maskOpacity: [{ tick: 0 }] };
...
test('multi-clip move snaps once, clamps as a group, and orders collision-safe commits', () => {
  const state = clipTimelineState({
    tracks: [timelineTrack('a'), timelineTrack('b')],
    clips: [
  ...
  const before = structuredClone(state);
  const plan = requireValue(planClipMove(state, ['a', 'b'], 'a', 6.4, {
    playheadTick: 7,
    pixelsPerTick: 10,
  }));
  ...
  const bypassed = requireValue(planClipMove(state, ['a', 'b'], 'a', 6.4, {
    playheadTick: 7,
    pixelsPerTick: 10,
    altKey: true,
  }));
...
test('duplicate-move planning snaps a multi-selection and keeps originals as collision targets', () => {
  ...
  const before = structuredClone(state);
  const plan = requireValue(planClipDuplicateMove(state, ['a', 'b'], 'a', 2.6, {
    pixelsPerTick: 10,
  }));

  assert.equal(plan.valid, true);
  assert.equal(requireValue(plan.snapped).tick, 3);
  assert.equal(plan.deltaTicks, 3);
  assert.deepEqual(plan.operations, [
    { clipId: 'a', trackId: 'a', targetStartTick: 3 },
    { clipId: 'b', trackId: 'b', targetStartTick: 4 },
  ]);
  const overlap = requireValue(planClipDuplicateMove(state, ['a', 'b'], 'a', 1, {
    pixelsPerTick: 10,
    altKey: true,
  }));
  ...
  const unselectedTarget = requireValue(planClipDuplicateMove(state, ['a'], 'b', 3, {
    pixelsPerTick: 10,
    altKey: true,
  }));
...
test('duplicate-move planning reports negative and stationary overlap but permits audio overlap', () => {
  const state = clipTimelineState({
    tracks: [
      timelineTrack('visual'),
      timelineTrack('audio', 'audio'),
    ],
    clips: [
  ...
  const negative = requireValue(planClipDuplicateMove(state, ['source'], 'source', -1, {
    pixelsPerTick: 10,
    altKey: true,
  }));
  ...
  const blocked = requireValue(planClipDuplicateMove(state, ['source'], 'source', 5, {
    pixelsPerTick: 10,
    altKey: true,
  }));
  ...
  const audio = requireValue(planClipDuplicateMove(state, ['audio-source'], 'audio-source', 0, {
    pixelsPerTick: 10,
    altKey: true,
  }));
...
test('trim plans shared edges without changing source data and honors media bounds', () => {
  const state = clipTimelineState({
    tracks: [timelineTrack('a'), timelineTrack('b'), timelineTrack('video', 'video')],
    clips: [
      visualClip('a', 'a', 0, 4),
      ...
      videoTimelineClip({
        id: 'video', trackId: 'video', startTick: 10,
        inTick: 1, outTick: 5, sourceDuration: 6,
      }),
    ],
  });
  const before = structuredClone(state);
  const shared = requireValue(planClipTrim(state, ['a', 'b'], 'a', 'end', 9, {
    pixelsPerTick: 20,
    altKey: true,
  }));
  ...
  const media = requireValue(planClipTrim(state, ['video'], 'video', 'end', 99, {
    pixelsPerTick: 20,
    altKey: true,
  }));
...
test('trim-start cannot expose time before the earliest resolvable frame key', () => {
  ...
  const plan = requireValue(planClipTrim(
    state,
    ['opening-moved'],
    'opening-moved',
    'start',
...
test('audio adapters and edit plans preserve source media while changing timeline bounds', () => {
  ...
  assert.deepEqual(adaptAudioClipForTimeline(requireValue(clips[0]), 10), {
    id: 'audio:voice',
    audioClipId: 'voice',
  ...
  const moved = requireValue(planAudioClipMove(clips, ['voice', 'music'], 'voice', 0, {
    fps: 10,
    pixelsPerTick: 10,
    altKey: true,
  }));
  ...
  const start = requireValue(planAudioClipTrim(requireValue(clips[0]), 'start', 5, 10));
  ...
  const end = requireValue(planAudioClipTrim(requireValue(clips[0]), 'end', 60, 10));
...
test('canonical UI plans mixed visual and audio selection, move, Razor, and gap scope', () => {
  ...
  const move = requireValue(planClipMove(
    state,
