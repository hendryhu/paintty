...
import {
  ...
  shapeTransformCageVertices as shapeTransformCageVerticesForEditor,
  shapeTransformHandles as shapeTransformHandlesForEditor,
  shapeTransformQuantum as shapeTransformQuantumForEditor,
  snapShapeTransformPoint as snapShapeTransformPointForEditor,
  transformShapeFromCageHandle as transformShapeFromCageHandleForEditor,
  transformShapeFromHandle as transformShapeFromHandleForEditor,
  type ShapeTransformHandle,
} from '../src/lib/shapeTransform.ts';
import { resolvedShapeVertices as resolvedEditorShapeVertices } from '../src/lib/shapes.ts';
import type { EditorPoint, EditorShape } from '../src/lib/types/editor-domain.ts';

type ShapeFixture = object;
type TestShape = EditorShape & {
  vertices: EditorPoint[];
  anchor: EditorPoint;
  rotation: number;
  sides: number;
};
type TestPointArray = [EditorPoint, EditorPoint, EditorPoint, EditorPoint, ...EditorPoint[]];
type TestShapeHandles = Omit<ShapeTransformHandle[], 'find'> & {
  find(
    predicate: (value: ShapeTransformHandle, index: number, items: ShapeTransformHandle[]) => unknown,
  ): ShapeTransformHandle;
};

function shapeTransformCageVertices(shape: ShapeFixture): TestPointArray {
  return shapeTransformCageVerticesForEditor(shape as EditorShape) as TestPointArray;
}

function shapeTransformHandles(
  shape: ShapeFixture,
  options?: Parameters<typeof shapeTransformHandlesForEditor>[1],
): TestShapeHandles {
  return shapeTransformHandlesForEditor(shape as EditorShape, options) as TestShapeHandles;
}

function shapeTransformQuantum(shape: ShapeFixture): number {
  return shapeTransformQuantumForEditor(shape as EditorShape);
}

function snapShapeTransformPoint(shape: ShapeFixture, point: unknown): EditorPoint {
  return snapShapeTransformPointForEditor(shape as EditorShape, point)!;
}

function transformShapeFromHandle(
  shape: ShapeFixture,
  handleId: unknown,
  target: unknown,
  modifiers?: Parameters<typeof transformShapeFromHandleForEditor>[3],
): TestShape {
  return transformShapeFromHandleForEditor(
    shape as EditorShape,
    handleId,
    target,
    modifiers,
  ) as TestShape;
...
function transformShapeFromCageHandle(
  shape: ShapeFixture,
  handleId: unknown,
  target: unknown,
  modifiers?: Parameters<typeof transformShapeFromCageHandleForEditor>[3],
): TestShape {
  return transformShapeFromCageHandleForEditor(
    shape as EditorShape,
    handleId,
    target,
    modifiers,
...
function resolvedShapeVertices(shape: ShapeFixture): TestPointArray {
  return resolvedEditorShapeVertices(shape as EditorShape) as TestPointArray;
}
...
] as Array<[string, ShapeFixture]>;
type HandleType = ShapeTransformHandle['type'];
...
const handleTargetSize = (type: HandleType) => type === 'edge' || type === 'anchor' ? 18 : 20;
const containsPoint = (target: HandleTarget, point: EditorPoint) => point.x >= target.rect.left &&
  point.x <= target.rect.right && point.y >= target.rect.top && point.y <= target.rect.bottom;
const handleHitPriority: Record<HandleType, number> = { vertex: 0, edge: 1, anchor: 2, rotation: 3 };
function expectedHandleAt(targets: HandleTarget[], point: EditorPoint): string | null {
...
for (const [name, shape] of narrowHandleShapes) {
  for (const zoom of [8, 11, 14]) {
    for (const dpr of [1, 1.25, 2]) {
      ...
      const overlapSamples = new Map<string, EditorPoint>();
...
const coincidentTargets = (['vertex', 'edge', 'anchor', 'rotation'] as HandleType[]).map((type, stackOrder) => ({
  id: type,
  type,
  stackOrder,
...
function cageEdgeLengths(shape: ShapeFixture): number[] {
...
function screenCageEdgeLengths(shape: ShapeFixture, aspect: number): number[] {
  const cage = shapeTransformCageVertices(shape);
  return [
    Math.hypot((cage[1].x - cage[0].x) * aspect, cage[1].y - cage[0].y),
    Math.hypot((cage[2].x - cage[1].x) * aspect, cage[2].y - cage[1].y),
  ];
}

function screenDot(a: EditorPoint, b: EditorPoint, aspect: number): number {
  return a.x * aspect * b.x * aspect + a.y * b.y;
}

const diagonalLine = {
  kind: 'line', x0: 1, y0: 2, x1: 7, y1: 5, style: 'outline', detail: 'cell',
};
const lineAspect = 0.5;
const diagonalLineHandles = shapeTransformHandles(diagonalLine, {
  rotationAspect: lineAspect,
});
const diagonalLineAnchor = diagonalLineHandles.find(({ type }) => type === 'anchor');
const diagonalLineRotation = diagonalLineHandles.find(({ type }) => type === 'rotation');
const diagonalLineVertices = resolvedShapeVertices(diagonalLine);
assert.ok(Math.abs(screenDot({
  x: diagonalLineVertices[1].x - diagonalLineVertices[0].x,
  y: diagonalLineVertices[1].y - diagonalLineVertices[0].y,
}, {
  x: diagonalLineRotation.x - diagonalLineAnchor.x,
  y: diagonalLineRotation.y - diagonalLineAnchor.y,
}, lineAspect)) < 1e-9,
'a newly drawn diagonal line has a visually perpendicular rotation handle');
...
const handleCountCases: Array<[ShapeFixture, number, number]> = [
  [{ ...rect, kind: 'line' }, 2, 0],
...
for (const [shape, vertexCount, edgeCount] of handleCountCases) {
...
assert.equal(rotatedProportional.rotation, 45,
  'rotated proportional scaling preserves the authored angle');
assert.notDeepEqual(rotatedProportional.anchor, rotatedCompact.anchor,
  'the transform anchor follows a rotated resize instead of staying behind');
const rotatedOriginalOpposite = shapeTransformHandles(rotatedCompact)
  .find(({ id }) => id === 'vertex:0');
const rotatedResizedOpposite = shapeTransformHandles(rotatedProportional)
  .find(({ id }) => id === 'vertex:0');
assert.ok(
  Math.abs(rotatedOriginalOpposite.x - rotatedResizedOpposite.x) < 1e-9 &&
    Math.abs(rotatedOriginalOpposite.y - rotatedResizedOpposite.y) < 1e-9,
  'a rotated proportional resize keeps the opposite corner fixed in world space',
);
...
const terminalAspect = 0.5;
const screenRotationSource = { ...rect, anchor: { x: 2, y: 1 } };
const screenRotated = transformShapeFromHandle(
  screenRotationSource,
  'rotation',
  { x: 4, y: 1 },
  { rotationDelta: 90, rotationAspect: terminalAspect },
);
assert.deepEqual(
  screenCageEdgeLengths(screenRotationSource, terminalAspect)
    .map((value) => Number(value.toFixed(9))),
  screenCageEdgeLengths(screenRotated, terminalAspect)
    .map((value) => Number(value.toFixed(9))),
  'rotation preserves rendered aspect in rectangular terminal cells',
);
assert.equal(screenRotated.rotationAspect, terminalAspect,
  'the authored rotation records the canvas cell aspect');
const unsnappedRotation = transformShapeFromHandle(
  { ...screenRotationSource, rotation: 7 },
  'rotation',
  { x: 4, y: 1 },
  { rotationDelta: 11, rotationAspect: terminalAspect },
);
assert.equal(unsnappedRotation.rotation, 18,
  'ordinary rotation follows the pointer without quantizing its angle');
const snappedRotation = transformShapeFromHandle(
  { ...screenRotationSource, rotation: 7 },
  'rotation',
  { x: 4, y: 1 },
  { rotationDelta: 11, rotationAspect: terminalAspect, shift: true },
);
assert.equal(snappedRotation.rotation, 15,
  'Shift snaps the absolute authored rotation to fifteen-degree increments');
const snappedRotationAcrossZero = transformShapeFromHandle(
  { ...screenRotationSource, rotation: 350 },
  'rotation',
  { x: 4, y: 1 },
  { rotationDelta: 8, rotationAspect: terminalAspect, shift: true },
);
assert.equal(snappedRotationAcrossZero.rotation, 0,
  'Shift snapping normalizes the nearest full turn back to zero');

for (const kind of ['rect', 'circle', 'polygon']) {
  for (const rotation of [0, 35, 90, 173]) {
    const source = kind === 'polygon'
      ? {
        kind, style: 'outline', detail: 'cell', channel: 'glyph', rotation,
        rotationAspect: terminalAspect, anchor: { x: 4, y: 3 },
        vertices: [{ x: 1, y: 1 }, { x: 7, y: 2 }, { x: 6, y: 6 }, { x: 2, y: 7 }],
      }
      : {
        kind, style: 'outline', detail: 'cell', channel: 'glyph', rotation,
        rotationAspect: terminalAspect, anchor: { x: 4, y: 3 },
    ...
    const cage = shapeTransformCageVertices(source);
    const handle = shapeTransformHandles(source).find(({ id }) => id === 'rotation');
    const midpoint = {
      x: (cage[0].x + cage[1].x) / 2,
      y: (cage[0].y + cage[1].y) / 2,
    };
    const edge = { x: cage[1].x - cage[0].x, y: cage[1].y - cage[0].y };
    const stem = { x: handle.x - midpoint.x, y: handle.y - midpoint.y };
    assert.ok(Math.abs(screenDot(edge, stem, terminalAspect)) < 1e-9,
      `${kind} rotation handle remains screen-perpendicular at ${rotation} degrees`);
  }
}

const wrappedForward = transformShapeFromHandle(
  { ...screenRotationSource, rotation: 350 },
  'rotation',
  { x: 4, y: 1 },
  { rotationDelta: 20, rotationAspect: terminalAspect },
);
assert.equal(wrappedForward.rotation, 10, 'rotation wraps forward through zero');
const wrappedBackward = transformShapeFromHandle(
  { ...screenRotationSource, rotation: 10 },
  'rotation',
  { x: 4, y: 1 },
  { rotationDelta: -20, rotationAspect: terminalAspect },
);
assert.equal(wrappedBackward.rotation, 350, 'rotation wraps backward through zero');

const rotatedEdgeSource = {
  ...rect,
  x1: 8,
  y1: 3,
  rotation: 35,
  rotationAspect: terminalAspect,
  anchor: { x: 4, y: 1.5 },
};
const rotatedEdgeHandles = shapeTransformHandles(rotatedEdgeSource);
const draggedRotatedEdge = rotatedEdgeHandles.find(({ id }) => id === 'edge:1');
const fixedRotatedEdge = rotatedEdgeHandles.find(({ id }) => id === 'edge:3');
const rotatedEdgeResize = transformShapeFromCageHandle(
  rotatedEdgeSource,
  'edge:1',
  { x: draggedRotatedEdge.x + 2, y: draggedRotatedEdge.y },
);
const resizedRotatedHandles = shapeTransformHandles(rotatedEdgeResize);
const settledFixedEdge = resizedRotatedHandles.find(({ id }) => id === 'edge:3');
assert.notDeepEqual(rotatedEdgeResize.anchor, rotatedEdgeSource.anchor,
  'dragging a rotated side moves the transform anchor with the cage');
assert.ok(
  Math.abs(settledFixedEdge.x - fixedRotatedEdge.x) < 1e-9 &&
    Math.abs(settledFixedEdge.y - fixedRotatedEdge.y) < 1e-9,
  'dragging a rotated side leaves the opposite side fixed in world space',
);
assert.equal(rotatedEdgeResize.rotation, rotatedEdgeSource.rotation);
assert.equal(rotatedEdgeResize.rotationAspect, terminalAspect);
...
const runawayHandle = shapeTransformHandles(runawaySource)
  .find(({ id }) => id === 'vertex:2');
const boundedShiftCorner = transformShapeFromCageHandle(
  runawaySource,
  'vertex:2',
  { x: runawayHandle.x + 2.5, y: runawayHandle.y - 0.5 },
  { shift: true, rotationAspect: terminalAspect },
);
const boundedCage = shapeTransformCageVertices(boundedShiftCorner);
const boundedExtent = Math.max(...boundedCage.map((point) =>
  Math.hypot((point.x - runawaySource.anchor.x) * terminalAspect,
    point.y - runawaySource.anchor.y)));
assert.ok(boundedExtent < 10,
  'a small constrained corner drag cannot explode into a giant rotated cage');

const pivot = shapeTransformHandles(runawaySource).find(({ id }) => id === 'vertex:0');
const crossedShiftCorner = transformShapeFromCageHandle(
  runawaySource,
  'vertex:2',
  {
    x: pivot.x - (runawayHandle.x - pivot.x) * 0.25,
    y: pivot.y - (runawayHandle.y - pivot.y) * 0.25,
  },
  { shift: true, rotationAspect: terminalAspect },
);
const crossedHandle = shapeTransformHandles(crossedShiftCorner)
  .find(({ id }) => id === 'vertex:2');
const originalVector = {
  x: (runawayHandle.x - pivot.x) * terminalAspect,
  y: runawayHandle.y - pivot.y,
};
const crossedVector = {
  x: (crossedHandle.x - pivot.x) * terminalAspect,
  y: crossedHandle.y - pivot.y,
};
assert.ok(originalVector.x * crossedVector.x + originalVector.y * crossedVector.y > 0,
  'crossing the pivot cannot invert a Shift-constrained corner');
...
const ellipseEdgeGestures: Array<[
  string,
  EditorPoint,
...
const ellipseResizeDeltas = new Map<string, EditorPoint>([
  ['vertex:0', { x: -3, y: -2 }],
  ['vertex:1', { x: 3, y: -2 }],
  ['vertex:2', { x: 3, y: 2 }],
  ['vertex:3', { x: -3, y: 2 }],
...
const oppositeEllipseHandle = new Map<string, string>([
...
function localEllipseGeometry(shape: TestShape, origin: EditorPoint) {
  return {
    bounds: [
...
assert.equal(
  ...
  0,
  'a complete turn normalizes back to zero degrees',
);
...
function polygonArea(vertices: EditorPoint[]): number {
...
function rotatedDelta(delta: EditorPoint, degrees: number): EditorPoint {
...
function polygonForEdgeOracle(sides: number, translation: EditorPoint = { x: 0, y: 0 }) {
...
function changedVertexIndices(before: EditorPoint[], after: EditorPoint[]): number[] {
  ...
  for (const translation of [{ x: 0, y: 0 }, { x: 9, y: -6 }]) {
    ...
    for (const authoredDelta of [{ x: 3, y: -2 }, { x: -3, y: 2 }]) {
      ...
      for (let edgeIndex = 0; edgeIndex < sides; edgeIndex++) {
        ...
        const beforeWorld = resolvedShapeVertices(polygon);
        const afterWorld = resolvedShapeVertices(edited);
        afterWorld.forEach((point, index) => {
          const expected = adjacent.includes(index)
            ? {
              x: beforeWorld[index]!.x + screenDelta.x,
              y: beforeWorld[index]!.y + screenDelta.y,
            }
            : beforeWorld[index];
...
const restrictedBeforeWorld = resolvedShapeVertices(restrictedPolygon);
const restrictedAfterWorld = resolvedShapeVertices(restrictedEdgeMove);
restrictedAfterWorld.forEach((point, index) => {
  const expected = [2, 3].includes(index)
    ? {
      x: restrictedBeforeWorld[index]!.x + restrictedDelta.x,
      y: restrictedBeforeWorld[index]!.y + restrictedDelta.y,
    }
    : restrictedBeforeWorld[index];
  assert.ok(Math.abs(point.x - expected!.x) < 1e-9);
  assert.ok(Math.abs(point.y - expected!.y) < 1e-9);
});
