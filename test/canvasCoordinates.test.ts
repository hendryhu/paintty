...
import { canvasCoordinates } from '../src/lib/canvasCoordinates.ts';
import { compositeWorld } from '../src/lib/grid.ts';
import type { CanvasBackingLayout } from '../src/lib/types/canvas-components.ts';
import type {
  EditorBounds,
  EditorLayer,
  EditorPoint,
...
const moduleScript = canvasSource.match(
  /<script\b(?=[^>]*(?:\bmodule\b|context=["']module["']))[^>]*>([\s\S]*?)<\/script>/i,
);
assert.ok(moduleScript, 'Canvas must expose a pure module section for geometry tests');
const helperSource = `${moduleScript[1]}
  export { CANVAS_OVERSCAN, visibleCanvasViewport, viewportLocalPoint, sizeCanvasBacking, layoutViewportCanvas, canvasWithStableDimensions };
`;
const compiledHelperSource = ts.transpileModule(helperSource, {
  compilerOptions: {
...
interface InstrumentedCanvas {
  dataset: {
    widthAssignments?: string;
    heightAssignments?: string;
    backingResizes?: string;
  };
  style: {
    width?: string;
    height?: string;
...
interface CanvasGeometryHelpers {
  CANVAS_OVERSCAN: number;
  ...
  viewportLocalPoint(point: EditorPoint, viewport: EditorBounds): EditorPoint;
  sizeCanvasBacking(
    canvas: InstrumentedCanvas,
    width: number,
    height: number,
    pixelRatio: number,
  ): CanvasBackingLayout;
  canvasWithStableDimensions(canvas: InstrumentedCanvas): InstrumentedCanvas;
}

const helpers = await import(
  `data:text/javascript;base64,${Buffer.from(compiledHelperSource).toString('base64')}`
) as unknown as CanvasGeometryHelpers;
const {
  CANVAS_OVERSCAN,
  visibleCanvasViewport,
  viewportLocalPoint,
...
assert.deepEqual(
  viewportLocalPoint({ x: -3.5, y: 7 }, { x: -10, y: 2, w: 80, h: 24 }),
  { x: 6.5, y: 5 },
  'world coordinates translate into the bounded viewport backing canvas',
);
assert.match(canvasSource, /layoutViewportCanvas\(\s*imageCanvasEl,\s*vp,/,
  'reference media uses the visible viewport canvas instead of a document-clipped backing');
assert.match(canvasSource, /viewportLocalPoint\(\{ x: x \+ fx, y: y \+ fy \}, vp\)/,
  'raster sampling translates document cells through the viewport origin');
assert.ok(
  canvasSource.indexOf('<div class="grid-overlay"') < canvasSource.indexOf('<canvas bind:this={imageCanvasEl}'),
  'the editor grid is composited before reference media and terminal artwork',
);
assert.match(canvasSource, /\.grid-overlay\s*\{[^}]*z-index:\s*0;/,
  'the editor grid stays below opaque artwork instead of cutting through painted cells');

function assertViewportCoverage(
  viewportSize: EditorSize,
  documentSize: EditorSize,
  cellSize: EditorSize,
  pan: EditorPoint,
...
}] as unknown as EditorLayer[], distantViewport, { x: 0, y: 0, w: documentSize.w, h: documentSize.h });
assert.deepEqual(
...
function instrumentedCanvas(width = 300, height = 150): {
  ...
  const canvas: InstrumentedCanvas = {
    ...
    set width(value: number) { writes.width++; currentWidth = value; },
    get height() { return currentHeight; },
    set height(value: number) { writes.height++; currentHeight = value; },
  };
