...
import {
  ...
  renderTextToCells,
  textOverflowsBox,
} from '../src/lib/textLayer.ts';
import type { EditorBounds, EditorTextRun } from '../src/lib/types/editor-domain.ts';

const narrow = () => false;
const wideCjk = (glyph: string) => glyph === '界';
...
eq('Cut off overflow truncates both text and runs at the box height', {
  overflows: textOverflowsBox(cutSource, { w: 3, h: 1 } as EditorBounds, true, narrow),
  cut: cutTextToBox(cutSource, { w: 3, h: 1 } as EditorBounds, true, cutRuns, '#ffffff', narrow),
}, {
  overflows: true,
  cut: {
    text: 'ABC',
...
eq('Cut off overflow clips every visible no-wrap row and remaps later color runs', {
  overflows: textOverflowsBox(noWrapSource, { w: 3, h: 2 } as EditorBounds, false, wideCjk),
  cut: cutTextToBox(noWrapSource, { w: 3, h: 2 } as EditorBounds, false, noWrapRuns, '#ffffff', wideCjk),
}, {
  overflows: true,
  cut: {
    text: 'ABC\r\nFGH',
...
eq('horizontal overflow rejects a wide glyph that cannot fit atomically', {
  overflows: textOverflowsBox('A界B\nXY', { w: 2, h: 2 } as EditorBounds, false, wideCjk),
  cut: cutTextToBox('A界B\nXY', { w: 2, h: 2 } as EditorBounds, false, [], '#ffffff', wideCjk).text,
}, {
  overflows: true,
  cut: 'A\nXY',
});
...
function graphemes(text: string): Array<{ index: number; end: number }> {
  return [...graphemeSegmenter.segment(text)].map(({ segment, index }) => ({
    index,
    end: index + segment.length,
  }));
}
function runColorAt(runs: EditorTextRun[], index: number, base = '#ffffff'): string {
  return runs.find((run) => index >= run.start && index < run.end)?.fg || base;
}
function assertRunsValid(text: string, runs: EditorTextRun[], base = '#ffffff'): void {
...
function snappedRange(text: string, start: number, end: number): { from: number; to: number } {
...
const sentinelLayouts = [
  ...
  layoutText('A😀B', 3, true, (glyph: string) => glyph === '😀'),
];
...
const cutSentinels: Array<{
  text: string;
  box: EditorBounds;
  wide: (glyph: string) => boolean;
  expected: string;
}> = [
  { text: 'ABCDE', box: { w: 2, h: 2 } as EditorBounds, wide: narrow, expected: 'ABCD' },
  { text: '界A界B', box: { w: 3, h: 1 } as EditorBounds, wide: wideCjk, expected: '界A' },
  { text: 'A😀B', box: { w: 3, h: 1 } as EditorBounds, wide: (glyph) => glyph === '😀', expected: 'A😀' },
];
