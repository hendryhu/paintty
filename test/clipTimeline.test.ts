...
function stateWith(
  tracks: NonNullable<ClipTimelineStateDefinition['tracks']>,
  clips: NonNullable<ClipTimelineStateDefinition['clips']>,
): ClipTimelineState {
  return createClipTimelineState({ tracks, clips });
}

function clipById(state: ClipTimelineState, id: string): TimelineClip {
  return requireValue(state.clips.find((clip) => clip.id === id));
}

function frameTicks(clip: TimelineClip): number[] {
...
function malformedState(definition: object): ClipTimelineState {
  return Object.assign(Object.create(null) as ClipTimelineState, definition, { tags: [] });
}
...
test('visual clips normalize half-open source bounds and sparse generic tracks', () => {
  ...
  assert.equal(requireValue(requireValue(clip.frameKeys[1]).value as { pose?: string }).pose, 'last');
...
test('timeline constructors preserve extension discriminants', () => {
  ...
  assert.equal(track.kind, 'plugin-track');
  assert.equal(track['extensionValue'], 7);
  assert.equal(clip.kind, 'plugin-clip');
  assert.equal(clip['extensionValue'], 9);
});

test('deep cloning isolates plain data, maps, sets, typed arrays, and cycles', () => {
  interface CloneFixture {
    nested: { cells: Array<{ glyph: string }> };
  ...
  requireValue(copy.nested.cells[0]).glyph = 'B';
  requireValue(copy.map.get('key')).x = 9;
  copy.bytes[0] = 8;
  assert.equal(requireValue(value.nested.cells[0]).glyph, 'A');
  assert.equal(requireValue(value.map.get('key')).x, 1);
...
test('held frame and property resolution uses clip-local source ticks', () => {
  ...
  const key = requireValue(resolveHeldFrameKey(clip, 11));
  const propertyKey = requireValue(resolveClipPropertyKey(clip, 'position', 12));
  record(key.value)['pose'] = 'mutated read';
  record(propertyKey.value)['x'] = 99;
  assert.equal(record(requireValue(clip.frameKeys[1]).value)['pose'], 'B');
  assert.equal(record(requireValue(requireValue(clip.propertyTracks['position'])[1]).value)['x'], 4);
...
test('editing a held tick auto-keys a deep copy before applying the edit', () => {
  ...
  const result = expectChanged(editVisualFrame(original, 'visual', 6, (draft: unknown) => {
    const cells = record(draft)['cells'];
  ...
  assert.deepEqual(requireValue(edited.frameKeys[1]).value, { cells: [1, 2] });
  assert.deepEqual(requireValue(requireValue(original.clips[0]).frameKeys[0]).value, { cells: [1] });
  assert.notEqual(requireValue(edited.frameKeys[1]).value, requireValue(edited.frameKeys[0]).value);

  const exact = expectChanged(editVisualFrame(result.state, 'visual', 6, { cells: [9] }));
...
test('editing a gap creates exactly one one-tick visual clip and first frame', () => {
  ...
  const result = expectChanged(editVisualFrame(original, 'visual', 12, (draft: unknown) => {
    const cells = record(draft)['cells'];
    assert.ok(Array.isArray(cells));
    cells.push('X');
  }, {
  ...
  const clip = requireValue(result.state.clips[0]);
...
test('generic property edits auto-key held values inside their owning clip', () => {
  ...
  const result = expectChanged(editClipProperty(
    original,
    'clip',
    'position',
    6,
    (draft: unknown) => { record(draft)['x'] = 8; },
  ));
  const clip = clipById(result.state, 'clip');

  assert.equal(result.createdKey, true);
  assert.deepEqual(requireValue(clip.propertyTracks['position']).map((key) => key.tick), [0, 3]);
  assert.deepEqual(resolveClipProperty(clip, 'position', 6), { x: 8, y: 2 });
  assert.deepEqual(requireValue(original.clips[0]).propertyTracks['position'], [
...
test('visual trim reveals hidden keys and extends their final holds beyond the source', () => {
  ...
  assert.deepEqual(requireValue(trimmed.propertyTracks['opacity']).map((key) => key.tick), [0, 4]);
...
test('effect right edges extend held properties while media right edges stay source-bounded', () => {
  ...
  for (const kind of ['audio', 'video', 'media']) {
    ...
    const resized = expectChanged(resizeSelectedClipEdges(state, [kind], 'end', 1, 20));
...
test('move changes only startTick and rejects overlap atomically', () => {
  ...
  assert.deepEqual({
    ...
    outTick: 4,
    sourceDuration: 5,
    frameKeys: requireValue(original.clips[0]).frameKeys,
  });
  assert.equal(resolveHeldFrame(clip, 5), 'B');
  assert.equal(requireValue(original.clips[0]).startTick, 0);
...
test('clip duplication allocates fresh IDs, preserves payloads, and leaves originals untouched', () => {
  ...
  const result = expectChanged(duplicateTimelineClips(original, [{
  ...
  assert.notEqual(requireValue(copy.frameKeys[0]).value,
    requireValue(clipById(original, 'source').frameKeys[0]).value);
...
test('clip duplication rejects an invalid batch atomically while audio overlap remains valid', () => {
  ...
  const audioCopy = expectChanged(duplicateTimelineClips(audio, [{
  ...
  assert.equal(requireValue(audioCopy.state.clips[1]).assetId, 'asset');
...
test('Razor splits at an exact source boundary with deep-independent sources', () => {
  ...
  const split = expectChanged(razorSplitClip(original, 'source', 13, { rightClipId: 'right' }));
  ...
  assert.equal(requireValue(right.propertyTracks['position']).some((key) => key.tick === 4), true);
  assert.equal(left.frameKeys.some((key) => key.tick === 6), true);
  assert.equal(right.frameKeys.some((key) => key.tick === 0), true);

  const rightFrameValue = record(requireValue(right.frameKeys.find((key) => key.tick === 4)).value);
  const rightCells = rightFrameValue['cells'];
  assert.ok(Array.isArray(rightCells));
  record(rightCells[0])['c'] = 'R';
  record(requireValue(requireValue(right.propertyTracks['position']).find(
    (key) => key.tick === 4,
  )).value)['x'] = 99;
  const leftFrameValue = record(requireValue(left.frameKeys.find((key) => key.tick === 3)).value);
  const leftCells = leftFrameValue['cells'];
  assert.ok(Array.isArray(leftCells));
  assert.equal(record(leftCells[0])['c'], 'B');
  assert.equal(record(requireValue(requireValue(left.propertyTracks['position']).find(
    (key) => key.tick === 2,
  )).value)['x'], 2);
  const originalFrameValue = record(
    requireValue(requireValue(original.clips[0]).frameKeys[1]).value,
  );
  const originalCells = originalFrameValue['cells'];
  assert.ok(Array.isArray(originalCells));
  assert.equal(record(originalCells[0])['c'], 'B');
...
test('Razor rejects clip edges without consuming a deterministic id', () => {
  ...
  const valid = expectChanged(razorSplitClip(state, 'source', 3, { makeId }));
...
test('mixed-track Razor splits visual and audio clips with one project-tick boundary', () => {
  ...
  assert.equal(requireValue(audioPieces[0]).outPoint, 0.4);
  assert.equal(requireValue(audioPieces[1]).inPoint, 0.4);
  assert.equal(requireValue(audioPieces[1]).startTick, 3);
...
test('selected middle frame/property deletion reconnects preceding holds', () => {
  ...
  assert.deepEqual(frameTicks(requireValue(state.clips[0])), [0, 2, 4]);

  const clipDeleted = deleteTimelineSelection(keysDeleted.state, { clipIds: ['clip'] });
  assert.equal(record(clipDeleted)['removedClips'], 1);
...
test('deleting the first visible frame key advances the clip to create real blank time', () => {
  ...
  assert.deepEqual({
    ...
    removedKeys: record(deleted)['removedKeys'],
    removedClips: record(deleted)['removedClips'],
  ...
  assert.equal(requireValue(findClipAtTick(deleted.state, 'visual', 13)).id, 'clip');
...
test('video frame-key deletion advances its start and removes its final unresolved clip', () => {
  ...
  assert.equal(record(removed)['removedKeys'], 1);
  assert.equal(record(removed)['removedClips'], 1);
...
test('earliest and final property-key deletion preserves clip bounds and neighboring metadata', () => {
  ...
  assert.equal(record(earliest)['removedKeys'], 1);
  assert.equal(record(final)['removedKeys'], 1);
...
test('key deletion accepts timeline coordinates and no selection is a no-op', () => {
  ...
  const timelineFrame = Object.assign(
    Object.create(null) as { clipId: string; sourceTick: number },
    { clipId: 'clip', timelineTick: 7 },
  );
  const timelineSelection = { frameKeys: [timelineFrame] };
  const deleted = deleteTimelineSelection(state, timelineSelection);
...
test('selected clips and keys on locked tracks are not deleted', () => {
  ...
  assert.deepEqual(frameTicks(requireValue(keyDelete.state.clips[0])), [0, 1]);
});
...
test('mixed frame and property key motion is immutable, collision-safe, and preserves values', () => {
  ...
  const moved = expectChanged(moveTimelineKeys(state, clipTimelineSelection(selection), 1));
  const clip = clipById(moved.state, 'clip');
  ...
  assert.deepEqual(requireValue(clip.frameKeys.find((key) => key.tick === 4)).value, {
    pose: 'move',
  });
  assert.deepEqual(requireValue(requireValue(clip.propertyTracks['shapePath'])[0]).value, {
    path: { kind: 'line' },
  });
  assert.deepEqual(frameTicks(clipById(state, 'clip')), [1, 3, 6]);
  assert.deepEqual(moved.selection.frameKeys, [{ clipId: 'clip', sourceTick: 4 }]);

  const collision = moveTimelineKeys(state, clipTimelineSelection({
...
test('moving an opening frame advances clip source/project bounds without changing its end', () => {
  ...
  const moved = expectChanged(moveTimelineKeys(state, clipTimelineSelection({
...
test('ripple-delete shifts later clips equally on only selected track ids', () => {
  ...
  const ripple = expectChanged(rippleDeleteGap(state, ['a', 'b'], 3, 5));
...
test('selected visual and audio track gaps ripple together', () => {
  ...
  assert.deepEqual(record(result)['shiftedClipIds'], ['visual-later', 'audio-later']);
...
test('contextual audio Delete removes only selected clips and empty audio rows', () => {
  ...
  assert.equal(record(result)['removedClips'], 1);
  assert.deepEqual(record(result)['removedTrackIds'], ['audio']);
...
test('mixed multi-clip Delete leaves time in place and removes only newly empty audio rows', () => {
  ...
  assert.equal(record(mixed)['removedClips'], 3);
  assert.deepEqual(record(mixed)['removedTrackIds'], []);
  ...
  assert.deepEqual(record(finalAudio)['removedTrackIds'], ['audio']);
...
test('Delete ripple-closes a one-tick gap without collapsing either one-tick clip', () => {
  ...
  assert.equal(record(result)['deltaTicks'], -1);
...
test('global Add Empty Time moves later clips and splits crossing clips', () => {
  ...
  const result = expectChanged(addEmptyTime(original, 3, 2, { makeId }));
  ...
  record(requireValue(right.frameKeys.find((key) => key.tick === 3)).value)['pose'] = 'changed';
  assert.equal(record(requireValue(left.frameKeys.find((key) => key.tick === 2)).value)['pose'], 'B');
  assert.equal(record(requireValue(requireValue(original.clips[0]).frameKeys[1]).value)['pose'], 'B');
});
...
test('clip-local key shifts retime frame and every generic property track', () => {
  ...
  assert.deepEqual(requireValue(shifted.propertyTracks['position']).map((key) => key.tick), [1, 6]);
  assert.deepEqual(requireValue(shifted.propertyTracks['visibility']).map((key) => key.tick), [0]);
...
test('direct multi-selection edge resize touches only clips sharing the edge', () => {
  ...
  const resized = expectChanged(resizeSelectedClipEdges(original, ['a', 'b', 'c'], 'end', 4, 5));
...
test('shared edge resize applies one common clamp and keeps every clip one tick', () => {
  ...
  const resized = expectChanged(resizeSelectedClipEdges(state, ['a', 'b'], 'end', 4, -99));
  assert.equal(resized.deltaTicks, -2);
...
test('track and clip state helpers preserve ids, reject overlap, and cascade removal', () => {
  ...
  const trackAdded = expectChanged(addTimelineTrack(empty, { name: 'Layer' }, { makeId }));
  assert.equal(trackAdded.track.id, trackId);
  assert.equal(empty.tracks.length, 0);

  const clipAdded = expectChanged(addTimelineClip(trackAdded.state, {
    trackId,
    kind: 'visual',
    startTick: 2,
    inTick: 0,
  ...
  const afterRejected = expectChanged(addTimelineClip(overlap.state, {
    trackId,
    kind: 'visual',
    startTick: 8,
    inTick: 0,
  ...
  const groupAdded = expectChanged(addTimelineTrack(afterRejected.state, {
    kind: 'group',
  }, { makeId }));
  ...
  const updated = expectChanged(updateTimelineTrack(clipAdded.state, trackId, {
    id: 'ignored',
    name: 'Renamed',
    locked: true,
  }));
  ...
  const trackRemoved = expectChanged(removeTimelineTrack(clipAdded.state, trackId));
  assert.equal(trackRemoved.state.tracks.length, 0);
  assert.equal(trackRemoved.state.clips.length, 0);
  assert.equal(requireValue(trackRemoved.clips[0]).id, firstClipId);
});
...
test('every state transaction provides undo-friendly deep independence', () => {
  ...
  record(requireValue(moved.state.tracks[0])['metadata'])['color'] = 'blue';
  const movedValue = record(requireValue(clipById(moved.state, 'b1').frameKeys[0]).value);
  const movedCells = movedValue['cells'];
  assert.ok(Array.isArray(movedCells));
  record(movedCells[0])['c'] = 'X';

  assert.equal(record(requireValue(original.tracks[0])['metadata'])['color'], 'red');
  const originalBValue = record(requireValue(clipById(original, 'b1').frameKeys[0]).value);
  const originalBCells = originalBValue['cells'];
  assert.ok(Array.isArray(originalBCells));
  assert.equal(record(originalBCells[0])['c'], 'B');
  ...
  const clonedValue = record(requireValue(requireValue(cloned.clips[0]).frameKeys[0]).value);
  const clonedCells = clonedValue['cells'];
  assert.ok(Array.isArray(clonedCells));
  record(clonedCells[0])['c'] = 'Y';
  const originalAValue = record(requireValue(requireValue(original.clips[0]).frameKeys[0]).value);
  const originalACells = originalAValue['cells'];
  assert.ok(Array.isArray(originalACells));
  assert.equal(record(originalACells[0])['c'], 'A');
});
...
test('controller snapshots restore exactly and never expose mutable internal state', () => {
  ...
  const track = expectChanged(controller.addTrack({ name: 'Visual' })).track;
  const clip = expectChanged(controller.addVisualClip(track.id, {
    startTick: 0,
    inTick: 0,
    outTick: 3,
    sourceDuration: 3,
  ...
  controller.editVisualFrame(track.id, 1, (draft: unknown) => {
    const cells = record(draft)['cells'];
    assert.ok(Array.isArray(cells));
    cells[0] = 'B';
  });
  assert.deepEqual(frameTicks(requireValue(controller.getState().clips[0])), [0, 1]);
  assert.deepEqual(frameTicks(requireValue(snapshot.clips[0])), [0]);

  const exposed = controller.getState();
  requireValue(exposed.tracks[0]).name = 'Mutated outside';
  const exposedValue = record(requireValue(requireValue(exposed.clips[0]).frameKeys[0]).value);
  const exposedCells = exposedValue['cells'];
  assert.ok(Array.isArray(exposedCells));
  exposedCells[0] = 'X';
  assert.equal(requireValue(controller.getState().tracks[0]).name, 'Visual');
  const controllerValue = record(
    requireValue(requireValue(controller.getState().clips[0]).frameKeys[0]).value,
  );
  const controllerCells = controllerValue['cells'];
  assert.ok(Array.isArray(controllerCells));
  assert.equal(controllerCells[0], 'A');

  controller.restoreState(snapshot);
  assert.deepEqual(controller.getState(), snapshot);
  const split = expectChanged(controller.razorSplitClip(clip.id, 1));
  assert.equal(split.right.id, deterministicUuid('clip', 2));
  const splitValue = record(requireValue(requireValue(split.state.clips[0]).frameKeys[0]).value);
  const splitCells = splitValue['cells'];
  assert.ok(Array.isArray(splitCells));
  splitCells[0] = 'outside';
  const retainedValue = record(
    requireValue(requireValue(controller.getState().clips[0]).frameKeys[0]).value,
  );
  const retainedCells = retainedValue['cells'];
  assert.ok(Array.isArray(retainedCells));
  assert.equal(retainedCells[0], 'A');

  controller.restoreState(snapshot);
  const nextSplit = expectChanged(controller.razorSplitClip(clip.id, 2));
...
test('controller exposes the complete pure editing surface with deterministic results', () => {
  ...
  const trackA = expectChanged(controller.addTrack({})).track.id;
  const trackB = expectChanged(controller.addTrack({})).track.id;
  assert.deepEqual([trackA, trackB], [
    deterministicUuid('track', 5),
    deterministicUuid('track', 6),
  ]);

  const clipA = expectChanged(controller.addVisualClip(trackA, {
    startTick: 0, inTick: 0, outTick: 2, sourceDuration: 2,
    frameKeys: [{ tick: 0, value: 'A' }],
  })).clip.id;
  const clipB = expectChanged(controller.addVisualClip(trackB, {
    startTick: 0, inTick: 0, outTick: 2, sourceDuration: 2,
  ...
  assert.equal(requireValue(findContiguousGap(
    controller.getState(), [trackA, trackB], 2,
  )).durationTicks, 1);
...
test('state validation reports duplicate ids, missing tracks, invalid bounds, and overlap', () => {
  ...
  const errors = validateClipTimelineState(malformedState(invalid));
  ...
  assert.equal(validateClipTimelineState(malformedState(structural)).some(
...
test('canonical audio state strips runtime media from tracks, clips, and snapshots', () => {
  ...
  assert.equal('asset' in requireValue(state.clips[0]), false);
});
