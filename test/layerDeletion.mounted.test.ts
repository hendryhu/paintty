...
import { playing } from '../src/lib/frames.ts';
import {
...
import type { EditorLayer } from '../src/lib/types/editor-domain.ts';
import {
  requireInstance,
  requireValue,
  testCanvasContext,
...
type TestKeyboardContext = 'canvas' | 'timeline' | 'layers';

function canvasContext(): CanvasRenderingContext2D {
  return testCanvasContext({
    beginPath: vi.fn(),
    clearRect: vi.fn(),
    createImageData: testImageData,
    drawImage: vi.fn(),
    fillRect: vi.fn(),
    fillText: vi.fn(),
    lineTo: vi.fn(),
...
function row(name: string): HTMLElement {
  return requireValue(screen.getByText(name).closest<HTMLElement>('[data-layer-id]'));
}
...
function contextTarget(
  name: string,
  context: TestKeyboardContext | null = null,
): HTMLButtonElement {
...
async function activeRenameInput(name: string): Promise<HTMLInputElement> {
  await fireEvent.keyDown(window, { key: 'F2' });
  const input = requireInstance(await screen.findByDisplayValue(name), HTMLInputElement);
  input.focus();
  input.select();
  return input;
}
...
describe('mounted layer deletion', () => {
  test('startup keeps layer authoring disabled until recovery is ready', async () => {
    ...
    expect(requireInstance(screen.getByTitle('New layer'), HTMLButtonElement).disabled).toBe(true);
    expect(requireInstance(screen.getByTitle('New group'), HTMLButtonElement).disabled).toBe(true);

    await mounted.rerender({ disabled: false });
    expect(requireInstance(screen.getByTitle('New layer'), HTMLButtonElement).disabled).toBe(false);
    expect(requireInstance(screen.getByTitle('New group'), HTMLButtonElement).disabled).toBe(false);
  });
  ...
  test('retargets an unselected context row and deletes only that row', async () => {
    ...
    expect(screen.getByRole('menuitem', { name: 'Delete layer' })).not.toBeNull();
    expect([...get(selectedLayerIds)]).toEqual([requireValue(get(layers)[3]).id]);
  ...
  test('the plural context action deletes the frozen selected set', async () => {
    ...
    expect([...get(selectedLayerIds)]).toEqual([
      requireValue(get(layers)[0]).id,
      requireValue(get(layers)[2]).id,
    ]);
  });
  ...
  test('a group menu advertises and deletes its frozen eleven-row closure in one Undo', async () => {
    ...
    const groupedLayers: EditorLayer[] = [
      group,
      ...Array.from({ length: 10 }, (_, index): EditorLayer => ({
    ...
    const groupId = requireValue(get(layers)[0]).id;
    const survivorId = requireValue(get(layers).at(-1)).id;
    ...
    expect(requireValue(get(layers)[0]).id).toBe(groupId);
  ...
  test('context menu and rename typing suppress Ctrl+D, then Escape-close restores Layers ownership', async () => {
    ...
    await fireEvent.keyDown(window, { key: 'F2' });
    const input = requireInstance(await screen.findByDisplayValue('Gamma'), HTMLInputElement);
  ...
  ] satisfies ReadonlyArray<readonly [string, string, readonly string[], string]>;

  test.each(renameCases)('Enter restores Layers context for %s before immediate %s', async (
    _label, key, selectedNames, editedName,
  ) => {
    await fireEvent.click(row(requireValue(selectedNames[0])));
    ...
    await fireEvent.keyDown(requireValue(document.activeElement), { key });
    expect(layerNames()).toEqual(['Alpha', 'Beta', 'Gamma', 'Delta']
  ...
  test('Escape cancels rename, restores the edited row, and leaves no rename history entry', async () => {
    ...
    await fireEvent.keyDown(requireValue(document.activeElement), { key: 'Delete' });
    expect(layerNames()).toEqual(['Beta', 'Delta']);
    undo();
    await waitFor(() => expect(layerNames()).toEqual(['Alpha', 'Beta', 'Gamma', 'Delta']));
    expect(get(canUndo)).toBe(false);
  ...
  const blurCases = [
    ['Timeline', 'timeline'],
    ['popup', null],
  ] satisfies ReadonlyArray<readonly [string, TestKeyboardContext | null]>;

  test.each(blurCases)('blur commit to %s does not restore the edited layer row', async (name, context) => {
  ...
  test('project replacement abandons an in-progress rename without mutating the replacement', async () => {
    await fireEvent.click(row('Beta'));
    const id = requireValue(get(activeLayerId));
    const revision = get(authoredRevision);
    const input = await activeRenameInput('Beta');
  ...
  const neutralControlCases = [
    ['one selected layer', ['Alpha'], 'Delete', ['Beta', 'Gamma', 'Delta']],
    ['multiple selected layers', ['Alpha', 'Gamma'], 'Backspace', ['Beta', 'Delta']],
  ] satisfies ReadonlyArray<readonly [string, readonly string[], string, readonly string[]]>;

  test.each(neutralControlCases)('neutral controls preserve Layers deletion ownership for %s', async (
    _label, selectedNames, key, remaining,
  ) => {
    render(MenuBar);
    const timeline = render(TimelineV2, { expanded: false });
    await tick();
    await fireEvent.click(row(requireValue(selectedNames[0])));
    for (const name of selectedNames.slice(1)) {
      await fireEvent.click(row(name), { ctrlKey: true });
    }
    await timeline.rerender({ expanded: true });
    ...
    await fireEvent.keyDown(requireValue(controls.at(-2)), { key });
