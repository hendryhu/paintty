...
} from '../src/lib/textEditing.ts';
import type { EditorTextLayer } from '../src/lib/types/editor-domain.ts';
...
const layer = {
  ...
  fg: '#ffffff',
  runs: [],
} as unknown as EditorTextLayer;

let selection: ReturnType<typeof textSelectionAfterEvent> | null = null;
const keyup = (start: number, end: number): void => {
...
assert.deepEqual(
  textColorStateForSelection(colored, { layerId: layer.id, start: 0, end: 4 }),
  { color: '#c94f4f', mixed: false },
);
assert.deepEqual(
  textColorStateForSelection(colored, { layerId: layer.id, start: 0, end: 12 }),
  { color: '#c94f4f', mixed: true },
  'an uncolored gap keeps a same-colored pair of runs from appearing uniform',
);
assert.deepEqual(
  textColorStateForSelection(colored, {
    layerId: 7 as unknown as string,
    start: 0,
    end: 4,
  }),
