...
function roundPoint(value: object): Record<string, number> {
  return Object.fromEntries(Object.entries(value).map(([key, entry]) => {
    if (typeof entry !== 'number') throw new TypeError(`Expected ${key} to be numeric.`);
    return [key, +entry.toFixed(3)];
  }));
}
...
eq('legacy-segments-follow-authored-frame-spacing', {
  ...
  custom: requireValue(legacy[0]).custom,
  path: requireValue(legacy[0]).path,
}, {
  frames: [[0, 4]],
  custom: false,
...
eq('zero-inset-keeps-the-curve-on-the-full-lane',
  requireValue(temporalCurveSegments([
...
eq('legacy-ease-in-handles-show-its-real-endpoint-tangents', {
  outgoing: roundPoint(requireValue(legacyOutgoing).handle),
  incoming: roundPoint(requireValue(legacyIncoming).handle),
  outgoingControl: roundPoint(requireValue(legacyOutgoing).control),
  incomingControl: roundPoint(requireValue(legacyIncoming).control),
}, {
...
eq('dense-selected-track-keeps-only-the-two-terminal-sides-hidden', {
  ...
  first: { frame: requireValue(denseHandles[0]).frame, side: requireValue(denseHandles[0]).side },
  last: { frame: requireValue(denseHandles.at(-1)).frame, side: requireValue(denseHandles.at(-1)).side },
}, {
  count: 1438,
...
F.loadCanonicalTimeline(clipTimelineState({
  tracks: [{
    id: 'curve-track',
    kind: 'visual',
    locked: false,
    ...
    layer: {
      ...
      cells: {}, offset: { x: 0, y: 0 }, shape: undefined,
    },
  }],
  clips: [{
...
const presetEase = requireValue(middleEase()).temporalEase;
...
endStroke();
const customEase = requireValue(middleEase()).temporalEase;
undo();
eq('one-undo-restores-the-preset-before-a-grouped-handle-drag', {
  ease: requireValue(middleEase()).temporalEase,
