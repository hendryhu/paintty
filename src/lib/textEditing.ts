...
import type { EditorTextLayer, EditorTextRun } from './types/editor-domain.js';
...
interface ControlledHistoryEvent {
...
export const textSelection = writable<EditorTextSelection | null>(null);
...
export function createTextInputHistory(limit = 64) {
  ...
  const push = (stack: unknown[], state: unknown) => {
    stack.push(state);
    if (stack.length > capacity) stack.shift();
  };
  const step = (source: unknown[], target: unknown[], current: unknown): unknown | null => {
...
export function createControlledTextHistory(limit = 64) {
  ...
  return {
    ...
    beforeInput(event: Pick<ControlledHistoryEvent, 'inputType'>, state: unknown) {
    ...
    keydown<T>(event: ControlledHistoryEvent, current: T, restore: (state: T) => void) {
...
export function textSelectionAfterEvent(
  previous: EditorTextSelection | null,
  layerId: string,
  start: unknown,
  end: unknown,
  source = 'programmatic',
...
export function rememberTextSelection(layerId: string, start: unknown, end: unknown, source = 'programmatic'): void {
...
export function textSelectionForLayer(selection: EditorTextSelection | null, layerId: string | null | undefined): EditorTextSelection | null {
...
export function clearTextSelection(layerId: string | null = null): void {
  textSelection.update((selection) => (
    layerId == null || selection?.layerId === layerId ? null : selection
  ));
}

export function textColorStateForSelection(
  layer: EditorTextLayer | null | undefined,
  selection: EditorTextSelection | null | undefined,
  ...
  const length = String(textLayer.text || '').length;
  ...
  let cursor = start;
  const runs = [...(textLayer.runs || [])].sort((a, b) => a.start - b.start || a.end - b.end);
...
function runsEqual(a: readonly EditorTextRun[] | null | undefined, b: readonly EditorTextRun[] | null | undefined): boolean {
...
export function textColorPatchForSelection(
  layer: EditorTextLayer | null | undefined,
  selection: EditorTextSelection | null | undefined,
  color: string,
): { runs: EditorTextRun[]; fg?: string } | null {
