...
interface ParsedCellOptions {
  offset?: TimelinePoint;
  frameBounds?: TimelineBounds | null;
}
...
interface FrameThumbnailModelOptions {
  maxCells?: number;
  frameWidth?: number;
  frameHeight?: number;
  reference?: boolean;
  offset?: Partial<TimelinePoint>;
}

interface ThumbnailColors {
  emptySurface: string;
  emptyLine: string;
...
interface DrawThumbnailOptions {
  devicePixelRatio?: number;
  themeColors?: ThumbnailColors;
  backgroundChannel?: boolean;
  fontFamily?: string;
}

interface FrameThumbnailActionParams {
  model?: TimelineThumbnailModel | null;
  backgroundChannel?: boolean;
  fontFamily?: string;
}

interface FrameThumbnailDependencies {
  ResizeObserver?: typeof ResizeObserver;
  devicePixelRatio?: () => number;
  themeColors?: (node: HTMLCanvasElement) => ThumbnailColors;
}

interface RankedThumbnailSample {
  entry: TimelineThumbnailCellEntry;
  rank: {
    bin: number;
    contentRank: number;
    frequency: number;
...
function timelineCell(value: unknown): value is TimelineCell {
  return record(value);
}

function parsedCellEntry(
  key: string,
  cell: unknown,
  offset: TimelinePoint = { x: 0, y: 0 },
): TimelineThumbnailCellEntry | null {
  const match = /^(-?\d+),(-?\d+)$/.exec(key);
  if (!match || !timelineCell(cell)) return null;
...
function compareEntries(first: TimelineThumbnailCellEntry, second: TimelineThumbnailCellEntry): number {
  return first.y - second.y || first.x - second.x;
}

function gridDimensions(bounds: TimelineBounds, limit: number) {
...
function entrySignature(entry: TimelineThumbnailCellEntry): string {
  return [
    entry.cell.c || '',
    entry.cell.fg || '',
    entry.cell.bg || '',
...
function preferSample(candidate: RankedThumbnailSample, current: RankedThumbnailSample): boolean {
...
function parsedCellEntries(
  cells: Record<string, unknown> | null | undefined,
  limit: number,
  options: ParsedCellOptions = {},
): { entries: TimelineThumbnailCellEntry[]; bounds: TimelineBounds | null; truncated: boolean } {
  const sourceCells = cells || {};
  const exact: TimelineThumbnailCellEntry[] = [];
  let count = 0;
  ...
  for (const key in sourceCells) {
    if (!Object.prototype.hasOwnProperty.call(sourceCells, key)) continue;
    const entry = parsedCellEntry(key, sourceCells[key], options.offset);
    if (!entry) continue;
  ...
  const samples = new Map<number, RankedThumbnailSample>();
  for (const key in sourceCells) {
    if (!Object.prototype.hasOwnProperty.call(sourceCells, key)) continue;
    const entry = parsedCellEntry(key, sourceCells[key], options.offset);
    if (!entry) continue;
...
export function thumbnailFrameValue(
  clip: TimelineClip,
  segment: Pick<TimelineExposureSegment, 'keyIndex' | 'sourceTick'>,
): unknown {
...
export function buildFilmstripSamples(
  clip: TimelineClip,
  visibleRange: Partial<TimelineTickRange>,
...
export function buildFrameThumbnailModel(
  frameValue: unknown,
  options: FrameThumbnailModelOptions = {},
): TimelineThumbnailModel {
  const limit = Math.max(1, Math.min(
    MAX_MODEL_CELLS,
    Math.round(Number(options.maxCells)) || MAX_MODEL_CELLS,
  ));
  ...
  const cells = record(frameValue) && record(frameValue['cells'])
    ? frameValue['cells']
    : {};
  const { entries, bounds, truncated } = parsedCellEntries(cells, limit, {
    frameBounds,
    offset,
  });
  if (!entries.length) {
...
export function drawFrameThumbnail(
  node: HTMLCanvasElement,
  model: TimelineThumbnailModel | null | undefined,
  options: DrawThumbnailOptions = {},
) {
  ...
  if (!context) throw new TypeError('Timeline thumbnail requires a 2D canvas context.');
  ...
  const occupied = new Set(model.cells
  ...
  composeGlyphCoverage(context, drawGlyphs, () => {
    for (let index = model.cells.length - 1; index >= 0; index--) {
      const { x, y, cell } = model.cells[index]!;
  ...
  }, scale >= 5 ? () => redrawCellBoundaryCoverage(
      context,
      bounds.width,
      bounds.height,
      scale,
      ...
      offsetX,
      offsetY,
      (column, row) => occupied.has(`${bounds.x + column - 1},${bounds.y + row}`) &&
        occupied.has(`${bounds.x + column},${bounds.y + row}`),
      (column, row) => occupied.has(`${bounds.x + column},${bounds.y + row - 1}`) &&
        occupied.has(`${bounds.x + column},${bounds.y + row}`),
    ) : undefined);
  return { width, height, renderedCells: model.cells.length };
}

export function createFrameThumbnailAction(dependencies: FrameThumbnailDependencies = {}) {
  ...
  return function frameThumbnail(
    node: HTMLCanvasElement,
    params: FrameThumbnailActionParams | null | undefined,
  ) {
    let current: FrameThumbnailActionParams = params || {};
    ...
    const draw = () => drawFrameThumbnail(node, current.model, {
      ...(current.backgroundChannel === undefined
        ? {}
    ...
    draw();
    return {
      update(next: FrameThumbnailActionParams | null | undefined) {
        current = next || {};
        draw();
      },
      destroy() {
