...
import type { TextLayout } from './textLayer.js';

interface TextHitHelpers {
  boxOf(layer: EditorTextLayer): EditorBounds;
  isVisible(layer: EditorTextLayer): boolean;
  offsetOf?: ((layer: EditorTextLayer) => EditorPoint) | undefined;
}

interface TextGestureWithin {
  x?: unknown;
  y?: unknown;
  onGlyph?: boolean | undefined;
}
...
export interface TextGesture {
  ...
  fy1: number;
  layerId: string | null;
  startsOnGlyph: boolean;
  dragged: boolean;
}

function containsBox(layer: EditorTextLayer, x: number, y: number, helpers: TextHitHelpers): boolean {
  const box = helpers.boxOf(layer);
  return x >= box.x && x < box.x + box.w && y >= box.y && y < box.y + box.h;
}

export function textLayerHasGlyph(layer: EditorTextLayer | null | undefined, x: number, y: number, helpers: TextHitHelpers): boolean {
...
export function textLayerAt(
  layers: EditorLayer[],
  activeId: string | null,
  x: number,
  y: number,
  helpers: TextHitHelpers,
): EditorTextLayer | null {
  const isVisible = helpers.isVisible;
  const eligible = (layer: EditorLayer): layer is EditorTextLayer =>
    layer.type === 'text' && layer.box && isVisible(layer);
  const eligibleLayers = layers.filter(eligible);

  const active = eligibleLayers.find((layer) => layer.id === activeId);
  ...
  const glyphLayer = eligibleLayers.find((layer) => textLayerHasGlyph(layer, x, y, helpers));
  if (glyphLayer) return glyphLayer;

  if (active && containsBox(active, x, y, helpers)) return active;

  return eligibleLayers.find((layer) => containsBox(layer, x, y, helpers)) || null;
}

function pointFraction(value: unknown): number {
  const number = Number(value);
  return Number.isFinite(number) ? Math.max(0, Math.min(1, number)) : 0.5;
}

export function beginTextGesture(
  x: number,
  y: number,
  layerId: string | null = null,
  within: TextGestureWithin = {},
): TextGesture {
...
export function moveTextGesture(gesture: TextGesture | null, x: number, y: number, within: TextGestureWithin = {}): TextGesture | null {
...
export function textGestureBox(gesture: TextGesture | null): EditorBounds | null {
...
export function resolveTextGesture(gesture: TextGesture | null):
  { action: 'create'; box: EditorBounds } | { action: 'edit'; layerId: string } | null {
  if (!gesture) return null;
...
function textIndexAtPoint(
  layout: TextLayout,
  textLength: number,
  box: EditorBounds,
  point: EditorPoint & { fx: number; fy: number },
  tieDirection: 'forward' | 'backward',
): number {
...
export function textGestureSelection(
  layer: EditorTextLayer | null,
  box: EditorBounds | null,
  gesture: TextGesture | null,
  wideFn?: (glyph: string) => boolean,
): { start: number; end: number; direction: 'none' | 'forward' | 'backward' } | null {
