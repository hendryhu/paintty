...
interface VisualLeaseEntry {
  key: string;
  lease: MediaLease<MediaRuntime> | null;
  sequence: number;
}

interface AudioLeaseEntry {
  key: string;
  lease: MediaLease<MediaRuntime> | null;
}

function mediaGenerationKey(asset: MediaAsset): string {
  return `${asset.assetId}:${asset.hash}:${asset.generation}`;
}

function publishStatus(asset: MediaAsset, state: string, detail: unknown = null): void {
...
async function verifiedBlob(asset: MediaAsset): Promise<Blob> {
  ...
  return withMediaLease(asset.hash, async () => {
    try {
      ...
      if (!(error instanceof StaleMediaResourceError)) {
        ...
        publishStatus(asset, missing ? 'missing' : 'decode-failed', message);
      }
      throw error;
    }
  });
...
function disposeMedia(value: MediaRuntime, asset: MediaAsset): void {
  if (asset.kind === 'video') releaseVideoSource(value as VideoRuntime);
  else if (asset.kind === 'image') {
    const raster = (value as ImageRuntime).raster;
    if ('close' in raster && typeof raster.close === 'function') raster.close();
  }
}

export const mediaResourceManager = createMediaResourceManager<MediaAsset, MediaRuntime>({
  decode: decodeMedia,
...
export function acquireMediaResource(assetId: string): Promise<MediaLease<MediaRuntime>> {
...
const visualLeases = new Map<string, VisualLeaseEntry>();
const visualOwners = new Map<unknown, Set<string>>();
export const visualMediaRequestRevision = writable<number>(0);
let visualSequence = 0;

function visibleReferenceAssets(
  layerList: readonly ProjectLayer[],
...
function visualReferencesHaveRuntime(
  layerList: readonly ProjectLayer[],
  assetId: string,
  key: string,
): boolean {
...
export function syncVisibleMediaResources(
  layerList: readonly ProjectLayer[],
  requestedClipIds: ReadonlySet<string> = new Set(),
): void {
  ...
  for (const assetId of desired) {
    ...
    const entry: VisualLeaseEntry = { key, lease: null, sequence };
    ...
    acquireMediaResource(assetId).then((lease) => {
      ...
      const message = error instanceof Error ? error.message : String(error);
      if (!(error instanceof StaleMediaResourceError) && !/missing/i.test(message)) {
...
export function syncVisualMediaRequests(owner: unknown, assetIds: Iterable<string>): void {
...
export function releaseVisualMediaRequests(owner: unknown): void {
  if (!visualOwners.delete(owner)) return;
  visualMediaRequestRevision.update((revision) => revision + 1);
}

export function releaseVisibleMediaResources(): void {
  visualSequence++;
  for (const [assetId, entry] of visualLeases) {
    entry.lease?.release();
    setAssetRuntime(assetId, null);
...
const audioOwners = new Map<unknown, Set<string>>();
const audioLeases = new Map<string, AudioLeaseEntry>();

function requestedAudioIds(): Set<string> {
  const output = new Set<string>();
  for (const ids of audioOwners.values()) for (const id of ids) output.add(id);
  return output;
}

function reconcileAudioRequests(): void {
  ...
  for (const assetId of desired) {
    ...
    const entry: AudioLeaseEntry = { key, lease: null };
    ...
    acquireMediaResource(assetId).then((lease) => {
      ...
      entry.lease = lease;
      setAudioAssetRuntime(assetId, { ...(lease.value as AudioRuntime), runtimeMediaKey: key });
    }).catch((error) => {
...
export function syncAudioMediaRequests(owner: unknown, assetIds: Iterable<string>): void {
  audioOwners.set(owner, new Set(assetIds || []));
  reconcileAudioRequests();
}

export function releaseAudioMediaRequests(owner: unknown): void {
  audioOwners.delete(owner);
  reconcileAudioRequests();
}

export function activeMediaResourceHashes(): Set<string> {
