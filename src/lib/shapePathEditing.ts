...
interface ShapeAggregateMetrics {
  cx: number;
  cy: number;
  w: number;
  h: number;
...
type ShapeAggregateField = keyof ShapeAggregateMetrics;

function finite(value: unknown): value is number {
  return typeof value === 'number' && Number.isFinite(value);
}

function closeNumber(first: number, second: number): boolean {
  return Math.abs(first - second) < EPSILON;
}

function closePoints(first: EditorPoint[], second: EditorPoint[]): boolean {
...
function verticesWithUpdatedBounds(path: EditorShapePath, vertices: EditorPoint[]): EditorShapePath | null {
...
export function shapePathAggregateMetrics(value: unknown): ShapeAggregateMetrics | null {
...
function polygonSideCount(shape: EditorShape | null | undefined): number {
  return Math.max(3, Math.min(64,
    Math.round(Number(shape?.sides) || shape?.vertices?.length || 3)));
}

function clonePolygonShape(shape: EditorShape): EditorShape {
  return {
    ...shape,
...
function regularPolygonForMetrics(metrics: ShapeAggregateMetrics, sides: number): {
...
export function editPolygonSides(
  value: EditorShape | null | undefined,
  nextSides: unknown,
  gestureStart: EditorShape | null = null,
): EditorShape | null {
...
export function editShapePathAggregate(
  value: unknown,
  field: ShapeAggregateField,
  nextValue: unknown,
  ...
  if (baseline.kind === 'line') return null;
...
export function shapeForAnchorComponentEdit(
...
function wholeCell(value: unknown): boolean {
  return finite(value) && closeNumber(value, Math.round(value));
}

function wholeCellPoint(point: unknown): boolean {
  return !!point && typeof point === 'object' &&
    wholeCell((point as EditorPoint).x) && wholeCell((point as EditorPoint).y);
}

// Polygon detail changes only the rasterizer; authored vertices are never quantized.
export function canConvertShapeDetailToCell(shape: EditorShape | null, geometryAnimated = false): boolean {
...
export function shapeForStaticPathEdit(shape: EditorShape, path: unknown): EditorShape {
