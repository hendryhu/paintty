...
import type { MediaAsset, MediaRegistry, ProjectAssetRecord } from './types/media-types.js';
...
type ArchiveBlobEntry = Blob | ProjectAssetRecord;
type ArchiveBlobSource =
  | ReadonlyMap<string, ArchiveBlobEntry>
  | Record<string, ArchiveBlobEntry | undefined>
  | ((hash: string) => ArchiveBlobEntry | null | undefined | Promise<ArchiveBlobEntry | null | undefined>);

interface EncodeProjectArchiveOptions {
  document: string | UnknownRecord;
  mediaRegistry?: unknown;
  mediaBlobs?: ArchiveBlobSource;
  blobsByHash?: ArchiveBlobSource;
}

interface ProjectArchiveManifest extends UnknownRecord {
  format: string;
  version: number;
...
function blobBytes(value: Uint8Array): Uint8Array<ArrayBuffer> {
...
function blobEntry(
  source: ArchiveBlobSource | undefined,
  hash: string,
): ArchiveBlobEntry | null | undefined | Promise<ArchiveBlobEntry | null | undefined> {
  if (!source) return null;
  if (typeof source === 'function') return source(hash);
  if (source instanceof Map) return source.get(hash);
  return (source as Record<string, ArchiveBlobEntry | undefined>)[hash] || null;
}

function manifestAsset(asset: MediaAsset): UnknownRecord {
  return {
...
function sameManifestAsset(first: UnknownRecord, second: MediaAsset): boolean {
  return JSON.stringify({
    ...
    size: first['size'],
    mime: first['mime'],
  }) === JSON.stringify(manifestAsset(second));
}
...
export async function encodeProjectArchive({
  ...
  mediaBlobs,
  blobsByHash,
}: EncodeProjectArchiveOptions, output: 'blob' | 'uint8array' = 'blob'): Promise<Blob | Uint8Array> {
  const parsed: unknown = typeof document === 'string' ? JSON.parse(document) : document;
  if (!isUnknownRecord(parsed)) throw new TypeError('Project document must be an object.');
  ...
  for (const asset of unique.values()) {
    ...
    const data = await mediaBytes('blob' in blob ? blob.blob : blob);
...
export async function decodeProjectArchive(value: unknown): Promise<{
  document: UnknownRecord;
  ...
  const parsed: unknown = JSON.parse(strFromU8(projectEntry));
  if (!isUnknownRecord(parsed) || parsed['format'] !== PROJECT_PACKAGE_FORMAT ||
    parsed['version'] !== PROJECT_PACKAGE_VERSION || !isUnknownRecord(parsed['document'])) {
  ...
  const manifest: ProjectArchiveManifest = {
    ...
    assets: Array.isArray(parsed['assets'])
      ? parsed['assets'].filter(isUnknownRecord)
      : [],
  };
  const registry = normalizeMediaRegistry(manifest.document['media'], 'Project media registry');
  if (!Array.isArray(parsed['assets']) || manifest.assets.length !== parsed['assets'].length ||
  ...
  const declaredById = new Map<unknown, UnknownRecord>();
  ...
  for (const [hash, asset] of unique) {
    ...
    mediaBlobs.set(hash, new Blob([blobBytes(data)], {
