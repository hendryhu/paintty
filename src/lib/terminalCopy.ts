...
interface TerminalDialect {
  escape: string;
  reset: string;
  newline: string;
  escapeGlyph: (glyph: string) => string;
}
...
function visibleCell(cell: EditorCellValue): cell is EditorCell {
  return !!cell && !cell.cont && (!!cell.bg || (!!cell.c && cell.c !== ' '));
}

function lastVisibleColumn(row: EditorCellValue[]): number {
...
function styleOf(cell: EditorCell) : CellStyle {
...
function styleCodes(style: CellStyle, escape: string): string {
  let codes = '';
...
function renderRows(rows: EditorCellGrid, dialect: TerminalDialect) {
  let body = '';
...
const ANSI = {
  ...
  reset: '\x1b[0m',
  newline: '\n',
  escapeGlyph: (glyph: string) => glyph,
};
...
const BASH = {
  ...
  reset: '\\033[0m',
  newline: '\\n',
  escapeGlyph: (glyph: string) => glyph.replace(/\\/g, '\\\\'),
};
...
const POWERSHELL = {
  ...
  reset: '$e[0m',
  newline: '`n',
  escapeGlyph: (glyph: string) => glyph.replace(/[`"$]/g, (character: string) => '`' + character),
};

export function frameToAnsiText(rows: EditorCellGrid): string {
  return renderRows(rows, ANSI).body;
}

export function frameToBashCommand(rows: EditorCellGrid): string {
  const { body } = renderRows(rows, BASH);
  const safe = body.replace(/'/g, `'\\''`);
  return `printf '%b' '${safe}'`;
}

export function frameToPowerShellCommand(rows: EditorCellGrid): string {
