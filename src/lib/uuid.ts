...
export function assertUuid(value: unknown, label = 'UUID'): string {
  if (!isUuid(value)) throw new TypeError(`${label} must be an RFC 4122 UUID.`);
  return value;
}

export function uuidKey(value: unknown): string {
  return assertUuid(value).toLowerCase();
}

export function uuidFromCrypto(crypto: Crypto = globalThis.crypto): string {
...
type UuidGenerator = (kind?: string) => string;

let generateUuid: UuidGenerator = () => uuidFromCrypto();

export function newUuid(kind = 'entity'): string {
