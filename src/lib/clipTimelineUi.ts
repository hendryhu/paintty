...
import type { TimelineKeyboardEventLike, TimelineModifierState } from './types/timeline-ui.js';

interface PointerIntentOptions {
  surface?: 'ruler' | string;
  playing?: boolean;
  editable?: boolean;
}

interface TimelineSnapOptions extends TimelineModifierState {
  snapClips?: TimelineClipGeometry[];
...
interface FrameKeyAnchor extends TimelineFrameSelectionKey {}
interface PropertyKeyAnchor extends TimelinePropertySelectionKey {}

interface MarqueeOptions extends TimelineModifierState {
  startTick?: unknown;
  endTick?: unknown;
  trackIds?: TimelineIdSource;
}

interface RazorClickOptions {
  tick?: unknown;
  hoveredClipId?: unknown;
  hoveredTrackId?: unknown;
}

interface RazorPoint {
  tick: number;
  row: number;
}
...
function isClipTimelineSelection(value: unknown): value is ClipTimelineSelection {
  return value !== null && typeof value === 'object' &&
    'clipIds' in value && value.clipIds instanceof Set &&
    'frameKeys' in value && Array.isArray(value.frameKeys) &&
    'propertyKeys' in value && Array.isArray(value.propertyKeys) &&
    'trackHeaderIds' in value && value.trackHeaderIds instanceof Set;
}

function clipEndTick(clip: TimelineClip): number {
  return Number(clip?.startTick) + Math.max(1, Number(clip?.outTick) - Number(clip?.inTick));
}

function toggleModifier(modifiers: TimelineModifierState): boolean {
  return Boolean(modifiers?.ctrlKey || modifiers?.metaKey);
}

export function planTimelinePointerIntent(
  tool: unknown,
  tick: unknown,
...
function clearDirectTargets(selection: ClipTimelineSelection): ClipTimelineSelection {
...
function orderedClips(state: ClipTimelineState): TimelineClip[] {
...
function clipTrack(state: ClipTimelineState, clip: TimelineClip | null | undefined): TimelineTrack | null {
  return (state?.tracks || [])
    .find((track) => String(track.id) === String(clip?.trackId)) || null;
}

function clipIsEditable(state: ClipTimelineState, clip: TimelineClip | null | undefined): boolean {
  const track = clipTrack(state, clip);
  return Boolean(track && track.kind !== 'group' && !track.locked);
}

export function planClipContext(state: ClipTimelineState, selection: unknown, clipId: unknown) {
  const normalized = createClipTimelineSelection(selection, state);
...
function frameKeyIdentity(key: TimelineFrameSelectionKey): string {
  return `${String(key.clipId)}\u0000${Number(key.sourceTick)}`;
}

function propertyKeyIdentity(key: TimelinePropertySelectionKey): string {
  return `${String(key.clipId)}\u0000${String(key.propertyName)}\u0000${Number(key.sourceTick)}`;
}

export function timelinePropertyLabel(name: unknown): string {
...
export function planTimelineKeyContext(
  state: ClipTimelineState,
  selection: unknown,
  target: {
    clipId?: unknown;
  ...
  const frameKey: TimelineFrameSelectionKey = { clipId, sourceTick };
  const propertyKey: TimelinePropertySelectionKey = { clipId, sourceTick, propertyName };
  const targetSelected = kind === 'frame'
    ? normalized.frameKeys.some((entry) => frameKeyIdentity(entry) === frameKeyIdentity(frameKey))
    : normalized.propertyKeys.some((entry) =>
      propertyKeyIdentity(entry) === propertyKeyIdentity(propertyKey));
  const selectedFrameKeys = targetSelected
    ? normalized.frameKeys
    : kind === 'frame' ? [frameKey] : [];
...
export function planClipPropertyKeyMarkers(
  clip: TimelineClip,
  visibleRange: TimelineTickRange | null = null,
): PropertyMarker[] {
  ...
  for (let first = 0; first < markers.length;) {
    ...
    for (let index = first; index < last; index++) {
      ...
      if (!marker) continue;
      marker.stackIndex = index - first;
      marker.stackCount = stackCount;
    }
    first = last;
  }
  return markers;
...
export function planTimelineKeyMarkerLayout(
  frameMarkers: TimelineKeyMarker[] | null | undefined,
  propertyMarkers: TimelineKeyMarker[] | null | undefined,
  options: { pixelsPerTick?: unknown; rowHeight?: unknown } = {},
) {
  ...
  const entries: TimelineKeyMarker[] = [];
  for (const marker of frameMarkers || []) {
    if (Number.isFinite(Number(marker.timelineTick))) entries.push({ ...marker, kind: 'frame' });
  }
  for (const marker of propertyMarkers || []) {
    if (Number.isFinite(Number(marker.timelineTick))) entries.push({ ...marker, kind: 'property' });
  }
  ...
  for (let first = 0; first < entries.length;) {
    ...
    while (last < entries.length &&
      Number(entries[last]?.timelineTick) === Number(firstEntry.timelineTick)) last++;
...
export function planClipTrimHandleLayout(clipWidth: unknown, preferredHitWidth: unknown = 12) {
...
export function planFrameKeyClick(
  ...
  sourceTick: unknown,
  modifiers: TimelineModifierState = {},
  anchor: FrameKeyAnchor | null = null,
) {
  const normalized = createClipTimelineSelection(selection, state);
  ...
  if (modifiers.shiftKey && anchor && String(anchor.clipId) === target.clipId) {
...
export function planPropertyKeyClick(
  ...
  sourceTick: unknown,
  modifiers: TimelineModifierState = {},
  anchor: PropertyKeyAnchor | null = null,
) {
  const normalized = createClipTimelineSelection(selection, state);
  const clip = (state?.clips || []).find((candidate) => String(candidate.id) === String(clipId));
  ...
  const keys = Array.isArray(clip?.propertyTracks?.[name])
    ...
    : [];
  if (!clip || !name || !keys.some((key) => Number(key.tick) === tick)) {
    return { selection: normalized, anchor };
  }
  ...
  if (modifiers.shiftKey && anchoredTrack && anchor) {
    const lower = Math.min(Number(anchor.sourceTick), tick);
    const upper = Math.max(Number(anchor.sourceTick), tick);
    const range = keys
      .filter((key) => Number(key.tick) >= lower && Number(key.tick) <= upper)
...
export function planGapClick(
  state: ClipTimelineState,
  selection: unknown,
  hoveredTrackId: unknown,
  timelineTick: unknown,
  options: { maximumTick?: unknown } = {},
) {
  const normalized = createClipTimelineSelection(selection, state);
...
function projectTickForSource(clip: TimelineClip, sourceTick: number): number {
  return Number(clip.startTick) + Number(sourceTick) - Number(clip.inTick);
}

function tickIntersectsRange(tick: number, startTick: number, endTick: number): boolean {
  return tick >= startTick && tick <= endTick;
}

function clipIntersectsRange(clip: TimelineClip, startTick: number, endTick: number): boolean {
...
export function planTimelineMarquee(
  state: ClipTimelineState,
  selection: unknown,
  ...
  const hitClips: string[] = [];
  const hitFrameKeys: TimelineFrameSelectionKey[] = [];
  const hitPropertyKeys: TimelinePropertySelectionKey[] = [];
  ...
  const next = mode === 'replace'
    ? emptyClipTimelineSelection()
    : cloneClipTimelineSelection(normalized);
  const clipIds = mode === 'replace' ? new Set<string>() : new Set(next.clipIds);
...
export function timelineSelectionLayerTarget(state: ClipTimelineState, selection: unknown) {
  const normalized = createClipTimelineSelection(selection, state);
  ...
  const trackIds: Set<string> = directClipIds.size
  ...
  const onlyTrackId = [...trackIds][0];
  if (!onlyTrackId) return null;
  const track = tracks.get(onlyTrackId);
  ...
  const contentMaskOnly = propertyOnly && normalized.propertyKeys.every((key) =>
    key.propertyName === 'contentMask');
  return { layerId: String(layerId), part: maskOnly ? 'mask' : contentMaskOnly ? 'content-mask' : 'layer' };
}

export function planRazorClick(state: ClipTimelineState, options: RazorClickOptions = {}) {
...
function razorRowsForSegment(first: RazorPoint, second: RazorPoint): number[] {
...
function segmentTicksInRow(first: RazorPoint, second: RazorPoint, row: number): number[] {
...
export function planRazorDrag(
  state: ClipTimelineState,
  points: unknown,
  rowTracks: Array<{ trackId?: unknown; id?: unknown } | string | number> = [],
) {
  ...
  const rows = rowTracks.map((row) => typeof row === 'object'
    ? String(row.trackId ?? row.id ?? '')
    : String(row));
  const cuts: Array<{ clipId: string; trackId: string; tick: number; row: number }> = [];
  const seen = new Set<string>();
  const segments: Array<[RazorPoint, RazorPoint]> = [];
  if (path.length === 1 && path[0]) segments.push([path[0], path[0]]);
  else for (let index = 1; index < path.length; index++) {
  ...
  for (const [first, second] of segments) {
    for (const row of razorRowsForSegment(first, second)) {
      ...
      for (const tick of segmentTicksInRow(first, second, row)) {
        ...
        if (planned.kind !== 'razor-clip' || planned.tick == null ||
          planned.clipId == null || planned.trackId == null) continue;
  ...
  if (!last) return { kind: 'none', cuts: [], current: null };
...
export function planTimelineDelete(selection: unknown) {
  const source = isClipTimelineSelection(selection)
    ? cloneClipTimelineSelection(selection)
    : createClipTimelineSelection(selection || emptyClipTimelineSelection());
...
export function planTimelineDeleteKey(
  event: TimelineKeyboardEventLike,
  selection: unknown,
  options: { editing?: boolean; contextOwned?: boolean; playing?: boolean } = {},
) {
  const key = event?.key;
  const eventTarget = event.target && typeof event.target === 'object' ? event.target : null;
  const tagName = String(eventTarget && 'tagName' in eventTarget ? eventTarget.tagName : '').toUpperCase();
  const editing = options.editing ?? (
    ['INPUT', 'TEXTAREA', 'SELECT'].includes(tagName) ||
    Boolean(eventTarget && 'isContentEditable' in eventTarget && eventTarget.isContentEditable)
  );
...
export function planClipMove(
  ...
  draggedClipId: unknown,
  requestedStartTick: unknown,
...
function plannedClipsOverlap(first: TimelineClip, second: TimelineClip): boolean {
...
export function planClipDuplicateMove(
  ...
  const candidates = moving.flatMap((clip, index) => {
  ...
  if (!reason) {
    for (let first = 0; first < candidates.length; first++) {
      for (let second = first + 1; second < candidates.length; second++) {
        ...
        if (firstCandidate && secondCandidate && plannedClipsOverlap(firstCandidate, secondCandidate)) {
          reason = 'overlap';
          break;
        }
      }
...
function clipCanExtendHeldSource(clip: TimelineClip): boolean {
...
export function adaptAudioClipForTimeline(clip: AudioClipLike, fps: unknown) {
  return {
    ...
    inTick: 0,
    outTick: Math.max(1, audioClipDurationTicks(clip, Number(fps))),
    sourceDuration: Math.max(1, audioClipDurationTicks(clip, Number(fps))),
    frameKeys: [],
    kind: 'audio',
  };
}
