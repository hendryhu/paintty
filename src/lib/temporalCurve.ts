...
import type { TimelineCurveKey } from './types/timeline-ui.js';

interface TemporalCurveOptions {
  frameWidth?: number;
  height?: number;
  inset?: number;
  samples?: number;
...
interface TemporalCurveSegment {
  sourceFrame: number;
...
interface TemporalHandleGeometry {
  frame: number;
  side: TimelineTemporalSide;
  adjacentFrame: number;
  origin: TimelinePoint;
  control: TimelinePoint;
  handle: TimelineTemporalHandle;
  explicit: boolean;
}
...
function point(x: number, value: number, height: number, inset: number): TimelinePoint {
...
function pathFromPoints(points: TimelinePoint[]): string {
  return points.map((entry, index) =>
    `${index ? 'L' : 'M'}${format(entry.x)} ${format(entry.y)}`).join(' ');
}

function legacyHandle(interpolation: unknown, side: TimelineTemporalSide): TimelineTemporalHandle {
...
function optionsWithDefaults(options: TemporalCurveOptions = {}) {
...
export function orderedTemporalKeys(keys: unknown): TimelineCurveKey[] {
  return (Array.isArray(keys) ? keys : [])
    .filter((key): key is TimelineCurveKey => !!key && typeof key === 'object' &&
      Number.isInteger(Reflect.get(key, 'frame')))
    .slice()
    .sort((left, right) => left.frame - right.frame);
}

export function temporalCurveSegments(
  keys: unknown,
  options: TemporalCurveOptions = {},
  ...
  const segments: TemporalCurveSegment[] = [];
...
function handleGeometry(
  ordered: TimelineCurveKey[],
  keyIndex: unknown,
  side: unknown,
  options: TemporalCurveOptions,
): TemporalHandleGeometry | null {
...
export function temporalHandleGeometry(
  keys: unknown,
  keyIndex: unknown,
  side: unknown,
  options: TemporalCurveOptions = {},
...
export function selectedTemporalHandles(
  keys: unknown,
  selectedFrames: unknown,
  options: TemporalCurveOptions = {},
): TemporalHandleGeometry[] {
  const ordered = orderedTemporalKeys(keys);
  const selected = selectedFrames instanceof Set
    ? selectedFrames
    : new Set(Array.isArray(selectedFrames) ? selectedFrames : []);
  const handles: TemporalHandleGeometry[] = [];
...
export function temporalHandleFromPoint(
  side: unknown,
  frame: unknown,
  adjacentFrame: unknown,
  x: unknown,
  y: unknown,
  options: TemporalCurveOptions = {},
): TimelineTemporalHandle | null {
