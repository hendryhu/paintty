...
interface PopupFocusOptions {
  initialFocus?: string | HTMLElement | ((node: HTMLElement) => HTMLElement | null | undefined) | undefined;
  returnFocus?: HTMLElement | null | undefined;
  restoreFocus?: boolean | undefined;
}

function updatePopupCount(delta: number): void {
  popupCount = Math.max(0, popupCount + delta);
  popupOpen.set(popupCount > 0);
}

function focusTarget(node: HTMLElement, options: PopupFocusOptions): void {
  const requested = typeof options?.initialFocus === 'function'
    ? options.initialFocus(node)
    : options?.initialFocus;
  const target = typeof requested === 'string'
    ? node.querySelector<HTMLElement>(requested)
    : requested || node.querySelector<HTMLElement>(
...
export function popupFocus(node: HTMLElement, options: PopupFocusOptions = {}) {
  const previousFocus = (options.returnFocus || globalThis.document?.activeElement) as HTMLElement | null | undefined;
  let current = options;
  let active = true;
...
export function isTopPopup(node: HTMLElement | null | undefined): boolean {
