...
const SHAPE_KINDS = new Set<EditorShapeKind>(['line', 'rect', 'circle', 'polygon']);
...
type ShapePathComponentId = 'anchor' | 'rotation' | `vertex:${number}`;
type ShapePathComponentValue = EditorPoint | number;

function isRecord(value: unknown): value is Record<string, unknown> {
  return value !== null && typeof value === 'object';
}

function isShapeKind(value: unknown): value is EditorShapeKind {
  return typeof value === 'string' && SHAPE_KINDS.has(value as EditorShapeKind);
}
...
function normalizeRotation(value: unknown): number | null {
  ...
  return Math.abs(normalized) < 1e-9 || Math.abs(normalized - 360) < 1e-9
    ? 0
    : cleanNumber(normalized);
}

function normalizePoint(value: unknown): EditorPoint | null {
...
function normalizeVertices(value: unknown, kind: EditorShapeKind): EditorPoint[] | null {
  ...
  return vertices.every((point): point is EditorPoint => point !== null) ? vertices : null;
}

function withAdvancedGeometry(
  normalized: EditorShapePath,
  ...
  if (Object.prototype.hasOwnProperty.call(value, 'rotationAspect')) {
    const rotationAspect = value['rotationAspect'];
    if (!finite(rotationAspect) || rotationAspect <= 0) return null;
    normalized.rotationAspect = cleanNumber(rotationAspect);
  }
  return normalized;
}

// Half-cell precision is valid only for renderers that can represent subcells.
function geometryQuantum(shape: EditorShape): number {
  const subcell = shape.detail === 'half' || shape.detail === 'quarter';
  return subcell && shape.style !== 'special' && shape.style !== 'slope' ? 0.5 : 1;
}

function snapGeometry(value: number, quantum: number): number {
...
export function normalizeShapePathKey(
  value: unknown,
  expectedKind?: EditorShapeKind,
): EditorShapePath | null {
  if (!isRecord(value) || !isShapeKind(value['kind'])) return null;
  ...
  const w = value['w'];
  const h = value['h'];
  if (!finite(cx) || !finite(cy) || !finite(w) || !finite(h)) return null;
  return withAdvancedGeometry({
    kind,
    cx: cleanNumber(cx),
    cy: cleanNumber(cy),
    w: cleanNumber(Math.max(1, w)),
    h: cleanNumber(Math.max(1, h)),
  }, value);
}

export function editShapePathField(
  current: unknown,
  field: string,
  value: unknown,
  gestureStart: unknown = null,
): EditorShapePath | null {
  ...
  const fields: string[] = path.kind === 'line'
    ? ['x0', 'y0', 'x1', 'y1']
...
export function pathValueFromShape(shape: EditorShape | null | undefined): EditorShapePath | null {
  if (!shape || !SHAPE_KINDS.has(shape.kind)) return null;
  ...
  return normalizeShapePathKey({
    ...
    ...(Object.prototype.hasOwnProperty.call(shape, 'rotation')
      ...
      : {}),
    ...(Object.prototype.hasOwnProperty.call(shape, 'rotationAspect')
      ? { rotationAspect: shape.rotationAspect }
      : {}),
  }, shape.kind);
}

export function shapeWithPathValue(shape: EditorShape, value: unknown): EditorShape {
  ...
  if (path.kind === 'polygon') {
    ...
    return {
      ...
      ...(Object.prototype.hasOwnProperty.call(path, 'rotation')
        ...
        : {}),
      ...(Object.prototype.hasOwnProperty.call(path, 'rotationAspect')
        ? { rotationAspect: path.rotationAspect }
        : {}),
    };
  }
  if (path.kind === 'line') {
...
export function interpolateShapePath(from: unknown, to: unknown, progress: unknown): EditorShapePath | null {
  ...
  let interpolated: EditorShapePath;
  if (left.kind === 'polygon') {
    const matching = right as Extract<EditorShapePath, { kind: 'polygon' }>;
    if (left.vertices.length !== matching.vertices.length) return null;
    interpolated = {
      kind: 'polygon',
      vertices: left.vertices.map((point, index) => ({
        x: interpolateNumber(point.x, matching.vertices[index]!.x, t),
        y: interpolateNumber(point.y, matching.vertices[index]!.y, t),
      })),
    };
  } else if (left.kind === 'line') {
    const matching = right as Extract<EditorShapePath, { kind: 'line' }>;
    interpolated = {
      kind: 'line',
      x0: interpolateNumber(left.x0, matching.x0, t),
      y0: interpolateNumber(left.y0, matching.y0, t),
      x1: interpolateNumber(left.x1, matching.x1, t),
      y1: interpolateNumber(left.y1, matching.y1, t),
    };
  } else {
    const matching = right as Extract<EditorShapePath, { kind: 'rect' | 'circle' }>;
    interpolated = {
      kind: left.kind,
      cx: interpolateNumber(left.cx, matching.cx, t),
      cy: interpolateNumber(left.cy, matching.cy, t),
      w: interpolateNumber(left.w, matching.w, t),
      h: interpolateNumber(left.h, matching.h, t),
    };
  }
  ...
  if (Object.prototype.hasOwnProperty.call(left, 'rotationAspect') ||
    Object.prototype.hasOwnProperty.call(right, 'rotationAspect')) {
    interpolated.rotationAspect = interpolateNumber(
      left.rotationAspect || 1,
      right.rotationAspect || 1,
      t,
    );
  }
  return interpolated;
...
export function translateShapePathKey(value: unknown, dx: unknown, dy: unknown): EditorShapePath | null {
  const path = normalizeShapePathKey(value);
  if (!path || !finite(dx) || !finite(dy)) return null;
  let translated: EditorShapePath;
  if (path.kind === 'polygon') {
    translated = {
      ...path,
...
export function cloneShapePathKey(value: unknown): EditorShapePath | null {
  return normalizeShapePathKey(value);
}

export function shapePathVertices(value: unknown): EditorPoint[] {
  const path = normalizeShapePathKey(value);
  if (!path) return [];
  if (path.kind === 'polygon') return path.vertices.map((point) => ({ ...point }));
...
export function shapePathDefaultAnchor(value: unknown): EditorPoint | null {
  const path = normalizeShapePathKey(value);
  if (!path) return null;
...
export function enumerateShapePathComponents(value: unknown): Array<{
  id: ShapePathComponentId;
  type: 'vertex' | 'anchor' | 'rotation';
  index?: number;
  label: string;
}> {
  const path = normalizeShapePathKey(value);
  ...
  return [
    ...shapePathVertices(path).map((_, index) => ({
      id: `vertex:${index}` as const,
      type: 'vertex' as const,
      index,
      label: `Vertex ${index + 1}`,
    })),
    {
      id: SHAPE_PATH_COMPONENT_ANCHOR as 'anchor',
      type: 'anchor' as const,
      label: 'Anchor point',
    },
    {
      id: SHAPE_PATH_COMPONENT_ROTATION as 'rotation',
...
export function normalizeShapePathComponentId(componentId: unknown, value: unknown): ShapePathComponentId | null {
  const path = normalizeShapePathKey(value);
  if (!path) return null;
...
export function shapePathComponentValue(value: unknown, componentId: unknown): ShapePathComponentValue | null {
  const path = normalizeShapePathKey(value);
  ...
  const point = shapePathVertices(path)[index];
  return point ? { ...point } : null;
}

export function normalizeShapePathComponentValue(componentId: unknown, value: unknown): ShapePathComponentValue | null {
...
export function withShapePathComponentValue(
  value: unknown,
  componentId: unknown,
...
export function interpolateShapePathComponent(
  componentId: unknown,
  from: unknown,
  to: unknown,
  progress: unknown,
): ShapePathComponentValue | null {
...
export function shapePathComponentEqual(componentId: unknown, first: unknown, second: unknown): boolean {
...
export function shapePathEqual(a: unknown, b: unknown): boolean {
  ...
  const geometryEqual = left.kind === 'polygon'
    ? true
    : (left.kind === 'line'
      ? left.x0 === (right as Extract<EditorShapePath, { kind: 'line' }>).x0 &&
        left.y0 === (right as Extract<EditorShapePath, { kind: 'line' }>).y0 &&
        left.x1 === (right as Extract<EditorShapePath, { kind: 'line' }>).x1 &&
        left.y1 === (right as Extract<EditorShapePath, { kind: 'line' }>).y1
      : left.cx === (right as Extract<EditorShapePath, { kind: 'rect' | 'circle' }>).cx &&
        left.cy === (right as Extract<EditorShapePath, { kind: 'rect' | 'circle' }>).cy &&
        left.w === (right as Extract<EditorShapePath, { kind: 'rect' | 'circle' }>).w &&
        left.h === (right as Extract<EditorShapePath, { kind: 'rect' | 'circle' }>).h);
  if (!geometryEqual) return false;
  if (!!left.vertices !== !!right.vertices) return false;
  const rightVertices = right.vertices;
  if (left.vertices && rightVertices && (
    left.vertices.length !== rightVertices.length ||
    left.vertices.some((point, index) =>
      point.x !== rightVertices[index]!.x || point.y !== rightVertices[index]!.y)
  )) return false;
  if (!!left.anchor !== !!right.anchor) return false;
  const rightAnchor = right.anchor;
  if (left.anchor && rightAnchor && (
    left.anchor.x !== rightAnchor.x || left.anchor.y !== rightAnchor.y
  )) return false;
