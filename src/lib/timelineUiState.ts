import type { TimelineTag, TimelineTickRange } from './types/timeline-models.js';
import type { TimelineKeyboardEventLike, TimelineTagMarker } from './types/timeline-ui.js';
...
interface TimelineTagTargetInput {
  tick?: unknown;
  rowId?: unknown;
  surface?: unknown;
  valid?: unknown;
...
type PendingTimelineTagMarker = TimelineTag & {
  cluster?: true;
  customCount?: number;
  customIds?: string[];
  customValues?: string[];
...
export function trackHeaderDividerGeometry(headerWidth: unknown) {
...
export function timelineToolForShortcut(
  event: TimelineKeyboardEventLike,
  options: TimelineShortcutOptions = {},
): 'select' | 'razor' | 'tag' | null {
...
export function timelineZoomForShortcut(
  event: TimelineKeyboardEventLike,
  currentZoom: unknown,
  options: TimelineShortcutOptions = {},
) {
...
export function timelineTransportStatus(playheadTick: unknown, durationTicks: unknown) {
...
function normalizeTimelineTagTarget(target: TimelineTagTargetInput | null | undefined): TimelineTagTarget {
...
export function planTimelineTagGesture(
  startTarget: TimelineTagTargetInput | null | undefined,
  currentTarget: TimelineTagTargetInput | null | undefined,
  pointerDistance: unknown = 0,
  wasDragged = false,
) {
...
export function planTimelineTagMove(
  tag: TimelineTag | null | undefined,
  deltaPixels: unknown,
...
export function planTimelineMutationTransition(
  previousRevision: unknown,
  revision: unknown,
  gestures: { pointerEdit?: unknown; headerResize?: unknown } = {},
) {
...
export function normalizeTimelineTool(value: unknown): 'select' | 'razor' | 'tag' {
  return value === 'razor' || value === 'tag' ? value : 'select';
}

export function maximumTrackHeaderWidth(viewportWidth: unknown = Infinity): number {
...
export function clampTrackHeaderWidth(value: unknown, viewportWidth: unknown = Infinity): number {
...
export function loadTrackHeaderWidth(storage: Pick<Storage, 'getItem'> | null = browserStorage()): number {
  try {
...
export function persistTrackHeaderWidth(
  value: unknown,
...
export function resizeTrackHeaderWithKey(
  event: TimelineKeyboardEventLike,
  currentWidth: unknown,
  viewportWidth: unknown = Infinity,
) {
  let next = null;
...
export function rulerTickFromPixel(
  pixel: unknown,
  pixelsPerTick: unknown,
  durationTicks: unknown,
): number | null {
...
export function clampedRulerTickFromPixel(
  pixel: unknown,
  pixelsPerTick: unknown,
  durationTicks: unknown,
): number | null {
...
export function timelineTagMarkerLayout(
  tickPixel: unknown,
  stackIndex: unknown = 0,
  pixelsPerTick: unknown = 14,
) {
...
export function timelineExtentTicks(
  contentDurationTicks: unknown,
  laneViewportWidth: unknown,
  pixelsPerTick: unknown,
): number {
...
export function buildTimelineTagMarkers(
  tags: TimelineTag[],
  range: TimelineTickRange,
): TimelineTagMarker[] {
  const grouped = new Map<number, TimelineTag[]>();
  ...
  const markers: TimelineTagMarker[] = [];
  for (const tick of [...grouped.keys()].sort((first, second) => first - second)) {
    ...
    const tickMarkers: PendingTimelineTagMarker[] = entries.filter((tag) => tag.type !== 'custom');
    const firstCustom = custom[0];
    if (custom.length === 1 && firstCustom) tickMarkers.push(firstCustom);
    else if (custom.length > 1 && firstCustom) {
      tickMarkers.push({
        id: `custom-cluster:${tick}`,
        tick,
        type: 'custom',
        value: firstCustom.value,
        cluster: true,
