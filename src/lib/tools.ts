...
import {
  ...
  GRID_H,
  isBackgroundLayer,
  isEditingContentMask,
  isEditingEffectMask,
} from './grid.js';
import { isBackgroundPaintTarget } from './toolAvailability.js';
...
function pushRecent(color: string | null | undefined): void {
  if (!color) return;
...
function paintingBackground(): boolean {
  const id = get(activeLayerId);
  const layer = get(layers).find((l) => l.id === id);
  if (isBackgroundLayer(layer) || isBackgroundPaintTarget(layer)) return true;
  if (isEditingContentMask(layer)) return true;
  return false;
}

function clearOverlappingWide(x: number, y: number): void {
...
function paintBrush(x: number, y: number): false | void | true {
...
function paintSubcell(x: number, y: number, fx: number, fy: number): false | void | true {
  const res = get(toolOptions).subcell.mode || get(toolOptions).subcell.resolution || 'half';
  const top = fy < 0.5;
  const left = fx < 0.5;
  const bits = bitsForStroke(res as 'half' | 'quarter', top, left);
  clearOverlappingWide(x, y);
  const prev = getCell(x, y);
...
function existingSpecialConnections(x: number, y: number, style: string): number {
...
function clearSpecialOverlap(x: number, y: number): void {
  const here = getCell(x, y);
  const left = getCell(x - 1, y);
...
function planSpecialBrushPath(points: EditorPoint[], style: string) {
...
export function previewSpecialBrushGlyph(points: EditorPoint[], x: number, y: number, style: string = 'single'): string {
  if (!BOX_STYLES[style as EditorBoxStyle]) return '';
  const glyph = planSpecialBrushPath(points, style).find((item) => item.x === x && item.y === y);
  return glyph ? specialGlyph(glyph.connections, style) : BOX_STYLES[style as EditorBoxStyle].h;
}

export function paintSpecialBrushPath(points: EditorPoint[], style: string = 'single'): number {
  if (!BOX_STYLES[style as EditorBoxStyle]) return 0;
...
export function constrainDraggedShapeEndpoint(
  shape: EditorShape,
  which: 'a' | 'b',
  x: number,
  y: number,
): EditorShape {
  const constrainFromFixedStart = (value: EditorShape) => {
...
export function visibleColorFromCell(cell: EditorCellValue, preferBackground = false): string | null {
  if (!cell) return null;
  return preferBackground ? cell.bg || cell.fg || null : cell.fg || cell.bg || null;
}

export function displayedSampleCell(terminalCell: EditorCellValue, rasterColor: string | null): EditorCellValue {
...
function eyedrop(
  x: number,
  y: number,
  ev: EditorPointerModifiers | null | undefined,
  sampledCell: EditorCellValue,
): void {
...
function fill(x: number, y: number, fx: number, fy: number): false | void | true {
  const g = get(grid);
  const opts = get(toolOptions).fill;
  const selected = get(cellSelection);
  const allowed = (cx: number, cy: number) => selected.size === 0 || selected.has(`${cx},${cy}`);
  if (!allowed(x, y)) return false;
  const comp = opts.sampleAll ? g : null;
  const sample = opts.sampleAll
    ? (cx: number, cy: number) => (inBounds(cx, cy) ? comp![cy]?.[cx] ?? null : null)
    : (cx: number, cy: number) => getCell(cx, cy);
  ...
  const sameCell = (a: EditorCellValue, b: EditorCellValue): boolean => {
    if (bgLayer) return (a?.bg ?? null) === (b?.bg ?? null);
    const kind = (cell: EditorCellValue) => cell?.cont ? 'continuation' : (cell?.c ? 'glyph' : 'empty');
    const aKind = kind(a);
    const bKind = kind(b);
  ...
  const updateMap = new Map<string, EditorCellUpdate>();
  const queue = (cx: number, cy: number, cell: EditorCellValue) => updateMap.set(`${cx},${cy}`, { x: cx, y: cy, cell });
  if (bgLayer) {
    for (const point of points) queue(point.x, point.y, { ...newCell });
  } else {
    const targetKeys = new Set(points.map((point) => `${point.x},${point.y}`));
    const writePoints: Array<EditorPoint & { leader: boolean }> = [];
...
function fillEffectMask(active: EditorEffectLayer, x: number, y: number): false | void | true {
  const opts = get(toolOptions).fill;
  const selected = get(cellSelection);
  const allowed = (cx: number, cy: number) => selected.size === 0 || selected.has(`${cx},${cy}`);
  if (!allowed(x, y)) return false;
  const fallback = active.mask?.defaultStrength ?? 1;
  const strengthAt = (cx: number, cy: number) => getCell(cx, cy)?.mask ?? fallback;
...
function pickEffectMask(active: EditorEffectLayer, x: number, y: number): void {
...
export function applyTool(
  x: number,
  y: number,
  ev: EditorPointerModifiers | null,
  kind: 'down' | 'move' | 'up',
  fx = 0.5,
  fy = 0.5,
  sampledCell: EditorCellValue = undefined,
  toolOverride: EditorTool | null = null,
): false | void | true | number {
  const tool = toolOverride || get(activeTool);
  const active = get(layers).find((layer) => layer.id === get(activeLayerId));
  if (active?.type === 'effect' && isEditingEffectMask(active)) {
    if (tool === 'brush') {
      const color = get(paintColor);
  ...
  switch (tool) {
    ...
    case 'subcell': {
      ...
      return BOX_STYLES[mode as EditorBoxStyle]
